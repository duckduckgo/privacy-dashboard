export default {
  "connection": {
    "encrypt": {
      "title": "Krypter"
    },
    "sign": {
      "title": "Log ind"
    },
    "verify": {
      "title": "Verificer"
    },
    "derive": {
      "title": "Udled"
    },
    "wrap": {
      "title": "Pak"
    },
    "unwrap": {
      "title": "Pak"
    },
    "unknown": {
      "title": "Ukendt"
    },
    "certificateDetail": {
      "title": "Detaljer om sikkerhedscertifikat"
    },
    "commonName": {
      "title": "Almindeligt navn"
    },
    "publicKey": {
      "title": "Offentlig nøgle"
    },
    "algorithm": {
      "title": "Algoritme"
    },
    "keySize": {
      "title": "Nøglestørrelse"
    },
    "usage": {
      "title": "Anvendelse"
    },
    "summary": {
      "title": "Resumé"
    },
    "permanent": {
      "title": "Permanent"
    },
    "effectiveSize": {
      "title": "Effektiv størrelse"
    },
    "certificateNotFound": {
      "title": "Certifikat ikke fundet"
    },
    "certificateForDomain": {
      "title": "Certifikat for {domain}"
    },
    "insecureConnectionDesc": {
      "title": "Denne side bruger en ukrypteret forbindelse. Tredjeparter kan muligvis se din aktivitet eller opfange følsomme oplysninger, som du sender på denne side."
    },
    "invalidConnectionDesc": {
      "title": "Certifikatet for dette websted er ugyldigt. Du opretter muligvis forbindelse til en server, der foregiver at være <b>{domain}</b>, hvilket kan bringe dine fortrolige oplysninger i fare."
    },
    "upgradedConnectionDesc": {
      "title": "Vi har opgraderet forbindelsen på denne side for at beskytte de oplysninger, du sender, mens de overføres."
    },
    "secureConnectionDesc": {
      "title": "Denne side bruger en krypteret forbindelse, hvilket forhindrer tredjeparter i at se dine aktiviteter eller opsnappe følsomme oplysninger, som du sender på denne side."
    }
  },
  "ctascreens": {
    "protectionsUnavailableNote": {
      "title": "Beskyttelse af privatlivet er ikke tilgængelig for særlige sider eller lokale sider."
    },
    "spreadTitle": {
      "title": "Kan du lide at bruge DuckDuckGo?"
    },
    "spreadText": {
      "title": "Hjælp os med at sprede budskabet til din familie og venner"
    },
    "spreadButton": {
      "title": "Del DuckDuckGo"
    },
    "emailTitle": {
      "title": "Er du træt af, at e-mails bliver sporet?"
    },
    "emailText": {
      "title": "Tilmeld dig DuckDuckGo Email Protection til din udvidelse nu!"
    }
  },
  "firebutton": {
    "fireDialogHeader": {
      "title": "Luk faner og ryd data"
    },
    "fireDialogHeaderNoTabs": {
      "title": "Ryd data"
    },
    "optionCurrentSite": {
      "title": "Kun aktuelt websted"
    },
    "optionLastHour": {
      "title": "Sidste time"
    },
    "optionLast24Hour": {
      "title": "Sidste 24 timer"
    },
    "optionLast7days": {
      "title": "De seneste 7 dage"
    },
    "optionLast4Weeks": {
      "title": "De seneste 4 uger"
    },
    "optionAllTime": {
      "title": "Altid"
    },
    "historyDuration": {
      "title": "{duration, select, hour {en time} day {24 timer} week {en uge} month {4 uger} other {Alle}}"
    },
    "summaryClearTabsHistoryDuration": {
      "title": "{openTabs, plural, one {Vil du lukke <b>{openTabs}</b> fane og rydde <b>{durationDesc}</b>s browserhistorik og cookies?} other {Vil du lukke <b>{openTabs}</b> faner og rydde <b>{durationDesc}</b>s browserhistorik og cookies?}}"
    },
    "summaryClearTabsDuration": {
      "title": "{openTabs, plural, one {Vil du lukke <b>{openTabs}</b> fane og rydde <b>{durationDesc}</b>s cookies?} other {Vil du lukke <b>{openTabs}</b> faner og rydde <b>{durationDesc}</b>s cookies?}}"
    },
    "summaryClearHistoryDuration": {
      "title": "Vil du rydde browserhistorik og cookies for de seneste <b>{durationDesc}</b>?"
    },
    "summaryClearCookiesDuration": {
      "title": "Vil du rydde cookies for de seneste <b>{durationDesc}</b>?"
    },
    "summaryClearTabsHistoryAll": {
      "title": "Luk <b>{openTabs}</b> {openTabs, plural, =1 {fane} other {faner}}, og ryd <b>al</b> browserhistorik og alle cookies ({cookies} {cookies, plural, =1 {websted} other {websteder}})?"
    },
    "summaryClearTabsAll": {
      "title": "Luk <b>{openTabs}</b> {openTabs, plural, =1 {fane} other {faner}}, og slet <b>alle</b> cookies ({cookies} {cookies, plural, =1 {websted} other {websteder}})?"
    },
    "summaryClearHistoryAll": {
      "title": "{cookies, plural, one {Ryd <b>al</b> browserhistorik og cookies ({cookies} websted)?} other {Ryd <b>al</b> browserhistorik og cookies ({cookies} websteder)?}}"
    },
    "summaryClearCookiesAll": {
      "title": "{cookies, plural, one {Vil du rydde <b>alle</b> cookies ({cookies} websted)?} other {Vil du rydde <b>alle</b> cookies ({cookies} websteder)?}}"
    },
    "summaryClearTabsHistorySite": {
      "title": "{openTabs, plural, one {Vil du lukke <b>{openTabs} fane {site}</b> og rydde <b>alle {site}</b> cookies?} other {Vil du lukke <b>{openTabs} faner {site}</b> og rydde <b>alle {site}</b> cookies?}}"
    },
    "summaryClearTabsSite": {
      "title": "{openTabs, plural, one {Vil du lukke <b>{openTabs} fane {site}</b> og rydde <b>alle {site}</b> cookies?} other {Vil du lukke <b>{openTabs} faner {site}</b> og rydde <b>alle {site}</b> cookies?}}"
    },
    "summaryClearHistorySite": {
      "title": "Vil du rydde al browserhistorik og alle cookies for <b>{site}</b>?"
    },
    "summaryClearCookiesSite": {
      "title": "Vil du rydde alle cookies for <b>{site}</b>?"
    },
    "summaryPinnedIgnored": {
      "title": "{tabs, plural, one {<b>{tabs} fastgjort</b> fane ignoreres.} other {<b>{tabs} fastgjorte</b> faner ignoreres.}}"
    },
    "clearData": {
      "title": "Nulstil"
    },
    "cancel": {
      "title": "Annullér"
    },
    "historyAndDownloadsNotAffected": {
      "title": "Vælg en tidsperiode for også at rydde historikken."
    }
  },
  "permissions": {
    "camera": {
      "title": "Kamera"
    },
    "microphone": {
      "title": "Mikrofon"
    },
    "geolocation": {
      "title": "Geolokalisering"
    },
    "popups": {
      "title": "Pop op-vinduer"
    },
    "ask": {
      "title": "Spørg hver gang"
    },
    "notify": {
      "title": "Underret"
    },
    "grant": {
      "title": "Tillad altid"
    },
    "deny": {
      "title": "Afvis altid"
    }
  },
  "report": {
    "selectTheOptionDesc": {
      "title": "Hvis du indsender en anonym rapport om en defekt side, hjælper det os med at forbedre appen."
    },
    "selectTheOptionDescV2": {
      "title": "Vælg den mulighed, der bedst beskriver det problem, du oplevede."
    },
    "selectTheCategoryType": {
      "title": "Hvad er problemet?"
    },
    "categoryType1": {
      "title": "Webstedet fungerer ikke som forventet"
    },
    "categoryType2": {
      "title": "Jeg kan ikke lide indholdet på dette websted"
    },
    "categoryType3": {
      "title": "Generel feedback til DuckDuckGo-browseren"
    },
    "selectTheCategory": {
      "title": "Hvad er det, der ikke fungerer på dette websted?"
    },
    "tryTurningProtectionsOff": {
      "title": "Prøv at slå beskyttelse af personlige oplysninger fra for at se, om det løser problemet."
    },
    "skipThisStep": {
      "title": "Spring dette trin over"
    },
    "pickYourIssueFromTheList": {
      "title": "Beskriv hvad der skete"
    },
    "blocked": {
      "title": "Webstedet er blokeret eller blev ikke indlæst"
    },
    "layout": {
      "title": "Sidens layout er ødelagt"
    },
    "emptySpaces": {
      "title": "Siden indeholder store tomme områder"
    },
    "videos": {
      "title": "Videoen blev ikke afspillet eller indlæst"
    },
    "images": {
      "title": "Billeder blev ikke indlæst"
    },
    "comments": {
      "title": "Kommentarer, anmeldelser eller chats blev ikke indlæst"
    },
    "content": {
      "title": "Indhold mangler"
    },
    "links": {
      "title": "Links eller knapper fungerer ikke"
    },
    "login": {
      "title": "Kan ikke logge ind/tilmelde"
    },
    "loginV2": {
      "title": "Kan ikke logge ind eller tilmelde"
    },
    "shopping": {
      "title": "Kan ikke betale, tjekke ud eller shoppe"
    },
    "browser": {
      "title": "Problem med browser eller anden browserudvidelse"
    },
    "paywall": {
      "title": "Siden bad mig om at deaktivere annonceblokering"
    },
    "other": {
      "title": "Noget andet"
    },
    "tellUsMoreDesc": {
      "title": "Del flere detaljer (valgfrit):{bullet}Hvad skete der?{bullet}Hvad burde være sket?{bullet}Hjalp det at slå beskyttelse fra?"
    },
    "sendReport": {
      "title": "Send rapport"
    },
    "reportsAreAnonymousDesc": {
      "title": "Rapporter, der sendes til DuckDuckGo, indeholder kun oplysninger, som er nødvendige for at hjælpe os med at behandle din feedback."
    },
    "thankYou": {
      "title": "Tak!"
    },
    "yourReportWillHelpDesc": {
      "title": "Din rapport vil være med til at forbedre vores produkter og gøre oplevelsen bedre for andre brugere."
    },
    "dislike": {
      "title": "Jeg kan ikke lide indholdet"
    },
    "otherRequired": {
      "title": "Beskriv det problem, du oplevede (påkrævet)"
    },
    "otherOptional": {
      "title": "Beskriv det problem, du oplevede (valgfrit)"
    },
    "suggestionWhatHappened": {
      "title": "Hvad skete der?"
    },
    "suggestionWhatHappened2": {
      "title": "Hvad skulle der være sket?"
    },
    "suggestionWhatHappened3": {
      "title": "Hjalp det at slå beskyttelse af personlige oplysninger fra?"
    },
    "reportTitle": {
      "title": "Rapportér til DuckDuckGo"
    }
  },
  "shared": {
    "trackers": {
      "title": "Trackere"
    }
  },
  "site": {
    "updatingProtectionList": {
      "title": "Opdatering af beskyttelseslisten"
    },
    "protectionsEnabled": {
      "title": "Beskyttelse er <b>SLÅET TIL</b> for dette websted"
    },
    "protectionsDisabled": {
      "title": "Beskyttelse er <b>SLÅET FRA</b> for dette websted"
    },
    "protectionsDisabledRemote": {
      "title": "Vi har midlertidigt deaktiveret beskyttelse af privatlivet, da det ser ud til at påvirke visning af dette websted."
    },
    "protectionsDisabledRemoteOverride": {
      "title": "Vi anbefaler, at du deaktiverer beskyttelse af privatlivet for dette websted for at forhindre påvirkning af visning af webstedet."
    },
    "connectionDescriptionUnencrypted": {
      "title": "<b>Dette websted er ikke sikkert</b> og kan lække alle oplysninger, du sender på denne side."
    },
    "connectionDescriptionInvalidCertificate": {
      "title": "<b>Certifikatet for dette websted er ugyldigt.</b> Du opretter muligvis forbindelse til en server, der foregiver at være <b>{domain}</b>, hvilket kan bringe dine fortrolige oplysninger i fare."
    },
    "trackerNetworksSummaryBlocked": {
      "title": "Følgende anmodninger fra tredjepartsdomæner blev blokeret fra at blive indlæst, fordi de blev identificeret som sporingsanmodninger. Hvis en virksomheds anmodninger indlæses, kan det give dem mulighed for at profilere dig."
    },
    "trackerNetworksSummaryNoneBlocked": {
      "title": "Ingen sporingsanmodninger blev blokeret fra indlæsning på denne side. Hvis en virksomheds anmodninger indlæses, kan det give dem mulighed for at profilere dig."
    },
    "trackerNetworksSummaryNoneFound": {
      "title": "Vi har ikke identificeret nogen sporingsanmodninger på denne side."
    },
    "trackerNetworksSummaryNone": {
      "title": "Vi har ikke fundet nogen virksomheder, der forsøger at indlæse sporingsanmodninger på denne side."
    },
    "trackerNetworksSummaryAllowedOnly": {
      "title": "For at forhindre påvirkning af visning af webstedet, har vi ikke blokeret nogen virksomheder fra at indlæse sporingsanmodninger på denne side."
    },
    "trackerNetworksSummaryProtectionsOff": {
      "title": "Ingen sporingsanmodninger blev blokeret fra at blive indlæst, fordi beskyttelse er slået fra på dette websted. Hvis en virksomheds anmodninger indlæses, kan det give dem mulighed for at profilere dig."
    },
    "createNewDuckAddress": {
      "title": "Opret privat Duck-adresse"
    },
    "createNewDuckAddressCopied": {
      "title": "Kopieret til udklipsholderen!"
    },
    "websiteNotWorkingQ": {
      "title": "Fungerer webstedet ikke som forventet?"
    },
    "websiteNotWorkingPrompt": {
      "title": "Virker webstedet ikke?"
    },
    "websiteNotWorkingCta": {
      "title": "Rapportér ødelagt websted"
    },
    "websiteNotWorkingAdvice": {
      "title": "Det kan hjælpe at slå beskyttelserne FRA."
    },
    "websiteNotWorkingPromptFollowUp": {
      "title": "Virkerwebstedet stadig ikke?"
    },
    "takePrecautions": {
      "title": "Tag forholdsregler"
    },
    "majorTrackingNetworkDesc": {
      "title": "{blocked, select, true {<b>Dette websted er ejet af {companyDisplayName}</b>, som driver et sporingsnetværk på tværs af {companyPrevalence} % af de mest populære websteder. Det lykkedes os at blokere nogle af deres anmodninger på denne side.} other {<b>Dette websted er ejet af {companyDisplayName}</b>, som driver et sporingsnetværk på tværs af {companyPrevalence} % af de mest populære websteder. }}"
    },
    "trackersBlockedDesc": {
      "title": "{companyCount, plural, =0 {Vi har blokeret nogle virksomheder fra at indlæse sporingsanmodninger på denne side.} =2 {Vi blokerede <b>{firstCompany}</b> og <b>{secondCompany}</b> fra at indlæse sporingsanmodninger på denne side.} =3 {Vi har blokeret <b>{firstCompany}</b>, <b>{secondCompany}</b> og <b>{thirdCompany}</b> fra at indlæse sporingsanmodninger på denne side.} =4 {Vi har blokeret <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b> og <b>{fourthCompany}</b> fra at indlæse sporingsanmodninger på denne side.} =5 {Vi har blokeret <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b>, <b>{fourthCompany}</b> og <b>1 anden</b> fra at indlæse sporingsanmodninger på denne side.} one {Vi har blokeret <b>{firstCompany}</b> fra at indlæse sporingsanmodninger på denne side.} other {Vi har blokeret <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b>, <b>{fourthCompany}</b> og <b>{othersCount} andre</b> fra at indlæse sporingsanmodninger på denne side.}}"
    },
    "cookiesMinimized": {
      "title": "Administrerede cookies"
    },
    "cookiesHidden": {
      "title": "Pop op-vindue om cookies skjult"
    },
    "cookiesHiddenSummary": {
      "title": "Vi var kun i stand til at skjule pop op-vinduet på dette websted, fordi der ikke var nogen muligheder for at administrere cookiepræferencer. Vores andre beskyttelser mod sporing gælder stadig."
    },
    "cookiesMinimizedSummary": {
      "title": "Vi har sat dine cookie-indstillinger til at maksimere fortrolighed og lukket pop op-vinduet."
    },
    "cookiesMinimizedSettings": {
      "title": "Deaktiver i indstillinger"
    },
    "connectionSecure": {
      "title": "Forbindelsen er krypteret"
    },
    "connectionNotSecure": {
      "title": "Forbindelsen er ikke krypteret"
    },
    "connectionNotSecureInvalidCertificate": {
      "title": "Forbindelsen er muligvis usikker"
    },
    "trackerNetworksDesc": {
      "title": "Anmodninger blokeret fra indlæsning"
    },
    "trackerNetworksNotBlocked": {
      "title": "Ingen sporingsanmodninger blokeret"
    },
    "trackerNetworksNotFound": {
      "title": "Ingen sporingsanmodninger fundet"
    },
    "thirdPartiesLoaded": {
      "title": "Anmodninger fra tredjeparter"
    },
    "thirdPartiesNoneFound": {
      "title": "Ingen anmodninger fra tredjepart fundet"
    },
    "firstPartyDesc": {
      "title": "{companyName} ejer dette websted og de kendte trackere, der findes på denne side, så vi blokerede dem ikke."
    },
    "noTrackersFound": {
      "title": "Vi har ikke fundet nogen virksomheder, der forsøger at indlæse sporingsanmodninger på denne side."
    },
    "trackersFoundForAllowlisted": {
      "title": "Trackere hjælper virksomheder med at profilere dig. Vi har fundet disse virksomheder, der overvåger din aktivitet på denne side."
    },
    "trackersFoundAndBlocked": {
      "title": "Trackere hjælper virksomheder med at profilere dig. Vi har forhindret disse virksomheder i at overvåge din aktivitet på denne side."
    },
    "trackerNetworkUnknown": {
      "title": "Tracker-netværk ukendt"
    },
    "trackerDomainsForCompany": {
      "title": "Tracker-domæner for {companyName}"
    },
    "zeroTrackersFound": {
      "title": "'{'0'}' tracker fundet"
    },
    "trackerOwnedByThisSite": {
      "title": "Trackere ejet af dette websted (tilladt)"
    },
    "trackerCountForDomain": {
      "title": "{blocked, select, true {{trackerCount, plural, one {{trackerCount} Tracker blokeret på {domain}} other {{trackerCount} Trackere blokeret på {domain}}}} other {{trackerCount, plural, one {{trackerCount} Tracker fundet på {domain}} other {{trackerCount} Trackere fundet på {domain}}}}}"
    },
    "trackerLimitationsNote": {
      "title": "Bemærk: Platformsbegrænsninger kan begrænse vores evne til at registrere alle anmodninger."
    },
    "trackerAboutLink": {
      "title": "Om vores beskyttelse mod sporing på nettet"
    },
    "trackerAdLink": {
      "title": "Sådan påvirker vores søgeannoncer vores beskyttelse"
    },
    "sectionHeadingAdAttribution": {
      "title": "Følgende domænes anmodninger blev indlæst, fordi der for nylig blev klikket på en {domain}-annonce på DuckDuckGo. Disse anmodninger hjælper med at evaluere annoncernes effektivitet. Alle annoncer på DuckDuckGo er ikke-profilerende."
    },
    "sectionHeadingIgnore": {
      "title": "Følgende domænes anmodninger blev indlæst for at forhindre forstyrrelse på webstedet."
    },
    "sectionHeadingFirstParty": {
      "title": "Følgende domæners anmodninger blev indlæst, fordi de er knyttet til {domain}."
    },
    "sectionHeadingThirdParty": {
      "title": "Følgende domænes anmodninger blev også indlæst."
    },
    "sectionHeadingProtectionsDisabled": {
      "title": "Følgende domæners anmodninger blev indlæst, fordi beskyttelsen er slået fra."
    },
    "thirdPartiesSummaryLoaded": {
      "title": "Følgende anmodninger fra tredjepartsdomæner blev indlæst. Hvis en virksomheds anmodninger indlæses, kan det give dem mulighed for at profilere dig, selv om vores andre beskyttelser mod websporing stadig anvendes."
    },
    "thirdPartiesSummaryProtectionsOff": {
      "title": "Følgende anmodninger fra tredjepartsdomæner blev indlæst. Hvis en virksomheds anmodninger indlæses, kan det give dem mulighed for at profilere dig, selv om vores andre beskyttelser mod websporing stadig anvendes."
    },
    "thirdPartiesSummaryNone": {
      "title": "Vi har ikke fundet nogen anmodninger fra tredjepartsdomæner."
    },
    "analyticsCategory": {
      "title": "Analyseværktøj"
    },
    "advertisingCategory": {
      "title": "Reklamer"
    },
    "socialCategory": {
      "title": "Sociale netværk"
    },
    "contentDeliveryCategory": {
      "title": "Levering af indhold"
    },
    "embeddedContentCategory": {
      "title": "Indlejret indhold"
    },
    "searchPlaceholder": {
      "title": "Søg med DuckDuckGo"
    },
    "searchGoButton": {
      "title": "Søg"
    },
    "optionsButton": {
      "title": "Flere muligheder"
    },
    "navigationComplete": {
      "title": "Færdig"
    },
    "navigationClose": {
      "title": "Luk"
    },
    "navigationCancel": {
      "title": "Annuller"
    },
    "navigationBack": {
      "title": "Tilbage"
    },
    "enableProtectionsSwitch": {
      "title": "Aktivér beskyttelser"
    },
    "disableProtectionsSwitch": {
      "title": "Deaktiver beskyttelser"
    },
    "errorMessage": {
      "title": "Noget gik galt, og vi kunne ikke indlæse dette indhold. Prøv at genindlæse siden."
    }
  }
}