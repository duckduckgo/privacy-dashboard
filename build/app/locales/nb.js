export default {
  "connection": {
    "encrypt": {
      "title": "Krypter"
    },
    "sign": {
      "title": "Signer"
    },
    "verify": {
      "title": "Verifiser"
    },
    "derive": {
      "title": "Hent fra"
    },
    "wrap": {
      "title": "Pakk inn"
    },
    "unwrap": {
      "title": "Pakk ut"
    },
    "unknown": {
      "title": "Ukjent"
    },
    "certificateDetail": {
      "title": "Detaljer om sikkerhetssertifikatet"
    },
    "commonName": {
      "title": "Navn"
    },
    "publicKey": {
      "title": "Offentlig nøkkel"
    },
    "algorithm": {
      "title": "Algoritme"
    },
    "keySize": {
      "title": "Nøkkelstørrelse"
    },
    "usage": {
      "title": "Bruk"
    },
    "summary": {
      "title": "Sammendrag"
    },
    "permanent": {
      "title": "Permanent"
    },
    "effectiveSize": {
      "title": "Effektiv størrelse"
    },
    "certificateNotFound": {
      "title": "Fant ikke sertifikat"
    },
    "certificateForDomain": {
      "title": "Sertifikat for {domain}"
    },
    "insecureConnectionDesc": {
      "title": "Denne siden bruker en ukryptert tilkobling. Tredjeparter kan kanskje se aktiviteten din eller fange opp sensitiv informasjon du sender på denne siden."
    },
    "invalidConnectionDesc": {
      "title": "Sertifikatet for dette nettstedet er ugyldig. Det kan være du er koblet til en server som utgir seg for å være <b>{domain}</b>, noe som kan sette din konfidensielle informasjon i fare."
    },
    "upgradedConnectionDesc": {
      "title": "Vi har oppgradert tilkoblingen på denne siden, så informasjonen du sender er beskyttet i transitt."
    },
    "secureConnectionDesc": {
      "title": "Denne siden bruker en kryptert tilkobling, som forhindrer tredjeparter i å se aktiviteten din eller fange opp sensitiv informasjon du sender på denne siden."
    }
  },
  "ctascreens": {
    "protectionsUnavailableNote": {
      "title": "Personvernbeskyttelser er ikke tilgjengelige for spesialsider eller lokale sider."
    },
    "spreadTitle": {
      "title": "Liker du å bruke DuckDuckGo?"
    },
    "spreadText": {
      "title": "Fortell familie og venner om oss"
    },
    "spreadButton": {
      "title": "Spre DuckDuckGo"
    },
    "emailTitle": {
      "title": "Lei av at e-poster blir sporet?"
    },
    "emailText": {
      "title": "Registrer deg for DuckDuckGos Email Protection for utvidelsen din nå!"
    }
  },
  "firebutton": {
    "fireDialogHeader": {
      "title": "Lukk faner og slett data"
    },
    "fireDialogHeaderNoTabs": {
      "title": "Slett data"
    },
    "optionCurrentSite": {
      "title": "Bare nåværende nettsted"
    },
    "optionLastHour": {
      "title": "Den siste timen"
    },
    "optionLast24Hour": {
      "title": "De siste 24 timene"
    },
    "optionLast7days": {
      "title": "De siste 7 dagene"
    },
    "optionLast4Weeks": {
      "title": "De siste 4 ukene"
    },
    "optionAllTime": {
      "title": "All tid"
    },
    "historyDuration": {
      "title": "{duration, select, hour {én time} day {24 timer} week {én uke} month {4 uker} other {Alle}}"
    },
    "summaryClearTabsHistoryDuration": {
      "title": "{openTabs, plural, one {Vil du lukke <b>{openTabs}</b> fane og fjerne <b>{durationDesc}</b> med nettleserhistorikk og informasjonskapsler?} other {Vil du lukke <b>{openTabs}</b> faner og fjerne <b>{durationDesc}</b> med nettleserhistorikk og informasjonskapsler?}}"
    },
    "summaryClearTabsDuration": {
      "title": "{openTabs, plural, one {Vil du lukke <b>{openTabs}</b> fane og fjerne <b>{durationDesc}</b> med informasjonskapsler?} other {Vil du lukke <b>{openTabs}</b> faner og fjerne <b>{durationDesc}</b> med informasjonskapsler?}}"
    },
    "summaryClearHistoryDuration": {
      "title": "Vil du fjerne <b>{durationDesc}</b> med nettleserhistorikk og informasjonskapsler?"
    },
    "summaryClearCookiesDuration": {
      "title": "Vil du fjerne <b>{durationDesc}</b> med informasjonskapsler?"
    },
    "summaryClearTabsHistoryAll": {
      "title": "Vil du lukke <b>{openTabs}</b> {openTabs, plural, =1 {fane} other {faner}} og fjerne <b>all</b> nettleserhistorikk og alle informasjonskapsler ({cookies} {cookies, plural, =1 {nettsted} other {nettsteder}})?"
    },
    "summaryClearTabsAll": {
      "title": "Vil du lukke <b>{openTabs}</b> {openTabs, plural, =1 {fane} other {faner}}, og fjerne <b>alle</b> informasjonskapsler ({cookies} {cookies, plural, =1 {nettsted} other {nettsteder}})?"
    },
    "summaryClearHistoryAll": {
      "title": "{cookies, plural, one {Vil du fjerne <b>all</b> nettleserhistorikk og alle informasjonskapsler ({cookies} nettsted)?} other {Vil du fjerne <b>all</b> nettleserhistorikk og alle informasjonskapsler ({cookies} nettsteder)?}}"
    },
    "summaryClearCookiesAll": {
      "title": "{cookies, plural, one {Vil du slette <b>alle</b> informasjonskapsler ({cookies} nettsted)?} other {Vil du slette <b>alle</b> informasjonskapsler ({cookies} nettsteder)?}}"
    },
    "summaryClearTabsHistorySite": {
      "title": "{openTabs, plural, one {Vil du lukke <b>{openTabs} fane for {site}</b>og slette <b>alle informasjonskapsler for {site}</b>?} other {Vil du lukke <b>{openTabs} faner for {site}</b> og slette <b>alle informasjonskapsler for {site}</b>?}}"
    },
    "summaryClearTabsSite": {
      "title": "{openTabs, plural, one {Vil du lukke <b>{openTabs} {site}</b>-fane og slette <b>alle informasjonskapsler for {site}</b>?} other {Vil du lukke <b>{openTabs} {site}</b>-faner og slette <b>alle informasjonskapsler for {site}</b>?}}"
    },
    "summaryClearHistorySite": {
      "title": "Vil du slette <b>all nettleserhistorikk og alle informasjonskapsler for {site}</b>?"
    },
    "summaryClearCookiesSite": {
      "title": "Vil du slette <b>alle informasjonskapsler for {site}</b>?"
    },
    "summaryPinnedIgnored": {
      "title": "{tabs, plural, one {<b>{tabs} festet</b> fane blir ignorert.} other {<b>{tabs} festede</b> faner blir ignorert.}}"
    },
    "clearData": {
      "title": "Klarvær"
    },
    "cancel": {
      "title": "Avbryt"
    },
    "historyAndDownloadsNotAffected": {
      "title": "Hvis du også vil tømme historikken, må du velge en tidsperiode."
    }
  },
  "permissions": {
    "camera": {
      "title": "Kamera"
    },
    "microphone": {
      "title": "Mikrofon"
    },
    "geolocation": {
      "title": "Geolokalisering"
    },
    "popups": {
      "title": "Popup-vinduer"
    },
    "ask": {
      "title": "Spør hver gang"
    },
    "notify": {
      "title": "Varsle"
    },
    "grant": {
      "title": "Alltid tillat"
    },
    "deny": {
      "title": "Alltid nekte"
    }
  },
  "report": {
    "selectTheOptionDesc": {
      "title": "Du kan hjelpe oss med å forbedre appen ved å sende inn en rapport om et defekt nettsted."
    },
    "selectTheOptionDescV2": {
      "title": "Velg alternativet som best beskriver problemet du opplevde."
    },
    "selectTheCategoryType": {
      "title": "Hva er problemet?"
    },
    "categoryType1": {
      "title": "Nettstedet fungerer ikke som forventet"
    },
    "categoryType2": {
      "title": "Jeg liker ikke innholdet på dette nettstedet"
    },
    "categoryType3": {
      "title": "Generell tilbakemelding om DuckDuckGo"
    },
    "selectTheCategory": {
      "title": "Hva er det som ikke fungerer på dette nettstedet?"
    },
    "tryTurningProtectionsOff": {
      "title": "Prøv å slå av Personvern for å se om det løser problemet."
    },
    "skipThisStep": {
      "title": "Hopp over dette trinnet"
    },
    "pickYourIssueFromTheList": {
      "title": "Beskriv hva som skjedde"
    },
    "blocked": {
      "title": "Nettstedet er blokkert eller ble ikke lastet"
    },
    "layout": {
      "title": "Nettstedets layout er ødelagt"
    },
    "emptySpaces": {
      "title": "Nettstedet inneholder store tomme områder"
    },
    "videos": {
      "title": "Videoen spilte ikke av eller ble ikke lastet"
    },
    "images": {
      "title": "Bildene ble ikke lastet inn"
    },
    "comments": {
      "title": "Kommentarer, anmeldelser eller chatter ble ikke lastet"
    },
    "content": {
      "title": "Innhold mangler"
    },
    "links": {
      "title": "Linker eller knapper fungerer ikke"
    },
    "login": {
      "title": "Kan ikke logge på / registrere meg"
    },
    "loginV2": {
      "title": "Kan ikke logge på eller registrere meg"
    },
    "shopping": {
      "title": "Kan ikke betale, gå til kassen eller kjøpe"
    },
    "browser": {
      "title": "Problem med nettleser eller annen nettleserutvidelse"
    },
    "paywall": {
      "title": "Nettstedet ba meg om å deaktivere annonseblokkering"
    },
    "other": {
      "title": "Noe annet"
    },
    "tellUsMoreDesc": {
      "title": "Del flere detaljer (valgfritt):{bullet}Hva skjedde?{bullet}Hva skulle ha skjedd?{bullet}Hjalp det å slå av beskyttelse?"
    },
    "sendReport": {
      "title": "Send rapport"
    },
    "reportsAreAnonymousDesc": {
      "title": "Rapporter som sendes til DuckDuckGo, inneholder bare den informasjonen som er nødvendig for at vi skal kunne behandle tilbakemeldingen din."
    },
    "thankYou": {
      "title": "Takk!"
    },
    "yourReportWillHelpDesc": {
      "title": "Rapporten din bidrar til å forbedre produktene våre og gjøre opplevelsen bedre for andre."
    },
    "dislike": {
      "title": "Jeg liker ikke innholdet"
    },
    "otherRequired": {
      "title": "Beskriv problemet du opplevde (obligatorisk)"
    },
    "otherOptional": {
      "title": "Beskriv problemet du opplevde (valgfritt)"
    },
    "suggestionWhatHappened": {
      "title": "Hva skjedde?"
    },
    "suggestionWhatHappened2": {
      "title": "Hva skulle ha skjedd?"
    },
    "suggestionWhatHappened3": {
      "title": "Hjalp det å slå av Personvern?"
    },
    "reportTitle": {
      "title": "Rapporter til DuckDuckGo"
    }
  },
  "shared": {
    "trackers": {
      "title": "Sporere"
    }
  },
  "site": {
    "updatingProtectionList": {
      "title": "Oppdaterer beskyttelseslisten"
    },
    "protectionsEnabled": {
      "title": "Beskyttelse er <b>PÅ</b> på dette nettstedet"
    },
    "protectionsDisabled": {
      "title": "Beskyttelse er <b>AV</b> på dette nettstedet"
    },
    "protectionsDisabledRemote": {
      "title": "Vi har midlertidig deaktivert personvernbeskyttelsene, da de ser ut til å få nettstedet til å slutte å fungere."
    },
    "protectionsDisabledRemoteOverride": {
      "title": "Vi anbefaler å deaktivere personvernbeskyttelsene for dette nettstedet for å forhindre at det slutter å fungere som det skal."
    },
    "connectionDescriptionUnencrypted": {
      "title": "<b>Dette nettstedet er ikke sikkert</b> og kan kompromittere informasjon du sender på denne siden."
    },
    "connectionDescriptionInvalidCertificate": {
      "title": "<b>Sertifikatet for dette nettstedet er ugyldig</b>. Det kan være du er koblet til en server som utgir seg for å være <b>{domain}</b>, noe som kan sette din konfidensielle informasjon i fare."
    },
    "trackerNetworksSummaryBlocked": {
      "title": "Forespørslene fra følgende tredjepartsdomener ble blokkert fra lasting, fordi de ble identifisert som sporende forespørsler. Hvis forespørslene fra et selskap blir lastet, har selskapet mulighet til å profilere deg."
    },
    "trackerNetworksSummaryNoneBlocked": {
      "title": "Ingen sporende forespørsler ble blokkert fra å bli lastet på denne siden. Hvis forespørslene fra et firma lastes, kan de profilere deg."
    },
    "trackerNetworksSummaryNoneFound": {
      "title": "Vi har ikke identifisert noen sporende forespørsler på denne siden."
    },
    "trackerNetworksSummaryNone": {
      "title": "Vi fant ingen firmaer som prøvde å laste sporende forespørsler på denne siden."
    },
    "trackerNetworksSummaryAllowedOnly": {
      "title": "For at nettstedet skal fungere, har vi unnlatt å blokkere sporende forespørsler fra firmaer på denne siden."
    },
    "trackerNetworksSummaryProtectionsOff": {
      "title": "Ingen sporende forespørsler ble blokkert fra lasting, fordi beskyttelse er slått av for dette nettstedet. Hvis forespørslene fra et selskap blir lastet, har selskapet mulighet til å profilere deg."
    },
    "createNewDuckAddress": {
      "title": "Generer privat Duck Address"
    },
    "createNewDuckAddressCopied": {
      "title": "Kopiert til utklippstavlen din!"
    },
    "websiteNotWorkingQ": {
      "title": "Virker ikke nettsiden som forventet?"
    },
    "websiteNotWorkingPrompt": {
      "title": "Fungerer ikke nettstedet?"
    },
    "websiteNotWorkingCta": {
      "title": "Rapporter nettstedfeil"
    },
    "websiteNotWorkingAdvice": {
      "title": "Det kan hjelpe å slå beskyttelsen AV."
    },
    "websiteNotWorkingPromptFollowUp": {
      "title": "Fungerer nettstedet fremdeles ikke?"
    },
    "takePrecautions": {
      "title": "Ta forholdsregler"
    },
    "majorTrackingNetworkDesc": {
      "title": "{blocked, select, true {<b>Dette nettstedet eies av {companyDisplayName}</b>, som driver et sporingsnettverk på {companyPrevalence} % av de mest populære nettstedene. Vi klarte å blokkere noen av forespørslene deres på denne siden.} other {<b>Dette nettstedet eies av {companyDisplayName}</b>, som driver et sporingsnettverk på {companyPrevalence} % av de mest populære nettstedene. }}"
    },
    "trackersBlockedDesc": {
      "title": "{companyCount, plural, =0 {Vi blokkerte noen selskaper fra å laste inn sporende forespørsler på denne siden.} =2 {Vi har blokkert <b>{firstCompany}</b> og <b>{secondCompany}</b> fra å laste sporende forespørsler på denne siden.} =3 {Vi har blokkert <b>{firstCompany}</b>, <b>{secondCompany}</b> og <b>{thirdCompany}</b> fra å laste sporende forespørsler på denne siden.} =4 {Vi har blokkert <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b> og <b>{fourthCompany}</b> fra å laste sporende forespørsler på denne siden.} =5 {Vi har blokkert <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b>, <b>{fourthCompany}</b> og <b>1 til</b> fra å laste sporende forespørsler på denne siden.} one {Vi har blokkert <b>{firstCompany}</b> fra å laste sporende forespørsler på denne siden.} other {Vi har blokkert <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b>, <b>{fourthCompany}</b> og <b>{othersCount} andre</b> fra å laste sporende forespørsler på denne siden.}}"
    },
    "cookiesMinimized": {
      "title": "Informasjonskapsler administrert"
    },
    "cookiesHidden": {
      "title": "Popup-vindu om informasjonskapsler skjult"
    },
    "cookiesHiddenSummary": {
      "title": "Popup-vinduet om informasjonskapsler på dette nettstedet ga ingen alternativer for preferanser, så vi har bare skjult det. Våre andre nettsporingsbeskyttelser gjelder fortsatt."
    },
    "cookiesMinimizedSummary": {
      "title": "Vi har angitt informasjonskapselpreferansene dine for å maksimere personvernet, og lukket popupen."
    },
    "cookiesMinimizedSettings": {
      "title": "Deaktiver i innstillingene"
    },
    "connectionSecure": {
      "title": "Tilkoblingen er kryptert"
    },
    "connectionNotSecure": {
      "title": "Tilkoblingen er ikke kryptert"
    },
    "connectionNotSecureInvalidCertificate": {
      "title": "Tilkoblingen kan være usikker"
    },
    "trackerNetworksDesc": {
      "title": "Forespørsler blokkert fra lasting"
    },
    "trackerNetworksNotBlocked": {
      "title": "Ingen sporende forespørsler blokkert"
    },
    "trackerNetworksNotFound": {
      "title": "Ingen sporende forespørsler funnet"
    },
    "thirdPartiesLoaded": {
      "title": "Tredjepartsforespørsler lastet"
    },
    "thirdPartiesNoneFound": {
      "title": "Ingen tredjepartsforespørsler er funnet"
    },
    "firstPartyDesc": {
      "title": "{companyName} eier dette nettstedet og de kjente sporerne som finnes på denne siden, så vi blokkerte dem ikke."
    },
    "noTrackersFound": {
      "title": "Vi fant ingen firmaer som prøvde å laste sporende forespørsler på denne siden."
    },
    "trackersFoundForAllowlisted": {
      "title": "Sporere hjelper firmaer å profilere deg. Vi oppdaget at disse selskapene overvåker aktiviteten din på denne siden."
    },
    "trackersFoundAndBlocked": {
      "title": "Sporere hjelper firmaer å profilere deg. Vi blokkerte disse selskapene fra å overvåke aktiviteten din på denne siden."
    },
    "trackerNetworkUnknown": {
      "title": "Sporingsnettverket ukjent"
    },
    "trackerDomainsForCompany": {
      "title": "Sporingsdomener for {companyName}"
    },
    "zeroTrackersFound": {
      "title": "0 sporere funnet"
    },
    "trackerOwnedByThisSite": {
      "title": "Sporere som eies av dette nettstedet (tillatt)"
    },
    "trackerCountForDomain": {
      "title": "{blocked, select, true {{trackerCount, plural, one {{trackerCount} sporer er blokkert på {domain}} other {{trackerCount} sporere er blokkert på {domain}}}} other {{trackerCount, plural, one {{trackerCount} sporer funnet på {domain}} other {{trackerCount} sporere funnet på {domain}}}}}"
    },
    "trackerLimitationsNote": {
      "title": "Merk: Plattformbegrensninger kan begrense kapasiteten vår til å oppdage alle forespørsler."
    },
    "trackerAboutLink": {
      "title": "Om nettsporingsbeskyttelsen vår"
    },
    "trackerAdLink": {
      "title": "Hvordan søkeannonsene våre påvirker beskyttelsen vår"
    },
    "sectionHeadingAdAttribution": {
      "title": "Forespørslene fra følgende domene ble lastet fordi en {domain}-annonse på DuckDuckGo nylig ble klikket på. Disse forespørslene bidrar til å evaluere annonseeffektiviteten. Ingen annonser på DuckDuckGo profilerer deg."
    },
    "sectionHeadingIgnore": {
      "title": "Forespørslene fra følgende domener ble lastet for å forhindre at nettstedene skulle slutte å fungere."
    },
    "sectionHeadingFirstParty": {
      "title": "Forespørslene fra følgende domener ble lastet fordi de er tilknyttet {domain}."
    },
    "sectionHeadingThirdParty": {
      "title": "Forespørslene fra følgende domener ble også lastet."
    },
    "sectionHeadingProtectionsDisabled": {
      "title": "Forespørslene fra følgende domener ble lastet fordi beskyttelsen er slått av."
    },
    "thirdPartiesSummaryLoaded": {
      "title": "Forespørslene fra følgende tredjepartsdomener ble lastet. Hvis forespørslene fra et selskap blir lastet, har selskapet mulighet til å profilere deg, selv om de andre nettsporingsbeskyttelsene våre fremdeles gjelder."
    },
    "thirdPartiesSummaryProtectionsOff": {
      "title": "Forespørslene fra følgende tredjepartsdomener ble lastet. Hvis forespørslene fra et selskap blir lastet, har selskapet mulighet til å profilere deg, selv om de andre nettsporingsbeskyttelsene våre fremdeles gjelder."
    },
    "thirdPartiesSummaryNone": {
      "title": "Vi identifiserte ingen forespørsler fra tredjepartsdomener."
    },
    "analyticsCategory": {
      "title": "Analyse"
    },
    "advertisingCategory": {
      "title": "Annonsering"
    },
    "socialCategory": {
      "title": "Sosialt nettverk"
    },
    "contentDeliveryCategory": {
      "title": "Levering av innhold"
    },
    "embeddedContentCategory": {
      "title": "Innebygd innhold"
    },
    "searchPlaceholder": {
      "title": "Søk i DuckDuckGo"
    },
    "searchGoButton": {
      "title": "Søk"
    },
    "optionsButton": {
      "title": "Flere alternativer"
    },
    "navigationComplete": {
      "title": "Ferdig"
    },
    "navigationClose": {
      "title": "Lukk"
    },
    "navigationCancel": {
      "title": "Avbryt"
    },
    "navigationBack": {
      "title": "Tilbake"
    },
    "enableProtectionsSwitch": {
      "title": "Aktiver beskyttelse"
    },
    "disableProtectionsSwitch": {
      "title": "Deaktiver beskyttelse"
    },
    "errorMessage": {
      "title": "Noe gikk galt, så vi kunne ikke laste dette innholdet. Prøv å laste siden på nytt."
    }
  }
}