export default {
  "connection": {
    "encrypt": {
      "title": "Kryptera"
    },
    "sign": {
      "title": "Underteckna"
    },
    "verify": {
      "title": "Verifiera"
    },
    "derive": {
      "title": "Härled"
    },
    "wrap": {
      "title": "Packa in"
    },
    "unwrap": {
      "title": "Packa upp"
    },
    "unknown": {
      "title": "Okänd"
    },
    "certificateDetail": {
      "title": "Information om säkerhetscertifikat"
    },
    "commonName": {
      "title": "Vanligt namn"
    },
    "publicKey": {
      "title": "Offentlig nyckel"
    },
    "algorithm": {
      "title": "Algoritm"
    },
    "keySize": {
      "title": "Nyckelstorlek"
    },
    "usage": {
      "title": "Användning"
    },
    "summary": {
      "title": "Sammanfattning"
    },
    "permanent": {
      "title": "Permanent"
    },
    "effectiveSize": {
      "title": "Verklig storlek"
    },
    "certificateNotFound": {
      "title": "Certifikatet hittades inte"
    },
    "certificateForDomain": {
      "title": "Certifikat för {domain}"
    },
    "insecureConnectionDesc": {
      "title": "Den här sidan använder en okrypterad anslutning. Tredje part kan ha möjlighet att se din aktivitet eller avlyssna känslig information som du skickar på den här sidan."
    },
    "invalidConnectionDesc": {
      "title": "Certifikatet för denna webbplats är ogiltigt. Du kanske ansluter till en server som låtsas vara <b>{domain}</b>. Detta kan utgöra en risk för din konfidentiella information."
    },
    "upgradedConnectionDesc": {
      "title": "Vi har uppgraderat anslutningen på den här sidan för att skydda den information som du skickar medan den överförs."
    },
    "secureConnectionDesc": {
      "title": "Den här sidan använder en krypterad anslutning, vilket förhindrar att tredje part kan se din aktivitet eller avlyssna känslig information som du skickar på den här sidan."
    }
  },
  "ctascreens": {
    "protectionsUnavailableNote": {
      "title": "Integritetsskydd är inte tillgängligt för särskilda sidor eller lokala sidor."
    },
    "spreadTitle": {
      "title": "Älskar du att använda DuckDuckGo?"
    },
    "spreadText": {
      "title": "Hjälp oss att sprida budskapet till din familj och dina vänner"
    },
    "spreadButton": {
      "title": "Sprid DuckDuckGo"
    },
    "emailTitle": {
      "title": "Trött på att e-postmeddelanden spåras?"
    },
    "emailText": {
      "title": "Registrera dig för e-postskydd från DuckDuckGo för ditt tillägg nu!"
    }
  },
  "firebutton": {
    "fireDialogHeader": {
      "title": "Stäng flikar och rensa data"
    },
    "fireDialogHeaderNoTabs": {
      "title": "Rensa data"
    },
    "optionCurrentSite": {
      "title": "Endast nuvarande webbplats"
    },
    "optionLastHour": {
      "title": "Senaste timmen"
    },
    "optionLast24Hour": {
      "title": "Senaste 24 timmarna"
    },
    "optionLast7days": {
      "title": "Senaste 7 dagarna"
    },
    "optionLast4Weeks": {
      "title": "Senaste 4 veckorna"
    },
    "optionAllTime": {
      "title": "För all tid"
    },
    "historyDuration": {
      "title": "{duration, select, hour {en timmes} day {24 timmars} week {en veckas} month {4 veckors} other {Alla}}"
    },
    "summaryClearTabsHistoryDuration": {
      "title": "{openTabs, plural, one {Vill du stänga <b>{openTabs}</b> flik och rensa <b>{durationDesc}</b> surfhistorik och cookies?} other {Vill du stänga <b>{openTabs}</b> flikar och rensa <b>{durationDesc}</b> surfhistorik och cookies?}}"
    },
    "summaryClearTabsDuration": {
      "title": "{openTabs, plural, one {Vill du stänga <b>{openTabs}</b> flik och rensa <b>{durationDesc}</b> cookies?} other {Vill du stänga <b>{openTabs}</b> flikar och rensa <b>{durationDesc}</b> cookies?}}"
    },
    "summaryClearHistoryDuration": {
      "title": "Vill du rensa <b>{durationDesc}</b> surfhistorik och cookies?"
    },
    "summaryClearCookiesDuration": {
      "title": "Vill du rensa <b>{durationDesc}</b> cookies?"
    },
    "summaryClearTabsHistoryAll": {
      "title": "Vill du stänga <b>{openTabs}</b> {openTabs, plural, =1 {flik} other {flikar}}, rensa <b>all</b> surfhistorik och alla ({cookies} {cookies, plural, =1 {webbplats} other {webbplatser}})?"
    },
    "summaryClearTabsAll": {
      "title": "Vill du stänga <b>{openTabs}</b> {openTabs, plural, =1 {flik} other {flikar}} och rensa <b>alla</b> cookies ({cookies} {cookies, plural, =1 {webbplats} other {webbplatser}})?"
    },
    "summaryClearHistoryAll": {
      "title": "{cookies, plural, one {Vill du rensa <b>all</b> surfhistorik och alla cookies ({cookies} webbplats)?} other {Vill du rensa <b>all</b> surfhistorik och alla cookies ({cookies} webbplatser)?}}"
    },
    "summaryClearCookiesAll": {
      "title": "{cookies, plural, one {Vill du rensa <b>alla</b> cookies ({cookies} webbplats)?} other {Vill du rensa <b>alla</b> cookies ({cookies} webbplatser)?}}"
    },
    "summaryClearTabsHistorySite": {
      "title": "{openTabs, plural, one {Vill du stänga <b>{openTabs} flik för {site}</b> och rensa <b>alla cookies för {site}</b>?} other {Vill du stänga <b>{openTabs} flikar för {site}</b> och rensa <b>alla cookies för {site}</b>?}}"
    },
    "summaryClearTabsSite": {
      "title": "{openTabs, plural, one {Vill du stänga <b>{openTabs} flik för {site}</b> och rensa <b>alla cookies för {site}</b>?} other {Vill du stänga <b>{openTabs} flikar för {site}</b> och rensa <b>alla cookies för {site}</b>?}}"
    },
    "summaryClearHistorySite": {
      "title": "Vill du rensa <b>all surfhistorik och alla cookies för {site}</b>?"
    },
    "summaryClearCookiesSite": {
      "title": "Vill du rensa <b>alla cookies för {site}</b>?"
    },
    "summaryPinnedIgnored": {
      "title": "{tabs, plural, one {<b>{tabs} fäst</b> flik kommer att ignoreras.} other {<b>{tabs} fästa</b> flikar kommer att ignoreras.}}"
    },
    "clearData": {
      "title": "Klart"
    },
    "cancel": {
      "title": "Avbryt"
    },
    "historyAndDownloadsNotAffected": {
      "title": "Om du också vill rensa historiken väljer du en tidsperiod."
    }
  },
  "permissions": {
    "camera": {
      "title": "Kamera"
    },
    "microphone": {
      "title": "Mikrofon"
    },
    "geolocation": {
      "title": "Geolokalisering"
    },
    "popups": {
      "title": "Popup-fönster"
    },
    "ask": {
      "title": "Fråga varje gång"
    },
    "notify": {
      "title": "Meddela"
    },
    "grant": {
      "title": "Tillåt alltid"
    },
    "deny": {
      "title": "Neka alltid"
    }
  },
  "report": {
    "selectTheOptionDesc": {
      "title": "Genom att skicka in en anonym anmälan om att webbplatsen inte fungerar hjälper du oss att förbättra appen."
    },
    "selectTheOptionDescV2": {
      "title": "Välj det alternativ som bäst beskriver det problem som du har stött på."
    },
    "selectTheCategoryType": {
      "title": "Vad är problemet?"
    },
    "categoryType1": {
      "title": "Webbplatsen fungerar inte som förväntat"
    },
    "categoryType2": {
      "title": "Jag ogillar innehållet på den här webbplatsen"
    },
    "categoryType3": {
      "title": "Allmän feedback om DuckDuckGo-webbläsaren"
    },
    "selectTheCategory": {
      "title": "Vad är det som inte fungerar på den här webbplatsen?"
    },
    "tryTurningProtectionsOff": {
      "title": "Prova med att stänga av integritetsskyddet för att se om det löser problemet."
    },
    "skipThisStep": {
      "title": "Hoppa över det här steget"
    },
    "pickYourIssueFromTheList": {
      "title": "Berätta vad som hände:"
    },
    "blocked": {
      "title": "Webbplatsen är blockerad eller lästes inte in"
    },
    "layout": {
      "title": "Webbplatsens layout är trasig"
    },
    "emptySpaces": {
      "title": "Webbplatsen innehåller stora tomma fält"
    },
    "videos": {
      "title": "Videon spelades inte upp eller laddades inte"
    },
    "images": {
      "title": "Bilderna kunde inte laddas"
    },
    "comments": {
      "title": "Kommentarer, recensioner eller chattar laddades inte"
    },
    "content": {
      "title": "Innehåll saknas"
    },
    "links": {
      "title": "Länkar eller knappar fungerar inte"
    },
    "login": {
      "title": "Det går inte att logga in/registrera sig"
    },
    "loginV2": {
      "title": "Det går inte att logga in eller registrera sig"
    },
    "shopping": {
      "title": "Det går inte att betala, gå till kassan eller shoppa"
    },
    "browser": {
      "title": "Problem med webbläsare eller annat tilläggsprogram"
    },
    "paywall": {
      "title": "Webbplatsen bad mig att inaktivera annonsblockeraren"
    },
    "other": {
      "title": "Övrigt"
    },
    "tellUsMoreDesc": {
      "title": "Berätta mer (valfritt):{bullet}Vad hände?{bullet}Vad borde ha hänt?{bullet}Hjälpte det att stänga av skydd?"
    },
    "sendReport": {
      "title": "Skicka anmälan"
    },
    "reportsAreAnonymousDesc": {
      "title": "Rapporter som skickas till DuckDuckGo innehåller endast information som krävs för att vi ska kunna bemöta din feedback."
    },
    "thankYou": {
      "title": "Tack!"
    },
    "yourReportWillHelpDesc": {
      "title": "Din rapport bidrar till att förbättra våra produkter och göra upplevelsen bättre för andra."
    },
    "dislike": {
      "title": "Jag ogillar innehållet"
    },
    "otherRequired": {
      "title": "Beskriv det problem som du stötte på (obligatoriskt)"
    },
    "otherOptional": {
      "title": "Beskriv det problem som du stötte på (valfritt)"
    },
    "suggestionWhatHappened": {
      "title": "Vad hände?"
    },
    "suggestionWhatHappened2": {
      "title": "Vad borde ha hänt?"
    },
    "suggestionWhatHappened3": {
      "title": "Hjälpte det att stänga av integritetsskyddet?"
    },
    "reportTitle": {
      "title": "Rapportera till DuckDuckGo"
    }
  },
  "shared": {
    "trackers": {
      "title": "Spårare"
    }
  },
  "site": {
    "updatingProtectionList": {
      "title": "Skyddslistan uppdateras"
    },
    "protectionsEnabled": {
      "title": "Skydd är <b>PÅ</b> för den här webbplatsen"
    },
    "protectionsDisabled": {
      "title": "Skydd är <b>AV</b> för den här webbplatsen"
    },
    "protectionsDisabledRemote": {
      "title": "Vi har tillfälligt stängt av integritetsskydd eftersom det verkade göra att denna webbplats inte fungerade."
    },
    "protectionsDisabledRemoteOverride": {
      "title": "Vi rekommenderar att du inaktiverar integritetsskydd för den här webbplatsen för att förhindra fel på webbplatsen."
    },
    "connectionDescriptionUnencrypted": {
      "title": "<b>Den här webbplatsen är inte säker</b> och kan äventyra eventuell information som du skickar på sidan."
    },
    "connectionDescriptionInvalidCertificate": {
      "title": "<b>Certifikatet för denna webbplats är ogiltigt</b>. Du kanske ansluter till en server som låtsas vara <b>{domain}</b>. Detta kan utgöra en risk för din konfidentiella information."
    },
    "trackerNetworksSummaryBlocked": {
      "title": "Följande tredjepartsdomäners förfrågningar blockerades från att läsas in eftersom de identifierades som spårningsförfrågningar. Om ett företags förfrågningar läses in kan det göra det möjligt för företaget att profilera dig."
    },
    "trackerNetworksSummaryNoneBlocked": {
      "title": "Inga spårningsförfrågningar blockerades från att läsas in på den här sidan. Om ett företags förfrågningar läses in kan det göra det möjligt för företaget att profilera dig."
    },
    "trackerNetworksSummaryNoneFound": {
      "title": "Vi har inte identifierat några spårningsförfrågningar på den här sidan."
    },
    "trackerNetworksSummaryNone": {
      "title": "Vi har inte hittat några företag som försöker läsa in spårningsförfrågningar på den här sidan."
    },
    "trackerNetworksSummaryAllowedOnly": {
      "title": "För att förhindra fel på webbplatsen har vi inte blockerat några företag från att läsa in spårningsförfrågningar på den här sidan."
    },
    "trackerNetworksSummaryProtectionsOff": {
      "title": "Inga spårningsförfrågningar blockerades från att läsas in eftersom skyddet är avstängt för den här webbplatsen. Om ett företags förfrågningar läses in kan det göra det möjligt för företaget att profilera dig."
    },
    "createNewDuckAddress": {
      "title": "Generera privat Duck Address"
    },
    "createNewDuckAddressCopied": {
      "title": "Kopierad till Urklipp!"
    },
    "websiteNotWorkingQ": {
      "title": "Fungerar inte webbplatsen som förväntat?"
    },
    "websiteNotWorkingPrompt": {
      "title": "Fungerar inte webbplatsen?"
    },
    "websiteNotWorkingCta": {
      "title": "Rapportera skadad webbplats"
    },
    "websiteNotWorkingAdvice": {
      "title": "Det kan hjälpa att stänga av skyddet."
    },
    "websiteNotWorkingPromptFollowUp": {
      "title": "Fungerar det fortfarande inte?"
    },
    "takePrecautions": {
      "title": "Vidta försiktighetsåtgärder"
    },
    "majorTrackingNetworkDesc": {
      "title": "{blocked, select, true {<b>Den här webbplatsen ägs av {companyDisplayName}</b>, som driver ett spårningsnätverk på {companyPrevalence} % av de populäraste webbplatserna. Vi har lyckats blockera några av deras förfrågningar på den här sidan.} other {<b>Den här webbplatsen ägs av {companyDisplayName}</b>, som driver ett spårningsnätverk på {companyPrevalence} % av de populäraste webbplatserna. }}"
    },
    "trackersBlockedDesc": {
      "title": "{companyCount, plural, =0 {Vi har blockerat vissa företag från att läsa in spårningsförfrågningar på den här sidan.} =2 {Vi har blockerat <b>{firstCompany}</b> och <b>{secondCompany}</b> från att läsa in spårningsförfrågningar på den här sidan.} =3 {Vi har blockerat <b>{firstCompany}</b>, <b>{secondCompany}</b> och <b>{thirdCompany}</b> från att läsa in spårningsförfrågningar på den här sidan.} =4 {Vi har blockerat <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b> och <b>{fourthCompany}</b> från att läsa in spårningsförfrågningar på den här sidan.} =5 {Vi har blockerat <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b>, <b>{fourthCompany}</b> och <b>1 annat företag </b> från att läsa in spårningsförfrågningar på den här sidan.} one {Vi har blockerat <b>{firstCompany}</b> från att läsa in spårningsförfrågningar på den här sidan.} other {Vi har blockerat <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b>, <b>{fourthCompany}</b> och <b>{othersCount} andra företag</b> från att läsa in spårningsförfrågningar på den här sidan.}}"
    },
    "cookiesMinimized": {
      "title": "Cookies hanteras"
    },
    "cookiesHidden": {
      "title": "Popup-fönster för cookies dolt"
    },
    "cookiesHiddenSummary": {
      "title": "Vi kunde bara dölja popup-fönstret för cookies på den här webbplatsen eftersom det inte erbjöds några alternativ för att hantera inställningar för cookies. Våra övriga webbspårningsskydd gäller fortfarande."
    },
    "cookiesMinimizedSummary": {
      "title": "Vi har konfigurerat dina cookieinställningar för att maximera din integritet och stängt popup-fönstret."
    },
    "cookiesMinimizedSettings": {
      "title": "Inaktivera i inställningar"
    },
    "connectionSecure": {
      "title": "Anslutningen är krypterad"
    },
    "connectionNotSecure": {
      "title": "Anslutningen är inte krypterad"
    },
    "connectionNotSecureInvalidCertificate": {
      "title": "Anslutningen kan vara osäker"
    },
    "trackerNetworksDesc": {
      "title": "Förfrågningar blockerades från att läsas in"
    },
    "trackerNetworksNotBlocked": {
      "title": "Inga spårningsförfrågningar blockerades"
    },
    "trackerNetworksNotFound": {
      "title": "Inga spårningsförfrågningar hittades"
    },
    "thirdPartiesLoaded": {
      "title": "Förfrågningar från tredje part lästes in"
    },
    "thirdPartiesNoneFound": {
      "title": "Inga förfrågningar från tredje part hittades"
    },
    "firstPartyDesc": {
      "title": "{companyName} äger den här webbplatsen och de kända spårare som hittades på sidan, så vi har inte blockerat dem."
    },
    "noTrackersFound": {
      "title": "Vi har inte hittat några företag som försöker läsa in spårningsförfrågningar på den här sidan."
    },
    "trackersFoundForAllowlisted": {
      "title": "Spårare hjälper företag att profilera dig. Vi har upptäckt att följande företag övervakar din aktivitet på den här sidan."
    },
    "trackersFoundAndBlocked": {
      "title": "Spårare hjälper företag att profilera dig. Vi har blockerat följande företag från att övervaka din aktivitet på den här sidan."
    },
    "trackerNetworkUnknown": {
      "title": "Okänt spårarnätverk"
    },
    "trackerDomainsForCompany": {
      "title": "Spårardomäner för {companyName}"
    },
    "zeroTrackersFound": {
      "title": "0 spårare hittades"
    },
    "trackerOwnedByThisSite": {
      "title": "Spårare som ägs av den här webbplatsen (tillåtna)"
    },
    "trackerCountForDomain": {
      "title": "{blocked, select, true {{trackerCount, plural, one {{trackerCount} spårare blockerad på {domain}} other {{trackerCount} spårare blockerade på {domain}}}} other {{trackerCount, plural, one {{trackerCount} spårare hittades på {domain}} other {{trackerCount} spårare hittades på {domain}}}}}"
    },
    "trackerLimitationsNote": {
      "title": "Obs! Plattformsbegränsningar kan begränsa vår förmåga att upptäcka alla förfrågningar."
    },
    "trackerAboutLink": {
      "title": "Om våra webbspårningsskydd"
    },
    "trackerAdLink": {
      "title": "Hur våra sökannonser påverkar vårt skydd"
    },
    "sectionHeadingAdAttribution": {
      "title": "Följande domäns förfrågningar lästes in eftersom du nyligen klickade på en annons från {domain} på DuckDuckGo. Dessa förfrågningar hjälper till att utvärdera annonsens effektivitet. Alla annonser på DuckDuckGo är icke-profilerande."
    },
    "sectionHeadingIgnore": {
      "title": "Följande domäners förfrågningar lästes in för att förhindra fel på webbplatsen."
    },
    "sectionHeadingFirstParty": {
      "title": "Följande domäners förfrågningar lästes in eftersom de är kopplade till {domain}."
    },
    "sectionHeadingThirdParty": {
      "title": "Följande domäners förfrågningar lästes också in."
    },
    "sectionHeadingProtectionsDisabled": {
      "title": "Följande domäners förfrågningar lästes in eftersom skydd är avstängda."
    },
    "thirdPartiesSummaryLoaded": {
      "title": "Följande tredjepartsdomäners förfrågningar lästes in. Om ett företags förfrågningar läses in kan det göra det möjligt för företaget att profilera dig, även om våra övriga webbspårningsskydd fortfarande är aktiva."
    },
    "thirdPartiesSummaryProtectionsOff": {
      "title": "Följande tredjepartsdomäners förfrågningar lästes in. Om ett företags förfrågningar läses in kan det göra det möjligt för företaget att profilera dig, även om våra övriga webbspårningsskydd fortfarande är aktiva."
    },
    "thirdPartiesSummaryNone": {
      "title": "Vi har inte identifierat några förfrågningar från tredjepartsdomäner."
    },
    "analyticsCategory": {
      "title": "Analys"
    },
    "advertisingCategory": {
      "title": "Annonsering"
    },
    "socialCategory": {
      "title": "Socialt nätverk"
    },
    "contentDeliveryCategory": {
      "title": "Innehållsleverans"
    },
    "embeddedContentCategory": {
      "title": "Inbäddat innehåll"
    },
    "searchPlaceholder": {
      "title": "Sök med DuckDuckGo"
    },
    "searchGoButton": {
      "title": "Sök"
    },
    "optionsButton": {
      "title": "Fler alternativ"
    },
    "navigationComplete": {
      "title": "Klart"
    },
    "navigationClose": {
      "title": "Stäng"
    },
    "navigationCancel": {
      "title": "Avbryt"
    },
    "navigationBack": {
      "title": "Tillbaka"
    },
    "enableProtectionsSwitch": {
      "title": "Aktivera skydd"
    },
    "disableProtectionsSwitch": {
      "title": "Inaktivera skydd"
    },
    "errorMessage": {
      "title": "Något gick fel och vi kunde inte läsa in detta innehåll. Försök läsa in sidan igen."
    }
  }
}