export default {
  "connection": {
    "encrypt": {
      "title": "Šifriraj"
    },
    "sign": {
      "title": "Znak"
    },
    "verify": {
      "title": "Preveri"
    },
    "derive": {
      "title": "Izpelji"
    },
    "wrap": {
      "title": "Zavij"
    },
    "unwrap": {
      "title": "Razvij"
    },
    "unknown": {
      "title": "Neznano"
    },
    "certificateDetail": {
      "title": "Podrobnosti varnostnega potrdila"
    },
    "commonName": {
      "title": "Splošno ime"
    },
    "publicKey": {
      "title": "Javni ključ"
    },
    "algorithm": {
      "title": "Algoritem"
    },
    "keySize": {
      "title": "Velikost ključa"
    },
    "usage": {
      "title": "Uporaba"
    },
    "summary": {
      "title": "Povzetek"
    },
    "permanent": {
      "title": "Trajno"
    },
    "effectiveSize": {
      "title": "Učinkovita velikost"
    },
    "certificateNotFound": {
      "title": "Certifikata ni bilo mogoče najti"
    },
    "certificateForDomain": {
      "title": "Potrdilo za spletno mesto {domain}"
    },
    "insecureConnectionDesc": {
      "title": "Ta stran uporablja nešifrirano povezavo. Tretje osebe si lahko ogledajo vaše dejavnosti ali prestrežejo občutljive podatke, ki jih pošljete na tej strani."
    },
    "invalidConnectionDesc": {
      "title": "Certifikat za to stran ni veljaven. Morda se povezujete s strežnikom, ki se pretvarja, da je <b>{domain}</b>, kar bi lahko ogrozilo vaše zaupne podatke."
    },
    "upgradedConnectionDesc": {
      "title": "Na tej strani smo nadgradili povezavo za zaščito podatkov, ki jih pošiljate med prenosom."
    },
    "secureConnectionDesc": {
      "title": "Ta stran uporablja šifrirano povezavo, ki tretjim osebam preprečuje ogled vaše dejavnosti ali prestrezanje občutljivih podatkov, ki jih pošljete na tej strani."
    }
  },
  "ctascreens": {
    "protectionsUnavailableNote": {
      "title": "Zaščita zasebnosti ni na voljo za posebne ali lokalne strani."
    },
    "spreadTitle": {
      "title": "Radi uporabljate DuckDuckGo?"
    },
    "spreadText": {
      "title": "Pomagajte nam razširiti besedo med vašo družino in prijatelje"
    },
    "spreadButton": {
      "title": "Pomagaj širiti besedo o DuckDuckGo"
    },
    "emailTitle": {
      "title": "Ste naveličani sledenja e-poštnim sporočilom?"
    },
    "emailText": {
      "title": "Prijavite se na zaščito e-pošte DuckDuckGo za svojo razširitev zdaj!"
    }
  },
  "firebutton": {
    "fireDialogHeader": {
      "title": "Zapri zavihke in izbriši podatke"
    },
    "fireDialogHeaderNoTabs": {
      "title": "Počisti podatke"
    },
    "optionCurrentSite": {
      "title": "Samo trenutna stran"
    },
    "optionLastHour": {
      "title": "Zadnja ura"
    },
    "optionLast24Hour": {
      "title": "Zadnjih 24 ur"
    },
    "optionLast7days": {
      "title": "Zadnjih 7 dni"
    },
    "optionLast4Weeks": {
      "title": "Zadnje 4 tedne"
    },
    "optionAllTime": {
      "title": "Ves čas"
    },
    "historyDuration": {
      "title": "{duration, select, hour {ene ure} day {24 ur} week {enega tedna} month {4 tednov} other {Vse}}"
    },
    "summaryClearTabsHistoryDuration": {
      "title": "{openTabs, plural, one {Želite zapreti <b>{openTabs}</b> zavihek ter počistiti zgodovino brskanja in piškotke za obdobje <b>{durationDesc}</b>?} two {Želite zapreti <b>{openTabs}</b> zavihka ter počistiti zgodovino brskanja in piškotke za obdobje <b>{durationDesc}</b>?} few {Želite zapreti <b>{openTabs}</b> zavihke ter počistiti zgodovino brskanja in piškotke za obdobje <b>{durationDesc}</b>?} other {Želite zapreti <b>{openTabs}</b> zavihkov ter počistiti zgodovino brskanja in piškotke za obdobje <b>{durationDesc}</b>?}}"
    },
    "summaryClearTabsDuration": {
      "title": "{openTabs, plural, one {Želite zapreti <b>{openTabs}</b> zavihek in počistiti piškotke za obdobje <b>{durationDesc}</b>?} two {Želite zapreti <b>{openTabs}</b> zavihka in počistiti piškotke za obdobje <b>{durationDesc}</b>?} few {Želite zapreti <b>{openTabs}</b> zavihke in počistiti piškotke za obdobje <b>{durationDesc}</b>?} other {Želite zapreti <b>{openTabs}</b> zavihkov in počistiti piškotke za obdobje <b>{durationDesc}</b>?}}"
    },
    "summaryClearHistoryDuration": {
      "title": "Želite počistiti zgodovino brskanja in piškotke za obdobje <b>{durationDesc}</b>?"
    },
    "summaryClearCookiesDuration": {
      "title": "Želite počistiti piškotke za obdobje <b>{durationDesc}</b>?"
    },
    "summaryClearTabsHistoryAll": {
      "title": "Želite zapreti <b>{openTabs}</b> {openTabs, plural, =1 {zavihek} other {zavihka/-e/-ov}} ter počistiti <b>vso</b> zgodovino brskanja in piškotke ({cookies} {cookies, plural, =1 {spletnega mesta} other {spletnih mest}})?"
    },
    "summaryClearTabsAll": {
      "title": "Želite zapreti <b>{openTabs}</b> {openTabs, plural, =1 {zavihek} other {zavihka/-e/-ov}} in počistiti <b>vse</b> piškotke ({cookies} {cookies, plural, =1 {spletnega mesta} other {spletnih mest}})?"
    },
    "summaryClearHistoryAll": {
      "title": "{cookies, plural, one {Želite izbrisati <b>vso</b> zgodovino brskanja in piškotke (za {cookies} spletno mesto)?} two {Želite izbrisati <b>vso</b> zgodovino brskanja in piškotke (za {cookies} spletni mesti)?} few {Želite izbrisati <b>vso</b> zgodovino brskanja in piškotke (za {cookies} spletna mesta)?} other {Želite izbrisati <b>vso</b> zgodovino brskanja in piškotke (za {cookies} spletnih mest)?}}"
    },
    "summaryClearCookiesAll": {
      "title": "{cookies, plural, one {Želite počistiti <b>vse</b> piškotke (za {cookies} spletno mesto)?} two {Želite počistiti <b>vse</b> piškotke (za {cookies} spletni mesti)?} few {Želite počistiti <b>vse</b> piškotke (za {cookies} spletna mesta)?} other {Želite počistiti <b>vse</b> piškotke (za {cookies} spletnih mest)?}}"
    },
    "summaryClearTabsHistorySite": {
      "title": "{openTabs, plural, one {Želite zapreti <b>{openTabs} zavihek spletnega mesta {site}</b> in izbrisati <b>vse piškotke spletnega mesta {site}</b>?} two {Želite zapreti <b>{openTabs} zavihka spletnega mesta {site}</b> in izbrisati <b>vse piškotke spletnega mesta {site}</b>?} few {Želite zapreti <b>{openTabs} zavihke spletnega mesta {site}</b> in izbrisati <b>vse piškotke spletnega mesta {site}</b>?} other {Želite zapreti <b>{openTabs} zavihkov spletnega mesta {site}</b> in izbrisati <b>vse piškotke spletnega mesta {site}</b>?}}"
    },
    "summaryClearTabsSite": {
      "title": "{openTabs, plural, one {Želite zapreti <b>{openTabs} zavihek spletnega mesta {site}</b> in izbrisati <b>vse piškotke spletnega mesta {site}</b>?} two {Želite zapreti <b>{openTabs} zavihka spletnega mesta {site}</b> in izbrisati <b>vse piškotke spletnega mesta {site}</b>?} few {Želite zapreti <b>{openTabs} zavihke spletnega mesta {site}</b> in izbrisati <b>vse piškotke spletnega mesta {site}</b>?} other {Želite zapreti <b>{openTabs} zavihkov spletnega mesta {site}</b> in izbrisati <b>vse piškotke spletnega mesta {site}</b>?}}"
    },
    "summaryClearHistorySite": {
      "title": "Želite izbrisati <b>vso zgodovino brskanja in piškotke za spletno mesto {site}</b>?"
    },
    "summaryClearCookiesSite": {
      "title": "Želite izbrisati <b>vse piškotke za spletno mesto {site}</b>)?"
    },
    "summaryPinnedIgnored": {
      "title": "{tabs, plural, one {<b>{tabs} pripet</b> zavihek bo prezrt.} two {<b>{tabs} pripeta</b> zavihka bosta prezrta.} few {<b>{tabs} pripeti</b> zavihki bodo prezrti.} other {<b>{tabs} pripetih</b> zavihkov bo prezrtih.}}"
    },
    "clearData": {
      "title": "Počisti"
    },
    "cancel": {
      "title": "Preklic"
    },
    "historyAndDownloadsNotAffected": {
      "title": "Če želite počistiti tudi zgodovino, izberite časovno obdobje."
    }
  },
  "permissions": {
    "camera": {
      "title": "Kamera"
    },
    "microphone": {
      "title": "Mikrofon"
    },
    "geolocation": {
      "title": "Geolokacija"
    },
    "popups": {
      "title": "Pojavna okna"
    },
    "ask": {
      "title": "Vprašaj vsakič"
    },
    "notify": {
      "title": "Obvesti"
    },
    "grant": {
      "title": "Vedno dovolite"
    },
    "deny": {
      "title": "Vedno zavrnite"
    }
  },
  "report": {
    "selectTheOptionDesc": {
      "title": "Oddaja anonimnega poročila o nedelujočem mestu nam pomaga izboljšati aplikacijo."
    },
    "selectTheOptionDescV2": {
      "title": "Izberite možnost, ki najbolje opisuje težavo, na katero ste naleteli."
    },
    "selectTheCategoryType": {
      "title": "V čem je težava?"
    },
    "categoryType1": {
      "title": "Spletno mesto ne deluje po pričakovanjih"
    },
    "categoryType2": {
      "title": "Ni mi všeč vsebina na tej strani"
    },
    "categoryType3": {
      "title": "Splošne povratne informacije o brskalniku DuckDuckGo"
    },
    "selectTheCategory": {
      "title": "Kaj na tem spletnem mestu ne deluje?"
    },
    "tryTurningProtectionsOff": {
      "title": "Poskusite izklopiti zaščito zasebnosti in preveriti, ali to odpravi težavo."
    },
    "skipThisStep": {
      "title": "Preskoči ta korak"
    },
    "pickYourIssueFromTheList": {
      "title": "Opišite, kaj se je zgodilo"
    },
    "blocked": {
      "title": "Stran je blokirana ali se ni naložila"
    },
    "layout": {
      "title": "Postavitev spletnega mesta je neuporabna"
    },
    "emptySpaces": {
      "title": "Spletno mesto vsebuje velike prazne prostore"
    },
    "videos": {
      "title": "Video se ni predvajal ali naložil"
    },
    "images": {
      "title": "Slike se niso naložile"
    },
    "comments": {
      "title": "Komentarji, ocene ali klepeti se niso naložili"
    },
    "content": {
      "title": "Vsebina manjka"
    },
    "links": {
      "title": "Povezave ali tipke ne delujejo"
    },
    "login": {
      "title": "Ne morem se prijaviti/registrirati"
    },
    "loginV2": {
      "title": "Ne morem se prijaviti/registrirati"
    },
    "shopping": {
      "title": "Ne morem plačati, se odjaviti ali nakupovati"
    },
    "browser": {
      "title": "Težava z brskalnikom ali druga težava z razširitvijo brskalnika"
    },
    "paywall": {
      "title": "Spletno mesto me je pozvalo, naj onemogočim preprečevalec oglasov"
    },
    "other": {
      "title": "Nekaj drugega"
    },
    "tellUsMoreDesc": {
      "title": "Navedite več podrobnosti (neobvezno):{bullet}Kaj se je zgodilo?{bullet}Kaj bi se moralo zgoditi?{bullet}Ali je izklop zaščite odpravil težavo?"
    },
    "sendReport": {
      "title": "Pošljite poročilo"
    },
    "reportsAreAnonymousDesc": {
      "title": "Poročila, poslana DuckDuckGo, vključujejo samo informacije, ki so potrebne za obravnavanje vaših povratnih informacij."
    },
    "thankYou": {
      "title": "Hvala!"
    },
    "yourReportWillHelpDesc": {
      "title": "Vaše poročilo bo pripomoglo k izboljšanju brskalnika in boljši izkušnji drugih uporabnikov."
    },
    "dislike": {
      "title": "Vsebina mi ni všeč"
    },
    "otherRequired": {
      "title": "Opišite težavo, na katero ste naleteli (obvezno)"
    },
    "otherOptional": {
      "title": "Opišite težavo, na katero ste naleteli (izbirno)"
    },
    "suggestionWhatHappened": {
      "title": "Kaj se je zgodilo?"
    },
    "suggestionWhatHappened2": {
      "title": "Kaj bi se moralo zgoditi?"
    },
    "suggestionWhatHappened3": {
      "title": "Ali je izklop zaščite zasebnosti odpravil težavo?"
    },
    "reportTitle": {
      "title": "Prijavite DuckDuckGo"
    }
  },
  "shared": {
    "trackers": {
      "title": "Sledilniki"
    }
  },
  "site": {
    "updatingProtectionList": {
      "title": "Posodabljanje seznama zaščite"
    },
    "protectionsEnabled": {
      "title": "Zaščita za to stran je <b>OMOGOČENA</b>"
    },
    "protectionsDisabled": {
      "title": "Zaščita so za to stran je <b>ONEMOGOČENA</b>"
    },
    "protectionsDisabledRemote": {
      "title": "Zaščito zasebnosti smo začasno onemogočili, ker je videti, da zato prihaja do napak na tem spletnem mestu."
    },
    "protectionsDisabledRemoteOverride": {
      "title": "Priporočamo, da za to spletno mesto onemogočite zaščite zasebnosti, da preprečite prekinitev delovanja spletnega mesta."
    },
    "connectionDescriptionUnencrypted": {
      "title": "<b>To spletno mesto ni varno</b> in lahko ogrozi vse podatke, ki jih pošljete na tej strani."
    },
    "connectionDescriptionInvalidCertificate": {
      "title": "<b>Certifikat za to stran ni veljaven.</b> Morda se povezujete s strežnikom, ki se pretvarja, da je <b>{domain}</b>, kar bi lahko ogrozilo vaše zaupne podatke."
    },
    "trackerNetworksSummaryBlocked": {
      "title": "Nalaganje zahtevkov naslednjih domen tretjih oseb je bilo blokirano, ker so bili prepoznani kot zahtevki za sledenje. Če so zahtevki podjetja naloženi, lahko na podlagi tega oblikujejo vaš profil."
    },
    "trackerNetworksSummaryNoneBlocked": {
      "title": "Na tej strani nalaganje zahtevkov za sledenje ni bilo blokirano. Če so zahtevki podjetja naloženi, lahko na podlagi tega oblikujejo vaš profil."
    },
    "trackerNetworksSummaryNoneFound": {
      "title": "Na tej strani nismo odkrili nobenih zahtevkov za sledenje."
    },
    "trackerNetworksSummaryNone": {
      "title": "Na tej strani nismo našli nobenega podjetja, ki bi poskušalo naložiti zahtevke za sledenje."
    },
    "trackerNetworksSummaryAllowedOnly": {
      "title": "Da bi preprečili okvare spletnega mesta, na tej strani nismo blokirali nobenih podjetij, ki bi nalagala zahtevke za sledenje."
    },
    "trackerNetworksSummaryProtectionsOff": {
      "title": "Nalaganje zahtevkov za sledenje ni bilo blokirano, ker so zaščite za to spletno mesto izklopljene. Če so zahtevki podjetja naloženi, lahko na podlagi tega oblikujejo vaš profil."
    },
    "createNewDuckAddress": {
      "title": "Ustvarjanje zasebnega naslova Duck"
    },
    "createNewDuckAddressCopied": {
      "title": "Kopirano v odložišče!"
    },
    "websiteNotWorkingQ": {
      "title": "Spletno mesto ne deluje po pričakovanjih?"
    },
    "websiteNotWorkingPrompt": {
      "title": "Spletna stran ne deluje?"
    },
    "websiteNotWorkingCta": {
      "title": "Prijavite poškodovano spletno mesto"
    },
    "websiteNotWorkingAdvice": {
      "title": "IZKLOP zaščit lahko pomaga."
    },
    "websiteNotWorkingPromptFollowUp": {
      "title": "Stran še vedno ne deluje?"
    },
    "takePrecautions": {
      "title": "Previdnostni ukrepi"
    },
    "majorTrackingNetworkDesc": {
      "title": "{blocked, select, true {<b>To spletno mesto je v lasti podjetja {companyDisplayName}</b>, ki upravlja mrežo sledilnikov pri {companyPrevalence}% najboljših spletnih mest. Na tej strani smo uspeli blokirati nekatere njihove zahteve.} other {<b>To spletno mesto je v lasti podjetja {companyDisplayName}</b>, ki upravlja mrežo sledilnikov pri {companyPrevalence}% najboljših spletnih mest. }}"
    },
    "trackersBlockedDesc": {
      "title": "{companyCount, plural, =0 {Nekaterim podjetjem smo preprečili nalaganje zahtevkov za sledenje na tej strani.} =2 {Na tej strani smo blokirali nalaganje zahtevkov za sledenje podjetjema <b>{firstCompany}</b> in <b>{secondCompany}</b>.} =3 {Na tej strani smo blokirali nalaganje zahtevkov za sledenje podjetjem <b>{firstCompany}</b>, <b>{secondCompany}</b> in <b>{thirdCompany}</b>.} =4 {Na tej strani smo blokirali nalaganje zahtevkov za sledenje podjetjem <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b> in <b>{fourthCompany}</b>.} =5 {Na tej strani smo blokirali nalaganje zahtevkov za sledenje podjetjem <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b>, <b>{fourthCompany}</b> in <b>1 drugemu podjetju</b>.} one {Na tej strani smo blokirali nalaganje zahtevkov za sledenje podjetju <b>{firstCompany}</b>.} two {Na tej strani smo blokirali nalaganje zahtevkov za sledenje podjetjem <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b>, <b>{fourthCompany}</b> in <b>{othersCount} drugima podjetjema</b>.} few {Na tej strani smo blokirali nalaganje zahtevkov za sledenje podjetjem <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b>, <b>{fourthCompany}</b> in <b>{othersCount} drugim podjetjem</b>.} other {Na tej strani smo blokirali nalaganje zahtevkov za sledenje <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b>, <b>{fourthCompany}</b> in <b>{othersCount} drugim podjetjem</b>.}}"
    },
    "cookiesMinimized": {
      "title": "Upravljani piškotki"
    },
    "cookiesHidden": {
      "title": "Pojavno okno za piškotke je skrito"
    },
    "cookiesHiddenSummary": {
      "title": "Na tem spletnem mestu smo lahko le skrili pojavno okno za piškotke, ker ni bilo na voljo nobenih možnosti za upravljanje nastavitev piškotkov. Naša preostala zaščita pred spletnim sledenjem se še vedno izvaja."
    },
    "cookiesMinimizedSummary": {
      "title": "Nastavili smo vaše nastavitve piškotkov za čim večjo zasebnost in zaprli pojavno okno."
    },
    "cookiesMinimizedSettings": {
      "title": "Onemogoči v nastavitvah"
    },
    "connectionSecure": {
      "title": "Povezava je šifrirana"
    },
    "connectionNotSecure": {
      "title": "Povezava ni šifrirana"
    },
    "connectionNotSecureInvalidCertificate": {
      "title": "Povezava je morda nevarna"
    },
    "trackerNetworksDesc": {
      "title": "Zahtevki, katerih nalaganje je blokirano"
    },
    "trackerNetworksNotBlocked": {
      "title": "Noben zahtevek za sledenje ni bil blokiran"
    },
    "trackerNetworksNotFound": {
      "title": "Nobenega zahtevka za sledenje ni bilo mogoče najti"
    },
    "thirdPartiesLoaded": {
      "title": "Nalaganje zahtevkov tretjih oseb"
    },
    "thirdPartiesNoneFound": {
      "title": "Zahtev tretjih oseb ni bilo mogoče najti"
    },
    "firstPartyDesc": {
      "title": "Podjetje {companyName} je lastnik tega spletnega mesta in znanih sledilnikov na tej strani, zato jih nismo blokirali."
    },
    "noTrackersFound": {
      "title": "Na tej strani nismo našli nobenega podjetja, ki bi poskušalo naložiti zahtevke za sledenje."
    },
    "trackersFoundForAllowlisted": {
      "title": "Sledilniki podjetjem pomagajo pri oblikovanju vašega profila. Ta podjetja spremljajo vaše dejavnosti na tej strani."
    },
    "trackersFoundAndBlocked": {
      "title": "Sledilniki podjetjem pomagajo pri oblikovanju vašega profila. Tem podjetjem smo preprečili spremljanje vaših dejavnosti na tej strani."
    },
    "trackerNetworkUnknown": {
      "title": "Neznano omrežje sledilnika"
    },
    "trackerDomainsForCompany": {
      "title": "Domene sledilnikov podjetja {companyName}"
    },
    "zeroTrackersFound": {
      "title": "Nobenega sledilnika ni bilo mogoče najti"
    },
    "trackerOwnedByThisSite": {
      "title": "Sledilniki v lasti tega spletnega mesta (dovoljeni)"
    },
    "trackerCountForDomain": {
      "title": "{blocked, select, true {{trackerCount, plural, one {{trackerCount} sledilnik je bil blokiran na {domain}} two {{trackerCount} sledilnika sta bila blokirana na {domain}} few {{trackerCount} sledilniki so bili blokirani na {domain}} other {{trackerCount} sledilnikov je bilo blokiranih na {domain}}}} other {{trackerCount, plural, one {{trackerCount} sledilnik je bil najden na {domain}} two {{trackerCount} sledilnika sta bila najdena na {domain}} few {{trackerCount} sledilniki so bili najdeni na {domain}} other {{trackerCount} sledilnikov je bilo najdenih na {domain}}}}}"
    },
    "trackerLimitationsNote": {
      "title": "Upoštevajte: omejitve platforme lahko omejijo našo zmožnost zaznavanja vseh zahtevkov."
    },
    "trackerAboutLink": {
      "title": "O naši zaščiti spletnega sledenja"
    },
    "trackerAdLink": {
      "title": "Kako naši oglasi v iskalnem omrežju vplivajo na našo zaščito"
    },
    "sectionHeadingAdAttribution": {
      "title": "Naslednji zahtevki domen so bili naloženi, ker ste pred kratkim kliknili oglas {domain} na portalu DuckDuckGo. Ti zahtevki pomagajo oceniti učinkovitost oglasa. Vsi oglasi na DuckDuckGo so neprofilirani."
    },
    "sectionHeadingIgnore": {
      "title": "Naložene so bile naslednje zahteve za domene, da se prepreči prekinitev spletnega mesta."
    },
    "sectionHeadingFirstParty": {
      "title": "Naloženi so bili naslednji zahtevki domen, ker so povezani z [{domain}]."
    },
    "sectionHeadingThirdParty": {
      "title": "Naložene so bile tudi naslednje zahteve domene."
    },
    "sectionHeadingProtectionsDisabled": {
      "title": "Zahteve naslednjih domen so bile naložene, ker so zaščite izklopljene."
    },
    "thirdPartiesSummaryLoaded": {
      "title": "Naložene so bile naslednje zahteve za domene tretjih oseb. Če so zahteve podjetja izpolnjene, lahko podjetje na podlagi tega oblikuje vaš profil, čeprav še vedno veljajo naša druga zaščitna pravila za sledenje v spletu."
    },
    "thirdPartiesSummaryProtectionsOff": {
      "title": "Naložene so bile naslednje zahteve za domene tretjih oseb. Če so zahteve podjetja izpolnjene, lahko podjetje na podlagi tega oblikuje vaš profil, čeprav še vedno veljajo naša druga zaščitna pravila za sledenje v spletu."
    },
    "thirdPartiesSummaryNone": {
      "title": "Nismo ugotovili nobenih zahtevkov z domen tretjih oseb."
    },
    "analyticsCategory": {
      "title": "Analitika"
    },
    "advertisingCategory": {
      "title": "Oglaševanje"
    },
    "socialCategory": {
      "title": "Družbena omrežja"
    },
    "contentDeliveryCategory": {
      "title": "Dostava vsebine"
    },
    "embeddedContentCategory": {
      "title": "Vdelana vsebina"
    },
    "searchPlaceholder": {
      "title": "Iščite po DuckDuckGo"
    },
    "searchGoButton": {
      "title": "Iskanje"
    },
    "optionsButton": {
      "title": "Več možnosti"
    },
    "navigationComplete": {
      "title": "Končano"
    },
    "navigationClose": {
      "title": "Zapri"
    },
    "navigationCancel": {
      "title": "Prekliči"
    },
    "navigationBack": {
      "title": "Nazaj"
    },
    "enableProtectionsSwitch": {
      "title": "Omogoči zaščito"
    },
    "disableProtectionsSwitch": {
      "title": "Onemogoči zaščito"
    },
    "errorMessage": {
      "title": "Nekaj je šlo narobe in te vsebine nismo mogli naložiti. Poskusite znova naložiti stran."
    }
  }
}