export default {
  "connection": {
    "encrypt": {
      "title": "Szyfruj"
    },
    "sign": {
      "title": "Podpisz"
    },
    "verify": {
      "title": "Zweryfikuj"
    },
    "derive": {
      "title": "Utwórz pochodne"
    },
    "wrap": {
      "title": "Zapakuj"
    },
    "unwrap": {
      "title": "Odpakuj"
    },
    "unknown": {
      "title": "Nieznane"
    },
    "certificateDetail": {
      "title": "Szczegóły certyfikatu bezpieczeństwa"
    },
    "commonName": {
      "title": "Nazwa ogólna"
    },
    "publicKey": {
      "title": "Klucz publiczny"
    },
    "algorithm": {
      "title": "Algorytm"
    },
    "keySize": {
      "title": "Rozmiar klucza"
    },
    "usage": {
      "title": "Zastosowanie"
    },
    "summary": {
      "title": "Podsumowanie"
    },
    "permanent": {
      "title": "Stałe"
    },
    "effectiveSize": {
      "title": "Efektywny rozmiar"
    },
    "certificateNotFound": {
      "title": "Nie znaleziono certyfikatu"
    },
    "certificateForDomain": {
      "title": "Certyfikat dla {domain}"
    },
    "insecureConnectionDesc": {
      "title": "Ta strona korzysta z połączenia nieszyfrowanego. Strony trzecie mogą być w stanie obserwować Twoją aktywność lub przechwycić poufne informacje, które przesyłasz na tej stronie."
    },
    "invalidConnectionDesc": {
      "title": "Certyfikat tej witryny jest nieprawidłowy. Być może łączysz się z serwerem podszywającym się pod <b>{domain}</b>, co może narazić poufne informacje na niebezpieczeństwo."
    },
    "upgradedConnectionDesc": {
      "title": "Uaktualniliśmy połączenie na tej stronie, aby chronić przesyłane informacje."
    },
    "secureConnectionDesc": {
      "title": "Ta strona wykorzystuje połączenie szyfrowane, które uniemożliwia osobom trzecim obserwowanie Twojej aktywności lub przechwytywanie poufnych informacji przesyłanych przez Ciebie na tej stronie."
    }
  },
  "ctascreens": {
    "protectionsUnavailableNote": {
      "title": "Ochrona prywatności nie jest dostępna dla stron specjalnych i lokalnych."
    },
    "spreadTitle": {
      "title": "Lubisz korzystać z DuckDuckGo?"
    },
    "spreadText": {
      "title": "Powiedz o nas rodzinie i znajomym"
    },
    "spreadButton": {
      "title": "Promuj DuckDuckGo"
    },
    "emailTitle": {
      "title": "Masz dość śledzenia e-maili?"
    },
    "emailText": {
      "title": "Skorzystaj z ochrony e-mail DuckDuckGo Email Protection w ramach swojego rozszerzenia teraz!"
    }
  },
  "firebutton": {
    "fireDialogHeader": {
      "title": "Zamknij karty i wyczyść dane"
    },
    "fireDialogHeaderNoTabs": {
      "title": "Wyczyść dane"
    },
    "optionCurrentSite": {
      "title": "Tylko bieżąca strona"
    },
    "optionLastHour": {
      "title": "Ostatnia godzina"
    },
    "optionLast24Hour": {
      "title": "Ostatnie 24 godziny"
    },
    "optionLast7days": {
      "title": "Ostatnie 7 dni"
    },
    "optionLast4Weeks": {
      "title": "Ostatnie 4 tygodnie"
    },
    "optionAllTime": {
      "title": "Cały czas"
    },
    "historyDuration": {
      "title": "{duration, select, hour {jedna godzina} day {24 godziny} week {jeden tydzień} month {4 tygodnie} other {Wszystko}}"
    },
    "summaryClearTabsHistoryDuration": {
      "title": "{openTabs, plural, one {Zamknąć <b>{openTabs}</b> zakładkę wyczyścić <b>{durationDesc}</b> historię przeglądania i pliki cookie?} few {Zamknąć <b>{openTabs}</b> zakładki i wyczyścić <b>{durationDesc}</b> historię przeglądania i pliki cookie?} many {Zamknąć <b>{openTabs}</b> zakładek i wyczyścić <b>{durationDesc}</b> historię przeglądania i pliki cookie?} other {Zamknąć <b>{openTabs}</b> zakładki i wyczyścić <b>{durationDesc}</b> historię przeglądania i pliki cookie?}}"
    },
    "summaryClearTabsDuration": {
      "title": "{openTabs, plural, one {Zamknąć <b>{openTabs}</b> kartę i wyczyścić <b>{durationDesc}</b> plików cookie?} few {Zamknąć <b>{openTabs}</b> karty i wyczyścić <b>{durationDesc}</b> plików cookie?} many {Zamknąć <b>{openTabs}</b> kart i wyczyścić <b>{durationDesc}</b> plików cookie?} other {Zamknąć <b>{openTabs}</b> karty i wyczyścić <b>{durationDesc}</b> plików cookie?}}"
    },
    "summaryClearHistoryDuration": {
      "title": "Wyczyścić <b>{durationDesc}</b> historii przeglądania i pliki cookie?"
    },
    "summaryClearCookiesDuration": {
      "title": "Wyczyścić <b>{durationDesc}</b> plików cookie?"
    },
    "summaryClearTabsHistoryAll": {
      "title": "Zamknij <b>{openTabs}</b> {openTabs, plural, =1 {kartę} other {kart(y)}} i wyczyść <b>całą</b> historię przeglądania i pliki cookie ({cookies} {cookies, plural, =1 {strony} other {str.}})?"
    },
    "summaryClearTabsAll": {
      "title": "Zamknij <b>{openTabs}</b> {openTabs, plural, =1 {kartę} other {kart(y)}}, i wyczyść <b>wszystkie</b> pliki cookie ({cookies} {cookies, plural, =1 {strony} other {str.}})?"
    },
    "summaryClearHistoryAll": {
      "title": "{cookies, plural, one {Wyczyścić <b>całą</b> historię przeglądania i pliki cookie ({cookies} strony)?} few {Wyczyścić <b>całą</b> historię przeglądania i pliki cookie ({cookies} stron)?} many {Wyczyścić <b>całą</b> historię przeglądania i pliki cookie ({cookies} stron)?} other {Wyczyścić <b>całą</b> historię przeglądania i pliki cookie ({cookies} strony)?}}"
    },
    "summaryClearCookiesAll": {
      "title": "{cookies, plural, one {Wyczyścić <b>wszystkie</b> pliki cookie ({cookies} strony)?} few {Wyczyścić <b>wszystkie</b> pliki cookie ({cookies} stron)?} many {Wyczyścić <b>wszystkie</b> pliki cookie ({cookies} stron)?} other {Wyczyścić <b>wszystkie</b> pliki cookie ({cookies} strony)?}}"
    },
    "summaryClearTabsHistorySite": {
      "title": "{openTabs, plural, one {Zamknąć <b>{openTabs} kartę {site}</b> i wyczyścić <b>wszystkie pliki cookie {site}</b>?} few {Zamknąć <b>{openTabs} karty {site}</b> i wyczyścić <b>wszystkie pliki cookie {site}</b>?} many {Zamknąć <b>{openTabs} kart {site}</b> i wyczyścić <b>wszystkie pliki cookie {site}</b>?} other {Zamknąć <b>{openTabs} karty {site}</b> i wyczyścić <b>wszystkie pliki cookie {site}</b>?}}"
    },
    "summaryClearTabsSite": {
      "title": "{openTabs, plural, one {Zamknąć <b>{openTabs} kartę {site}</b> i wyczyścić <b>wszystkie pliki cookie {site}</b>?} few {Zamknąć <b>{openTabs} karty {site}</b> i wyczyścić <b>wszystkie pliki cookie {site}</b>?} many {Zamknąć <b>{openTabs} kart {site}</b> i wyczyścić <b>wszystkie pliki cookie {site}</b>?} other {Zamknąć <b>{openTabs} karty {site}</b> i wyczyścić <b>wszystkie pliki cookie {site}</b>?}}"
    },
    "summaryClearHistorySite": {
      "title": "Wyczyścić <b>całą historię przeglądania i pliki cookie {site}</b>?"
    },
    "summaryClearCookiesSite": {
      "title": "Wyczyścić <b>wszystkie pliki cookie {site}</b>?"
    },
    "summaryPinnedIgnored": {
      "title": "{tabs, plural, one {<b>{tabs} przypięta karta</b> zostanie zignorowana.} few {<b>{tabs} przypięte karty</b> zostaną zignorowane.} many {<b>{tabs} przypiętych kart</b> zostanie zignorowanych.} other {<b>{tabs} przypiętej karty</b> zostanie zignorowane.}}"
    },
    "clearData": {
      "title": "Wyczyść"
    },
    "cancel": {
      "title": "Anuluj"
    },
    "historyAndDownloadsNotAffected": {
      "title": "Aby wyczyścić również historię, wybierz okres."
    }
  },
  "permissions": {
    "camera": {
      "title": "Aparat"
    },
    "microphone": {
      "title": "Mikrofon"
    },
    "geolocation": {
      "title": "Geolokalizacja"
    },
    "popups": {
      "title": "Wyskakujące okienka"
    },
    "ask": {
      "title": "Pytaj za każdym razem"
    },
    "notify": {
      "title": "Powiadom"
    },
    "grant": {
      "title": "Zawsze zezwalaj"
    },
    "deny": {
      "title": "Zawsze odmawiaj"
    }
  },
  "report": {
    "selectTheOptionDesc": {
      "title": "Przesłanie anonimowego raportu o uszkodzonej witrynie pomaga nam ulepszać aplikację."
    },
    "selectTheOptionDescV2": {
      "title": "Wybierz opcję, która najlepiej opisuje napotkany problem."
    },
    "selectTheCategoryType": {
      "title": "Na czym polega problem?"
    },
    "categoryType1": {
      "title": "Witryna internetowa nie działa zgodnie z oczekiwaniami"
    },
    "categoryType2": {
      "title": "Nie podoba mi się zawartość tej strony"
    },
    "categoryType3": {
      "title": "Ogólne opinie dotyczące przeglądarki DuckDuckGo"
    },
    "selectTheCategory": {
      "title": "Co nie działa na tej witrynie?"
    },
    "tryTurningProtectionsOff": {
      "title": "Spróbuj wyłączyć mechanizmy ochrony prywatności, aby sprawdzić, czy to rozwiąże problem."
    },
    "skipThisStep": {
      "title": "Pomiń ten krok"
    },
    "pickYourIssueFromTheList": {
      "title": "Opisz, co się stało"
    },
    "blocked": {
      "title": "Witryna zablokowana lub nie została załadowana"
    },
    "layout": {
      "title": "Uszkodzony układ witryny"
    },
    "emptySpaces": {
      "title": "Witryna zawiera duże puste przestrzenie"
    },
    "videos": {
      "title": "Film nie został odtworzony lub załadowany"
    },
    "images": {
      "title": "Obrazy nie zostały załadowane"
    },
    "comments": {
      "title": "Komentarze, recenzje lub czaty nie zostały załadowane"
    },
    "content": {
      "title": "Brakuje treści"
    },
    "links": {
      "title": "Linki lub przyciski nie działają"
    },
    "login": {
      "title": "Nie mogę się zalogować/zarejestrować"
    },
    "loginV2": {
      "title": "Nie mogę się zalogować lub zarejestrować"
    },
    "shopping": {
      "title": "Nie mogę zapłacić, finalizować ani zrobić zakupów"
    },
    "browser": {
      "title": "Problem z przeglądarką lub rozszerzeniem przeglądarki"
    },
    "paywall": {
      "title": "Witryna poprosiła mnie o wyłączenie blokady reklam"
    },
    "other": {
      "title": "Coś innego"
    },
    "tellUsMoreDesc": {
      "title": "Udostępnij więcej szczegółów (opcjonalnie):{bullet}Co się stało?{bullet}Co powinno się wydarzyć?{bullet}Czy wyłączenie zabezpieczeń pomaga?"
    },
    "sendReport": {
      "title": "Wyślij raport"
    },
    "reportsAreAnonymousDesc": {
      "title": "Raporty wysyłane do DuckDuckGo zawierają jedynie informacje potrzebne nam do podjęcia działań w związku z Twoimi uwagami."
    },
    "thankYou": {
      "title": "Dziękujemy!"
    },
    "yourReportWillHelpDesc": {
      "title": "Twoje zgłoszenie pomoże ulepszyć nasze produkty i poprawić komfort korzystania innych użytkowników."
    },
    "dislike": {
      "title": "Nie podoba mi się zawartość"
    },
    "otherRequired": {
      "title": "Opisz napotkany problem (wymagane)"
    },
    "otherOptional": {
      "title": "Opisz napotkany problem (opcjonalnie)"
    },
    "suggestionWhatHappened": {
      "title": "Co się stało?"
    },
    "suggestionWhatHappened2": {
      "title": "Co powinno się wydarzyć?"
    },
    "suggestionWhatHappened3": {
      "title": "Czy wyłączenie mechanizmów ochrony prywatności pomogło?"
    },
    "reportTitle": {
      "title": "Zgłoś do DuckDuckGo"
    }
  },
  "shared": {
    "trackers": {
      "title": "Skrypty śledzące"
    }
  },
  "site": {
    "updatingProtectionList": {
      "title": "Aktualizowanie listy ochrony"
    },
    "protectionsEnabled": {
      "title": "Zabezpieczenia są <b>WŁĄCZONE</b> dla tej witryny"
    },
    "protectionsDisabled": {
      "title": "Zabezpieczenia są <b>WYŁĄCZONE</b> dla tej witryny"
    },
    "protectionsDisabledRemote": {
      "title": "Tymczasowo wyłączyliśmy ochronę prywatności, ponieważ wygląda na to, że zniekształca tę witrynę."
    },
    "protectionsDisabledRemoteOverride": {
      "title": "Zalecamy wyłączenie ochrony prywatności na tej stronie, aby zapobiec jej awarii."
    },
    "connectionDescriptionUnencrypted": {
      "title": "<b>Ta witryna nie jest bezpieczna</b> i może ujawnić wszystkie informacje, które w niej przesyłasz."
    },
    "connectionDescriptionInvalidCertificate": {
      "title": "<b>Certyfikat tej witryny jest nieprawidłowy.</b> Być może łączysz się z serwerem podszywającym się pod <b>{domain}</b>, co może narazić poufne informacje na niebezpieczeństwo."
    },
    "trackerNetworksSummaryBlocked": {
      "title": "Następujące żądania domen innych firm zostały zablokowane przed załadowaniem, ponieważ zostały zidentyfikowane jako żądania ze śledzeniem. Załadowanie żądań firmy może umożliwić jej profilowanie użytkownika."
    },
    "trackerNetworksSummaryNoneBlocked": {
      "title": "Na tej stronie nie zablokowano ładowania żadnych żądań ze śledzeniem. Załadowanie żądań firmy może umożliwić jej profilowanie użytkownika."
    },
    "trackerNetworksSummaryNoneFound": {
      "title": "Nie wykryliśmy żadnych żądań ze śledzeniem na tej stronie."
    },
    "trackerNetworksSummaryNone": {
      "title": "Nie zauważyliśmy, żeby jakieś firmy próbowały umieścić na tej stronie żądania ze śledzeniem."
    },
    "trackerNetworksSummaryAllowedOnly": {
      "title": "Nie zablokowaliśmy żadnym firmom możliwości ładowania żądań ze śledzeniem na tej stronie, aby zapobiec jej uszkodzeniu."
    },
    "trackerNetworksSummaryProtectionsOff": {
      "title": "Żadne żądania ze śledzeniem nie zostały zablokowane przed załadowaniem, ponieważ zabezpieczenia są wyłączone dla tej witryny. Załadowanie żądań firmy może umożliwić jej profilowanie użytkownika."
    },
    "createNewDuckAddress": {
      "title": "Wygeneruj prywatny adres Duck Address"
    },
    "createNewDuckAddressCopied": {
      "title": "Skopiowano do schowka!"
    },
    "websiteNotWorkingQ": {
      "title": "Witryna internetowa nie działa zgodnie z oczekiwaniami?"
    },
    "websiteNotWorkingPrompt": {
      "title": "Witryna internetowa nie działa?"
    },
    "websiteNotWorkingCta": {
      "title": "Zgłoś uszkodzoną witrynę"
    },
    "websiteNotWorkingAdvice": {
      "title": "WYŁĄCZENIE zabezpieczeń może pomóc."
    },
    "websiteNotWorkingPromptFollowUp": {
      "title": "Witryna nadal nie działa?"
    },
    "takePrecautions": {
      "title": "Zachowaj środki ostrożności"
    },
    "majorTrackingNetworkDesc": {
      "title": "{blocked, select, true {<b>Ta strona należy do firmy {companyDisplayName}</b>, która obsługuje sieć systemów śledzących w {companyPrevalence}% najbardziej popularnych witryn. Udało nam się zablokować niektóre z jej żądań na tej stronie.} other {<b>Ta strona należy do firmy {companyDisplayName}</b>, która obsługuje sieć systemów śledzących w {companyPrevalence}% najbardziej popularnych witryn. }}"
    },
    "trackersBlockedDesc": {
      "title": "{companyCount, plural, =0 {Zablokowaliśmy ładowanie żądań śledzenia wysyłanych przez niektóre firmy na tej stronie.} =2 {Zablokowaliśmy firmom <b>{firstCompany}</b> oraz <b>{secondCompany}</b> możliwość umieszczania na tej stronie żądań ze śledzeniem.} =3 {Zablokowaliśmy firmom <b>{firstCompany}</b>, <b>{secondCompany}</b> oraz <b>{thirdCompany}</b> możliwość umieszczania na tej stronie żądań ze śledzeniem.} =4 {Zablokowaliśmy firmom <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b> oraz <b>{fourthCompany}</b> możliwość umieszczania na tej stronie żądań ze śledzeniem.} =5 {Zablokowaliśmy firmom <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b>, <b>{fourthCompany}</b> oraz <b>1 innej firmie</b> możliwość umieszczania na tej stronie żądań ze śledzeniem.} one {Zablokowaliśmy firmie <b>{firstCompany}</b> możliwość umieszczania na tej stronie żądań ze śledzeniem.} few {Zablokowaliśmy firmom <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b>, <b>{fourthCompany}</b> i<b> {othersCount} innym firmom</b> możliwość umieszczania na tej stronie żądań ze śledzeniem.} many {Zablokowaliśmy firmom <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b>, <b>{fourthCompany}</b> i <b>{othersCount} innym firmom</b> możliwość umieszczania na tej stronie żądań ze śledzeniem.} other {Zablokowaliśmy firmom <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b>, <b>{fourthCompany}</b> i <b>{othersCount} innym firmom</b> możliwość umieszczania na tej stronie żądań ze śledzeniem.}}"
    },
    "cookiesMinimized": {
      "title": "Zarządzane pliki cookie"
    },
    "cookiesHidden": {
      "title": "Ukryte wyskakujące okienka dotyczące plików cookie"
    },
    "cookiesHiddenSummary": {
      "title": "Na tej stronie udało nam się tylko ukryć wyskakujące okienko dotyczące plików cookie, ponieważ nie zawierało ono opcji umożliwiających zarządzanie preferencjami dotyczącymi takich plików. Pozostałe zabezpieczenia przed śledzeniem w sieci nadal działają."
    },
    "cookiesMinimizedSummary": {
      "title": "Ustawiliśmy preferencje dotyczące plików cookie, aby zmaksymalizować prywatność i zamknęliśmy wyskakujące okienko."
    },
    "cookiesMinimizedSettings": {
      "title": "Wyłącz w ustawieniach"
    },
    "connectionSecure": {
      "title": "Połączenie jest szyfrowane"
    },
    "connectionNotSecure": {
      "title": "Połączenie nie jest szyfrowane"
    },
    "connectionNotSecureInvalidCertificate": {
      "title": "Połączenie może być niebezpieczne"
    },
    "trackerNetworksDesc": {
      "title": "Żądania zablokowane przed załadowaniem"
    },
    "trackerNetworksNotBlocked": {
      "title": "Brak zablokowanych żądań ze śledzeniem"
    },
    "trackerNetworksNotFound": {
      "title": "Nie znaleziono żądań ze śledzeniem"
    },
    "thirdPartiesLoaded": {
      "title": "Załadowano żądania innych firm"
    },
    "thirdPartiesNoneFound": {
      "title": "Nie znaleziono żądań stron trzecich"
    },
    "firstPartyDesc": {
      "title": "{companyName} jest właścicielem tej witryny i znanych skryptów śledzących znalezionych na tej stronie, dlatego ich nie zablokowaliśmy."
    },
    "noTrackersFound": {
      "title": "Nie zauważyliśmy, żeby jakieś firmy próbowały umieścić na tej stronie żądania ze śledzeniem."
    },
    "trackersFoundForAllowlisted": {
      "title": "Skrypty śledzące ułatwiają firmom profilowanie użytkownika. Znaleźliśmy następujące firmy, które monitorują Twoją aktywność na tej stronie."
    },
    "trackersFoundAndBlocked": {
      "title": "Skrypty śledzące ułatwiają firmom profilowanie użytkownika. Zablokowaliśmy tym firmom możliwość monitorowania Twojej aktywności na tej stronie."
    },
    "trackerNetworkUnknown": {
      "title": "Nieznana sieć skryptów śledzących"
    },
    "trackerDomainsForCompany": {
      "title": "Domeny skryptów śledzących {companyName}"
    },
    "zeroTrackersFound": {
      "title": "Znaleziono 0 skryptów śledzących"
    },
    "trackerOwnedByThisSite": {
      "title": "Skrypty śledzące należące do tej strony (dozwolone)"
    },
    "trackerCountForDomain": {
      "title": "{blocked, select, true {{trackerCount, plural, one {Zablokowano {trackerCount} skrypt śledzący na {domain}} few {Zablokowano {trackerCount} skrypty śledzące na {domain}} many {Zablokowano {trackerCount} skryptów śledzących na {domain}} other {Zablokowano {trackerCount} skryptu śledzącego na {domain}}}} other {{trackerCount, plural, one {Znaleziono {trackerCount} skrypt śledzący na {domain}} few {Znaleziono {trackerCount} skrypty śledzące na {domain}} many {Znaleziono {trackerCount} skryptów śledzących na {domain}} other {Znaleziono {trackerCount} skryptu śledzącego na {domain}}}}}"
    },
    "trackerLimitationsNote": {
      "title": "Uwaga: ograniczenia platformy mogą zmniejszać możliwość wykrywania wszystkich żądań."
    },
    "trackerAboutLink": {
      "title": "Informacje o zabezpieczeniach przed śledzeniem w sieci"
    },
    "trackerAdLink": {
      "title": "Jak reklamy w wyszukiwarce wpływają na zabezpieczenia"
    },
    "sectionHeadingAdAttribution": {
      "title": "Następujące żądania domeny zostały załadowane, ponieważ niedawno została kliknięta reklama {domain} na DuckDuckGo. Te żądania pomagają ocenić skuteczność reklam. Wszystkie reklamy na DuckDuckGo są nieprofilowane."
    },
    "sectionHeadingIgnore": {
      "title": "Następujące żądania domeny zostały załadowane, aby zapobiec uszkodzeniu witryny."
    },
    "sectionHeadingFirstParty": {
      "title": "Następujące żądania domeny zostały załadowane, ponieważ są powiązane z {domain}."
    },
    "sectionHeadingThirdParty": {
      "title": "Załadowano również żądania następującej domeny."
    },
    "sectionHeadingProtectionsDisabled": {
      "title": "Żądania następujących domen zostały załadowane, ponieważ zabezpieczenia są wyłączone."
    },
    "thirdPartiesSummaryLoaded": {
      "title": "Załadowano następujące żądania domen innych firm. Załadowanie żądań firmy może umożliwić jej profilowanie użytkownika, choć nadal mają zastosowanie nasze pozostałe zabezpieczenia dotyczące śledzenia w sieci."
    },
    "thirdPartiesSummaryProtectionsOff": {
      "title": "Załadowano następujące żądania domen innych firm. Załadowanie żądań firmy może umożliwić jej profilowanie użytkownika, choć nadal mają zastosowanie nasze pozostałe zabezpieczenia dotyczące śledzenia w sieci."
    },
    "thirdPartiesSummaryNone": {
      "title": "Nie zidentyfikowaliśmy żadnych żądań z domen stron trzecich."
    },
    "analyticsCategory": {
      "title": "Analityka"
    },
    "advertisingCategory": {
      "title": "Reklama"
    },
    "socialCategory": {
      "title": "Sieć społecznościowa"
    },
    "contentDeliveryCategory": {
      "title": "Dostarczanie treści"
    },
    "embeddedContentCategory": {
      "title": "Treść osadzona"
    },
    "searchPlaceholder": {
      "title": "Szukaj w DuckDuckGo"
    },
    "searchGoButton": {
      "title": "Szukaj"
    },
    "optionsButton": {
      "title": "Więcej opcji"
    },
    "navigationComplete": {
      "title": "Gotowe"
    },
    "navigationClose": {
      "title": "Zamknij"
    },
    "navigationCancel": {
      "title": "Anuluj"
    },
    "navigationBack": {
      "title": "Wstecz"
    },
    "enableProtectionsSwitch": {
      "title": "Włącz ochronę"
    },
    "disableProtectionsSwitch": {
      "title": "Wyłącz ochronę"
    },
    "errorMessage": {
      "title": "Coś poszło nie tak i nie udało się załadować tej zawartości. Spróbuj ponownie załadować stronę."
    }
  }
}