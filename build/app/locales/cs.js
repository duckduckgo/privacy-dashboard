export default {
  "connection": {
    "encrypt": {
      "title": "Zašifrujte"
    },
    "sign": {
      "title": "Podepište"
    },
    "verify": {
      "title": "Ověřte"
    },
    "derive": {
      "title": "Odvoďte"
    },
    "wrap": {
      "title": "Zabalte"
    },
    "unwrap": {
      "title": "Rozbalte"
    },
    "unknown": {
      "title": "Neznámé"
    },
    "certificateDetail": {
      "title": "Podrobnosti o bezpečnostním certifikátu"
    },
    "commonName": {
      "title": "Obecný název"
    },
    "publicKey": {
      "title": "Veřejný klíč"
    },
    "algorithm": {
      "title": "Algoritmus"
    },
    "keySize": {
      "title": "Velikost klíče"
    },
    "usage": {
      "title": "Využití"
    },
    "summary": {
      "title": "Shrnutí"
    },
    "permanent": {
      "title": "Trvale"
    },
    "effectiveSize": {
      "title": "Efektivní velikost"
    },
    "certificateNotFound": {
      "title": "Certifikát nenalezen"
    },
    "certificateForDomain": {
      "title": "Certifikát pro doménu {domain}"
    },
    "insecureConnectionDesc": {
      "title": "Tahle stránka používá nešifrované připojení. Třetí strany můžou zobrazit tvou aktivitu nebo zachytit citlivé informace, které na téhle stránce zadáváš."
    },
    "invalidConnectionDesc": {
      "title": "Tenhle web nemá platný certifikát. Možná se dokonce připojuješ k serveru, který předstírá, že je <b>{domain}</b>. Tvoje důvěrné informace by mohly být ohrožené."
    },
    "upgradedConnectionDesc": {
      "title": "Aktualizovali jsme připojení na téhle stránce, abychom ochránili zadávané informace."
    },
    "secureConnectionDesc": {
      "title": "Tahle stránka používá šifrované připojení. Třetí strany proto nemůžou zobrazit tvou aktivitu a zachytit citlivé informace, které na téhle stránce zadáváš."
    }
  },
  "ctascreens": {
    "protectionsUnavailableNote": {
      "title": "Ochrana soukromí není dostupná pro speciální stránky nebo lokální stránky."
    },
    "spreadTitle": {
      "title": "Baví tě DuckDuckGo?"
    },
    "spreadText": {
      "title": "Tak o nás řekni rodině a kamarádům."
    },
    "spreadButton": {
      "title": "Šiřte DuckDuckGo"
    },
    "emailTitle": {
      "title": "Už máš po krk toho, že tvoje e-maily jsou sledované?"
    },
    "emailText": {
      "title": "Tak na nic nečekej a pořiď si k rozšíření ještě ochranu e-mailů DuckDuckGo Email Protection!"
    }
  },
  "firebutton": {
    "fireDialogHeader": {
      "title": "Zavřít karty a vymazat data"
    },
    "fireDialogHeaderNoTabs": {
      "title": "Vymazat data"
    },
    "optionCurrentSite": {
      "title": "Jen aktuální stránka"
    },
    "optionLastHour": {
      "title": "Poslední hodina"
    },
    "optionLast24Hour": {
      "title": "Posledních 24 hodin"
    },
    "optionLast7days": {
      "title": "Posledních 7 dní"
    },
    "optionLast4Weeks": {
      "title": "Poslední 4 týdny"
    },
    "optionAllTime": {
      "title": "Po celou dobu"
    },
    "historyDuration": {
      "title": "{duration, select, hour {jednu hodinu} day {24 hodin} week {jeden týden} month {4 týdny} other {Vše}}"
    },
    "summaryClearTabsHistoryDuration": {
      "title": "{openTabs, plural, one {Zavřít <b>{openTabs}</b> kartu a vymazat <b>{durationDesc}</b> historie procházení a souborů cookie?} few {Zavřít <b>{openTabs}</b> karty a vymazat <b>{durationDesc}</b> historie procházení a souborů cookie?} many {Zavřít <b>{openTabs}</b> karty a vymazat <b>{durationDesc}</b> historie procházení a souborů cookie?} other {Zavřít <b>{openTabs}</b> karet a vymazat <b>{durationDesc}</b> historie procházení a souborů cookie?}}"
    },
    "summaryClearTabsDuration": {
      "title": "{openTabs, plural, one {Zavřít <b>{openTabs}</b> kartu a vymazat <b>{durationDesc}</b> souborů cookie?} few {Zavřít <b>{openTabs}</b> karty a vymazat <b>{durationDesc}</b> souborů cookie?} many {Zavřít <b>{openTabs}</b> karty a vymazat <b>{durationDesc}</b> souborů cookie?} other {Zavřít <b>{openTabs}</b> karet a vymazat <b>{durationDesc}</b> souborů cookie?}}"
    },
    "summaryClearHistoryDuration": {
      "title": "Vymazat <b>{durationDesc}</b> historie procházení a souborů cookie?"
    },
    "summaryClearCookiesDuration": {
      "title": "Vymazat <b>{durationDesc}</b> souborů cookie?"
    },
    "summaryClearTabsHistoryAll": {
      "title": "Zavřít <b>{openTabs}</b> {openTabs, plural, =1 {záložku} few {záložky} other {záložek}} a vymazat <b>veškerou</b> historii procházení a všechny soubory cookie ({cookies} {cookies, plural, =1 {stránka} few {stránky} other {stránek}})?"
    },
    "summaryClearTabsAll": {
      "title": "Zavřít <b>{openTabs}</b> {openTabs, plural, =1 {záložku} few {záložky} other {záložek}} a vymazat <b>všechny</b> soubory cookie ({cookies} {cookies, plural, =1 {stránka} few {stránky} other {stránek}})?"
    },
    "summaryClearHistoryAll": {
      "title": "{cookies, plural, one {Vymazat <b>veškerou</b> historii prohlížení a všechny soubory cookie ({cookies} stránka)?} few {Vymazat <b>veškerou</b> historii prohlížení a všechny soubory cookie ({cookies} stránky)?} many {Vymazat <b>veškerou</b> historii prohlížení a všechny soubory cookie ({cookies} stránky)?} other {Vymazat <b>veškerou</b> historii prohlížení a všechny soubory cookie ({cookies} stránek)?}}"
    },
    "summaryClearCookiesAll": {
      "title": "{cookies, plural, one {Vymazat <b>všechny</b> soubory cookie ({cookies} stránka)?} few {Vymazat <b>všechny</b> soubory cookie ({cookies} stránky)?} many {Vymazat <b>všechny</b> soubory cookie ({cookies} stránky)?} other {Vymazat <b>všechny</b> soubory cookie ({cookies} stránek)?}}"
    },
    "summaryClearTabsHistorySite": {
      "title": "{openTabs, plural, one {Zavřít <b>{openTabs} kartu {site}</b> a vymazat <b>všechny soubory cookie ze stránky {site}</b>?} few {Zavřít <b>{openTabs} karty {site}</b> a vymazat <b>všechny soubory cookie ze stránky {site}</b>?} many {Zavřít <b>{openTabs} karty {site}</b> a vymazat <b>všechny soubory cookie ze stránky {site}</b>?} other {Zavřít <b>{openTabs} karet {site}</b> a vymazat <b>všechny soubory cookie ze stránky {site}</b>?}}"
    },
    "summaryClearTabsSite": {
      "title": "{openTabs, plural, one {Zavřít <b>{openTabs} kartu {site}</b> a vymazat <b>všechny soubory cookie ze stránky {site}</b>?} few {Zavřít <b>{openTabs} karty {site}</b> a vymazat <b>všechny soubory cookie ze stránky {site}</b>?} many {Zavřít <b>{openTabs} karty {site}</b> a vymazat <b>všechny soubory cookie ze stránky {site}</b>?} other {Zavřít <b>{openTabs} karet {site}</b> a vymazat <b>všechny soubory cookie ze stránky {site}</b>?}}"
    },
    "summaryClearHistorySite": {
      "title": "Vymazat <b>celou historii prohlížení a soubory cookie ze stránky {site}</b>?"
    },
    "summaryClearCookiesSite": {
      "title": "Vymazat <b>všechny soubory cookie ze stránky {site}</b>?"
    },
    "summaryPinnedIgnored": {
      "title": "{tabs, plural, one {<b>{tabs} připnutá</b> karta bude ignorována.} few {<b>{tabs} připnuté</b> karty budou ignorovány.} many {<b>{tabs} připnuté</b> karty bude ignorováno.} other {<b>{tabs} připnutých</b> karet bude ignorováno.}}"
    },
    "clearData": {
      "title": "Vymazat"
    },
    "cancel": {
      "title": "Zrušit"
    },
    "historyAndDownloadsNotAffected": {
      "title": "Pokud chceš vymazat i historii, vyber časové období."
    }
  },
  "permissions": {
    "camera": {
      "title": "Fotoaparát"
    },
    "microphone": {
      "title": "Mikrofon"
    },
    "geolocation": {
      "title": "Poloha"
    },
    "popups": {
      "title": "Vyskakovací okna"
    },
    "ask": {
      "title": "Vždycky se ptát"
    },
    "notify": {
      "title": "Upozornit"
    },
    "grant": {
      "title": "Vždy povolit"
    },
    "deny": {
      "title": "Vždy zamítnout"
    }
  },
  "report": {
    "selectTheOptionDesc": {
      "title": "Anonymní nahlášení nefunkčního webu nám pomáhá aplikaci vylepšovat."
    },
    "selectTheOptionDescV2": {
      "title": "Vyber možnost, která nejlíp popisuje tvůj problém."
    },
    "selectTheCategoryType": {
      "title": "V čem je problém?"
    },
    "categoryType1": {
      "title": "Stránka nefunguje podle očekávání"
    },
    "categoryType2": {
      "title": "Obsah téhle stránky se mi nelíbí"
    },
    "categoryType3": {
      "title": "Obecná zpětná vazba k prohlížeči DuckDuckGo"
    },
    "selectTheCategory": {
      "title": "Co na téhle stránce nefunguje?"
    },
    "tryTurningProtectionsOff": {
      "title": "Zkus vypnout ochranné funkce a uvidíš, jestli se tím problém vyřeší."
    },
    "skipThisStep": {
      "title": "Přeskočit tento krok"
    },
    "pickYourIssueFromTheList": {
      "title": "Popište, co se stalo"
    },
    "blocked": {
      "title": "Stránka je zablokovaná nebo se nenačetla"
    },
    "layout": {
      "title": "Stránka má rozbité rozvržení"
    },
    "emptySpaces": {
      "title": "Na stránce jsou velké prázdné oblasti"
    },
    "videos": {
      "title": "Video se nenačetlo nebo nepřehrálo"
    },
    "images": {
      "title": "Obrázky nebyly načteny"
    },
    "comments": {
      "title": "Komentáře, recenze nebo chaty se nenačetly"
    },
    "content": {
      "title": "Chybí obsah"
    },
    "links": {
      "title": "Odkazy nebo tlačítka nefungují"
    },
    "login": {
      "title": "Nemůžu se přihlásit/zaregistrovat"
    },
    "loginV2": {
      "title": "Nemůžu se přihlásit ani zaregistrovat"
    },
    "shopping": {
      "title": "Nemůžu zaplatit nebo dokončit nákup"
    },
    "browser": {
      "title": "Problém s prohlížečem nebo rozšířením prohlížeče"
    },
    "paywall": {
      "title": "Stránka mě požádala, ať přestanu blokovat reklamy"
    },
    "other": {
      "title": "Něco jiného"
    },
    "tellUsMoreDesc": {
      "title": "Poděl se o další informace (volitelné):{bullet}Co se stalo?{bullet}Co se mělo stát?{bullet}Pomohlo ti vypnout ochranu?"
    },
    "sendReport": {
      "title": "Poslat zprávu"
    },
    "reportsAreAnonymousDesc": {
      "title": "Hlášení, které pošleš DuckDuckGo, obsahuje jenom informace, které nám pomůžou vyřešit tvou zpětnou vazbu."
    },
    "thankYou": {
      "title": "Děkujeme vám!"
    },
    "yourReportWillHelpDesc": {
      "title": "Tvoje hlášení nám pomůže vylepšovat naše služby a jejich funkčnost pro ostatní uživatele."
    },
    "dislike": {
      "title": "Nelíbí se mi obsah"
    },
    "otherRequired": {
      "title": "Popiš prosím problém, který se objevil (povinné)"
    },
    "otherOptional": {
      "title": "Popiš prosím problém, který se objevil (volitelné)"
    },
    "suggestionWhatHappened": {
      "title": "Co se stalo?"
    },
    "suggestionWhatHappened2": {
      "title": "Co se mělo stát?"
    },
    "suggestionWhatHappened3": {
      "title": "Pomohlo vypnutí ochranných funkcí?"
    },
    "reportTitle": {
      "title": "Nahlášení chyby DuckDuckGo"
    }
  },
  "shared": {
    "trackers": {
      "title": "Trackery"
    }
  },
  "site": {
    "updatingProtectionList": {
      "title": "Aktualizace seznamu ochranných prvků"
    },
    "protectionsEnabled": {
      "title": "Na tomhle webu je ochrana <b>ZAPNUTÁ</b>"
    },
    "protectionsDisabled": {
      "title": "Na tomhle webu je ochrana <b>VYPNUTÁ</b>"
    },
    "protectionsDisabledRemote": {
      "title": "Dočasně jsme vypnuli ochranu osobních údajů, protože se zdá, že při načítání stránky způsobuje chyby."
    },
    "protectionsDisabledRemoteOverride": {
      "title": "Doporučujeme u tohoto webu vypnout ochranu soukromí, aby při načítání stránky nezpůsobovala chyby."
    },
    "connectionDescriptionUnencrypted": {
      "title": "<b>Tahle stránka není zabezpečená</b> a všechny informace, které na ní zadáš, tak mohou být ohrožené."
    },
    "connectionDescriptionInvalidCertificate": {
      "title": "<b>Tenhle web nemá platný certifikát.</b> Možná se dokonce připojuješ k serveru, který předstírá, že je <b>{domain}</b>. Tvoje důvěrné informace by mohly být ohrožené."
    },
    "trackerNetworksSummaryBlocked": {
      "title": "Následující požadavky na domény třetích stran byly zablokovány, protože byly identifikovány jako požadavky se sledováním. Pokud se načtou požadavky služby, může jim to umožnit vytvořit si tvůj profil."
    },
    "trackerNetworksSummaryNoneBlocked": {
      "title": "Žádné požadavky se sledováním nebyly na téhle stránce zablokované. Pokud se načtou požadavky služby načtou, mohou mít možnost vytvořit si tvůj profil."
    },
    "trackerNetworksSummaryNoneFound": {
      "title": "Na téhle stránce jsme nezjistili žádné požadavky se sledováním."
    },
    "trackerNetworksSummaryNone": {
      "title": "Na téhle stránce jsme nenašli žádné služby, které by se snažily načíst požadavky se sledováním."
    },
    "trackerNetworksSummaryAllowedOnly": {
      "title": "Abychom zabránili chybám při načítání webu, nezablokovali jsme na téhle stránce žádné požadavky se sledováním."
    },
    "trackerNetworksSummaryProtectionsOff": {
      "title": "Žádné požadavky se sledováním nebyly zablokovány, protože ochrana je pro tento web vypnutá. Pokud se načtou požadavky služby, může jim to umožnit vytvořit si tvůj profil."
    },
    "createNewDuckAddress": {
      "title": "Vygenerovat soukromou adresu Duck"
    },
    "createNewDuckAddressCopied": {
      "title": "Zkopírováno do schránky!"
    },
    "websiteNotWorkingQ": {
      "title": "Nefunguje web podle očekávání?"
    },
    "websiteNotWorkingPrompt": {
      "title": "Webová stránka nefunguje?"
    },
    "websiteNotWorkingCta": {
      "title": "Nahlásit nefunkční web"
    },
    "websiteNotWorkingAdvice": {
      "title": "Možná zabere vypnutí ochrany."
    },
    "websiteNotWorkingPromptFollowUp": {
      "title": "Stránka pořád nefunguje?"
    },
    "takePrecautions": {
      "title": "Tady to chce opatrnost"
    },
    "majorTrackingNetworkDesc": {
      "title": "{blocked, select, true {<b>Tuhle stránku vlastní společnost {companyDisplayName}</b>, která provozuje síť trackerů na {companyPrevalence} % nejnavštěvovanějších webových stránek. Některé z jejich požadavků se nám na téhle stránce povedlo zablokovat.} other {<b>Tuhle stránku vlastní společnost {companyDisplayName}</b>, která provozuje síť trackerů na {companyPrevalence} % nejnavštěvovanějších webových stránek. }}"
    },
    "trackersBlockedDesc": {
      "title": "{companyCount, plural, =0 {Některým společnostem jsme zablokovali načítání požadavků na sledování na této stránce.} =2 {Na této stránce jsme zablokovali požadavky se sledováním od služby <b>{firstCompany}</b> a <b>{secondCompany}</b>.} =3 {Na této stránce jsme zablokovali požadavky se sledováním od služby <b>{firstCompany}</b>, <b>{secondCompany}</b> a <b>{thirdCompany}</b>.} =4 {Na této stránce jsme zablokovali požadavky se sledováním od služby <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b> a <b>{fourthCompany}</b>.} =5 {Na této stránce jsme zablokovali požadavky se sledováním od služby <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b>, <b>{fourthCompany}</b> a <b>1 další</b>.} one {Na téhle stránce jsme zablokovali požadavky se sledováním od služby <b>{firstCompany}</b>.} few {Na téhle stránce jsme zablokovali požadavky se sledováním od služby <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b>, <b>{fourthCompany}</b> a dalších (<b>{othersCount}</b>).} many {Na téhle stránce jsme zablokovali požadavky se sledováním od služby <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b>, <b>{fourthCompany}</b> a dalších (<b>{othersCount}</b>).} other {Na téhle stránce jsme zablokovali požadavky se sledováním od služby <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b>, <b>{fourthCompany}</b> a dalších (<b>{othersCount}</b>).}}"
    },
    "cookiesMinimized": {
      "title": "O cookies je postaráno!"
    },
    "cookiesHidden": {
      "title": "Skryté vyskakovací okno pro souhlas s cookies"
    },
    "cookiesHiddenSummary": {
      "title": "Na tomto webu se nám povedlo vyskakovací okno ohledně cookies jen skrýt, protože neobsahovalo žádné možnosti pro nastavení předvoleb souborů cookie. Naše další ochranné prvky proti sledování na webu ale běží dál."
    },
    "cookiesMinimizedSummary": {
      "title": "Tvoje předvolby pro cookies jsme nastavili na co nejvyšší ochranu soukromí a vyskakovací okno jsme zavřeli."
    },
    "cookiesMinimizedSettings": {
      "title": "Zakázat v nastavení"
    },
    "connectionSecure": {
      "title": "Připojení je šifrované"
    },
    "connectionNotSecure": {
      "title": "Připojení není šifrované"
    },
    "connectionNotSecureInvalidCertificate": {
      "title": "Připojení nemusí být zabezpečené"
    },
    "trackerNetworksDesc": {
      "title": "Načítání požadavků bylo zablokováno"
    },
    "trackerNetworksNotBlocked": {
      "title": "Nejsou blokovány žádné požadavky se sledováním"
    },
    "trackerNetworksNotFound": {
      "title": "Nebyly nalezeny žádné požadavky se sledováním"
    },
    "thirdPartiesLoaded": {
      "title": "Načteny požadavky třetích stran"
    },
    "thirdPartiesNoneFound": {
      "title": "Nenašli jsme žádné požadavky třetích stran"
    },
    "firstPartyDesc": {
      "title": "{companyName} vlastní tenhle web a trackery, které jsme na něm našli, takže jsme je nezablokovali."
    },
    "noTrackersFound": {
      "title": "Na téhle stránce jsme nenašli žádné služby, které by se snažily načíst požadavky se sledováním."
    },
    "trackersFoundForAllowlisted": {
      "title": "Trackery umožňují různým službám, aby si vytvořily tvůj profil. Na téhle stránce sledují tvou aktivitu tyhle služby."
    },
    "trackersFoundAndBlocked": {
      "title": "Trackery umožňují různým službám, aby si vytvořily tvůj profil. Na téhle stránce jsme zablokovali tyhle služby, které sledují tvou aktivitu."
    },
    "trackerNetworkUnknown": {
      "title": "Neznámá sledovací síť"
    },
    "trackerDomainsForCompany": {
      "title": "Domény trackerů služby {companyName}"
    },
    "zeroTrackersFound": {
      "title": "Žádné trackery nenalezeny"
    },
    "trackerOwnedByThisSite": {
      "title": "Trackery vlastněné tímhle webem (povolené)"
    },
    "trackerCountForDomain": {
      "title": "{blocked, select, true {{trackerCount, plural, one {{trackerCount} zablokovaný tracker v doméně {domain}} few {{trackerCount} zablokované trackery v doméně {domain}} many {{trackerCount} zablokovaného trackeru v doméně {domain}} other {{trackerCount} zablokovaných trackerů v doméně {domain}}}} other {{trackerCount, plural, one {V doméně {domain} jsme našli {trackerCount} tracker} few {V doméně {domain} jsme našli {trackerCount} trackery} many {V doméně {domain} jsme našli {trackerCount} trackeru} other {V doméně {domain} jsme našli {trackerCount} trackerů}}}}"
    },
    "trackerLimitationsNote": {
      "title": "Upozornění: omezení platformy mohou omezit naši schopnost detekovat všechny požadavky."
    },
    "trackerAboutLink": {
      "title": "O našich mechanismech na ochranu proti sledování na webu"
    },
    "trackerAdLink": {
      "title": "Jak naše reklamy ve vyhledávání ovlivňují naši ochranu"
    },
    "sectionHeadingAdAttribution": {
      "title": "Kvůli nedávnému kliknutí na reklamu {domain} na DuckDuckGo se načetly následující požadavky domény. Tyhle požadavky pomáhají vyhodnocovat účinnost reklam. Žádná reklama na DuckDuckGo nevytváří tvůj profil."
    },
    "sectionHeadingIgnore": {
      "title": "Následující požadavky domén se načetly, aby se zabránilo narušení funkčnosti webové stránky."
    },
    "sectionHeadingFirstParty": {
      "title": "Následující požadavky domén se načetly, protože jsou spojené se službou {domain}."
    },
    "sectionHeadingThirdParty": {
      "title": "Načetly se taky následující požadavky domén."
    },
    "sectionHeadingProtectionsDisabled": {
      "title": "Následující požadavky domén se načetly, protože ochrana je vypnutá."
    },
    "thirdPartiesSummaryLoaded": {
      "title": "Byly načteny následující požadavky domén třetích stran. Pokud se načtou požadavky společnosti, může jim to umožnit vytvořit si tvůj profil, i když naše ostatní mechanismy na ochranu proti sledování na webu jsou stále aktivní."
    },
    "thirdPartiesSummaryProtectionsOff": {
      "title": "Byly načteny následující požadavky domén třetích stran. Pokud se načtou požadavky společnosti, může jim to umožnit vytvořit si tvůj profil, i když naše ostatní mechanismy na ochranu proti sledování na webu jsou stále aktivní."
    },
    "thirdPartiesSummaryNone": {
      "title": "Neidentifikovali jsme žádné požadavky z domén třetích stran."
    },
    "analyticsCategory": {
      "title": "Analytika"
    },
    "advertisingCategory": {
      "title": "Reklama"
    },
    "socialCategory": {
      "title": "Sociální síť"
    },
    "contentDeliveryCategory": {
      "title": "Doručení obsahu"
    },
    "embeddedContentCategory": {
      "title": "Vložený obsah"
    },
    "searchPlaceholder": {
      "title": "Vyhledat prostřednictvím DuckDuckGo"
    },
    "searchGoButton": {
      "title": "Hledat"
    },
    "optionsButton": {
      "title": "Další možnosti"
    },
    "navigationComplete": {
      "title": "Hotovo"
    },
    "navigationClose": {
      "title": "Zavřít"
    },
    "navigationCancel": {
      "title": "Zrušit"
    },
    "navigationBack": {
      "title": "Zpět"
    },
    "enableProtectionsSwitch": {
      "title": "Zapnout ochranné prvky"
    },
    "disableProtectionsSwitch": {
      "title": "Vypnout ochranné prvky"
    },
    "errorMessage": {
      "title": "Něco se pokazilo a nepodařilo se nám tento obsah načíst. Zkus stránku načíst znovu."
    }
  }
}