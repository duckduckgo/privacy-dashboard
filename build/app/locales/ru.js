export default {
  "connection": {
    "encrypt": {
      "title": "Шифрование"
    },
    "sign": {
      "title": "Подпись"
    },
    "verify": {
      "title": "Подтверждение"
    },
    "derive": {
      "title": "Производные"
    },
    "wrap": {
      "title": "Сворачивание"
    },
    "unwrap": {
      "title": "Развертка"
    },
    "unknown": {
      "title": "Неизвестно"
    },
    "certificateDetail": {
      "title": "О сертификате безопасности"
    },
    "commonName": {
      "title": "Общепринятое название"
    },
    "publicKey": {
      "title": "Открытый ключ"
    },
    "algorithm": {
      "title": "Алгоритм"
    },
    "keySize": {
      "title": "Размер ключа"
    },
    "usage": {
      "title": "Применение"
    },
    "summary": {
      "title": "Сводка"
    },
    "permanent": {
      "title": "Постоянный"
    },
    "effectiveSize": {
      "title": "Фактический размер"
    },
    "certificateNotFound": {
      "title": "Сертификат не найден"
    },
    "certificateForDomain": {
      "title": "Сертификат {domain}"
    },
    "insecureConnectionDesc": {
      "title": "На этой странице используется нешифрованное соединение. Сторонние лица могут просматривать ваши действия и перехватывать конфиденциальную информацию, отправляемую на этой странице."
    },
    "invalidConnectionDesc": {
      "title": "Сертификат этого сайта недействителен. Возможно, вы пытаетесь подключиться к серверу, который выдает себя за <b>{domain}</b>, что ставит под угрозу вашу конфиденциальную информацию."
    },
    "upgradedConnectionDesc": {
      "title": "Чтобы защитить передаваемую вами информацию, мы повысили безопасность соединения на этой странице."
    },
    "secureConnectionDesc": {
      "title": "На этой странице используется зашифрованное соединение, которое не позволяет сторонним лицам просматривать ваши действия и перехватывать конфиденциальную информацию."
    }
  },
  "ctascreens": {
    "protectionsUnavailableNote": {
      "title": "Защита конфиденциальности не работает на специальных и локальных страницах."
    },
    "spreadTitle": {
      "title": "Нравится, как работает DuckDuckGo?"
    },
    "spreadText": {
      "title": "Расскажите о нем родственникам и друзьям"
    },
    "spreadButton": {
      "title": "Распространяйте DuckDuckGo"
    },
    "emailTitle": {
      "title": "Надоело, что отслеживают ваши письма?"
    },
    "emailText": {
      "title": "Подпишитесь на услугу защиты электронной почты DuckDuckGo, чтобы воспользоваться расширением!"
    }
  },
  "firebutton": {
    "fireDialogHeader": {
      "title": "Закрыть вкладки и удалить данные"
    },
    "fireDialogHeaderNoTabs": {
      "title": "Сбросить данные"
    },
    "optionCurrentSite": {
      "title": "Только с текущего сайта"
    },
    "optionLastHour": {
      "title": "За последний час"
    },
    "optionLast24Hour": {
      "title": "За последние 24 часа"
    },
    "optionLast7days": {
      "title": "За последние 7 дней"
    },
    "optionLast4Weeks": {
      "title": "За последние 4 недели"
    },
    "optionAllTime": {
      "title": "За всё время"
    },
    "historyDuration": {
      "title": "{duration, select, hour {за последний час} day {за последние 24 часа} week {за последнюю неделю} month {за последние 4 недели} other {Все}}"
    },
    "summaryClearTabsHistoryDuration": {
      "title": "{openTabs, plural, one {Закрыть <b>{openTabs}</b> вкладку и удалить историю просмотров и куки-файлы <b>{durationDesc}</b>?} few {Закрыть <b>{openTabs}</b> вкладки и удалить историю просмотров и куки-файлы <b>{durationDesc}</b>?} many {Закрыть <b>{openTabs}</b> вкладок и удалить историю просмотров и куки-файлы <b>{durationDesc}</b>?} other {Закрыть вкладки (<b>{openTabs}</b>) и удалить историю просмотров и куки-файлы <b>{durationDesc}</b>?}}"
    },
    "summaryClearTabsDuration": {
      "title": "{openTabs, plural, one {Закрыть <b>{openTabs}</b> вкладку и удалить куки-файлы <b>{durationDesc}</b>?} few {Закрыть <b>{openTabs}</b> вкладки и удалить куки-файлы <b>{durationDesc}</b>?} many {Закрыть <b>{openTabs}</b> вкладок и удалить куки-файлы <b>{durationDesc}</b>?} other {Закрыть вкладки (<b>{openTabs}</b>) и удалить куки-файлы <b>{durationDesc}</b>?}}"
    },
    "summaryClearHistoryDuration": {
      "title": "Удалить историю просмотров и куки-файлы <b>{durationDesc}</b>?"
    },
    "summaryClearCookiesDuration": {
      "title": "Удалить куки-файлы <b>{durationDesc}</b>?"
    },
    "summaryClearTabsHistoryAll": {
      "title": "Закрыть <b>{openTabs}</b> {openTabs, plural, =1 {вкладку} other {вклад.}} и удалить <b>всю</b> историю просмотров и куки-файлы (с {cookies} {cookies, plural, =1 {сайта} other {сайтов}})?"
    },
    "summaryClearTabsAll": {
      "title": "Закрыть <b>{openTabs}</b> {openTabs, plural, =1 {вкладку} other {вклад.}} и удалить <b>все</b> куки-файлы (с {cookies} {cookies, plural, =1 {сайта} other {сайтов}})?"
    },
    "summaryClearHistoryAll": {
      "title": "{cookies, plural, one {Удалить <b>всю</b> историю просмотров и куки-файлы (с {cookies} сайта)?} few {Удалить <b>всю</b> историю просмотров и куки-файлы (с {cookies} сайтов)?} many {Удалить <b>всю</b> историю просмотров и куки-файлы (с {cookies} сайтов)?} other {Удалить <b>всю</b> историю просмотров и куки-файлы (с {cookies} сайтов)?}}"
    },
    "summaryClearCookiesAll": {
      "title": "{cookies, plural, one {Удалить <b>все</b> куки-файлы (с {cookies} сайта)?} few {Удалить <b>все</b> куки-файлы (с {cookies} сайтов)?} many {Удалить <b>все</b> куки-файлы (с {cookies} сайтов)?} other {Удалить <b>все</b> куки-файлы (с {cookies} сайтов)?}}"
    },
    "summaryClearTabsHistorySite": {
      "title": "{openTabs, plural, one {Закрыть <b>{openTabs} вкладку {site}</b> и удалить <b>все куки-файлы с {site}</b>?} few {Закрыть <b>{openTabs} вкладки {site}</b> и удалить <b>все куки-файлы с {site}</b>?} many {Закрыть <b>{openTabs} вкладок {site}</b> и удалить <b>все куки-файлы с {site}</b>?} other {Закрыть <b>вкладки {site} ({openTabs})</b> и удалить <b>все куки-файлы с {site}</b>?}}"
    },
    "summaryClearTabsSite": {
      "title": "{openTabs, plural, one {Закрыть <b>{openTabs} вкладку {site}</b> и удалить <b>все куки-файлы с {site}</b>?} few {Закрыть <b>{openTabs} вкладки {site}</b> и удалить <b>все куки-файлы с {site}</b>?} many {Закрыть <b>{openTabs} вкладок {site}</b> и удалить <b>все куки-файлы с {site}</b>?} other {Закрыть <b>вкладки {site} ({openTabs})</b> и удалить <b>все куки-файлы с {site}</b>?}}"
    },
    "summaryClearHistorySite": {
      "title": "Удалить <b>всю</b> историю просмотров и куки-файлы с {site}?"
    },
    "summaryClearCookiesSite": {
      "title": "Удалить <b>все</b> куки-файлы с {site}?"
    },
    "summaryPinnedIgnored": {
      "title": "{tabs, plural, one {<b>{tabs} закрепленная</b> вкладка будет проигнорирована.} few {<b>{tabs} закрепленные</b> вкладки будут проигнорированы.} many {<b>{tabs} закрепленных</b> вкладок будет проигнорировано.} other {Закрепленные вкладки (<b>{tabs}</b>) будут проигнорированы.}}"
    },
    "clearData": {
      "title": "Очистить"
    },
    "cancel": {
      "title": "Отменить"
    },
    "historyAndDownloadsNotAffected": {
      "title": "Если вы также хотите очистить историю, выберите временной отрезок."
    }
  },
  "permissions": {
    "camera": {
      "title": "Камера"
    },
    "microphone": {
      "title": "Микрофон"
    },
    "geolocation": {
      "title": "Геолокация"
    },
    "popups": {
      "title": "Всплывающие окна"
    },
    "ask": {
      "title": "Спрашивать каждый раз"
    },
    "notify": {
      "title": "Спрашивать разрешение"
    },
    "grant": {
      "title": "Разрешать всегда"
    },
    "deny": {
      "title": "Не разрешать"
    }
  },
  "report": {
    "selectTheOptionDesc": {
      "title": "Ваша анонимная жалоба на проблемы с сайтом поможет нам улучшить приложение."
    },
    "selectTheOptionDescV2": {
      "title": "Выберите наиболее точное описание проблемы, с которой вы столкнулись."
    },
    "selectTheCategoryType": {
      "title": "В чем проблема?"
    },
    "categoryType1": {
      "title": "Сайт не работает должным образом"
    },
    "categoryType2": {
      "title": "Не нравится контент на сайте"
    },
    "categoryType3": {
      "title": "Общий отзыв о браузере DuckDuckGo"
    },
    "selectTheCategory": {
      "title": "Что не так с сайтом?"
    },
    "tryTurningProtectionsOff": {
      "title": "Попробуйте отключить защиту конфиденциальности. Возможно, это устранит проблему."
    },
    "skipThisStep": {
      "title": "Пропустить этот шаг"
    },
    "pickYourIssueFromTheList": {
      "title": "Опишите, что случилось"
    },
    "blocked": {
      "title": "Сайт блокируется или не грузится"
    },
    "layout": {
      "title": "Нарушена структура сайта"
    },
    "emptySpaces": {
      "title": "Крупные пустые пространства"
    },
    "videos": {
      "title": "Не грузится или не играет видео"
    },
    "images": {
      "title": "Не загружаются изображения"
    },
    "comments": {
      "title": "Не грузятся комментарии, отзывы или чат"
    },
    "content": {
      "title": "Отсутствует содержимое"
    },
    "links": {
      "title": "Не работают ссылки или кнопки"
    },
    "login": {
      "title": "Невозможно войти/зарегистрироваться"
    },
    "loginV2": {
      "title": "Невозможно войти или зарегистрироваться"
    },
    "shopping": {
      "title": "Не получается провести оплату или оформить заказ/покупку"
    },
    "browser": {
      "title": "Проблема с браузером или другим расширением"
    },
    "paywall": {
      "title": "Сайт просит отключить блокировку рекламы"
    },
    "other": {
      "title": "Что-то другое"
    },
    "tellUsMoreDesc": {
      "title": "Предоставьте дополнительную информацию (необязательно).{bullet}Что случилось?{bullet}Что должно было произойти?{bullet}Помогло ли отключение защиты?"
    },
    "sendReport": {
      "title": "Отправить жалобу"
    },
    "reportsAreAnonymousDesc": {
      "title": "Сообщения, отправленные в DuckDuckGo, содержат только информацию, необходимую для обработки вашего отзыва."
    },
    "thankYou": {
      "title": "Спасибо!"
    },
    "yourReportWillHelpDesc": {
      "title": "Ваше сообщение будет способствовать улучшению ПО и повышению удобства пользования."
    },
    "dislike": {
      "title": "Не нравится контент"
    },
    "otherRequired": {
      "title": "Опишите возникшую проблему (обязательно)"
    },
    "otherOptional": {
      "title": "Опишите возникшую проблему (необязательно)"
    },
    "suggestionWhatHappened": {
      "title": "Что случилось?"
    },
    "suggestionWhatHappened2": {
      "title": "Что должно было произойти?"
    },
    "suggestionWhatHappened3": {
      "title": "Помогло ли отключение защиты?"
    },
    "reportTitle": {
      "title": "Отчет в DuckDuckGo"
    }
  },
  "shared": {
    "trackers": {
      "title": "Трекеры"
    }
  },
  "site": {
    "updatingProtectionList": {
      "title": "Список защиты обновляется"
    },
    "protectionsEnabled": {
      "title": "Функции защиты на этом сайте <b>ВКЛЮЧЕНЫ</b>"
    },
    "protectionsDisabled": {
      "title": "Функции защиты на этом сайте <b>ВЫКЛЮЧЕНЫ</b>"
    },
    "protectionsDisabledRemote": {
      "title": "Мы временно отключили защиту конфиденциальности, так как она нарушала работу этого сайта."
    },
    "protectionsDisabledRemoteOverride": {
      "title": "Рекомендуем отключить защиту конфиденциальности на этом сайте, чтобы не нарушать его работу."
    },
    "connectionDescriptionUnencrypted": {
      "title": "<b>Сайт небезопасен</b> и не обеспечивает защиту информации, передаваемой на этой странице."
    },
    "connectionDescriptionInvalidCertificate": {
      "title": "<b>Сертификат этого сайта недействителен.</b> Возможно, вы пытаетесь подключиться к серверу, который выдает себя за <b>{domain}</b>, что ставит под угрозу вашу конфиденциальную информацию."
    },
    "trackerNetworksSummaryBlocked": {
      "title": "Мы заблокировали загрузку следующих запросов сторонних доменов, так как распознали в них попытки отслеживания. Загруженные запросы позволяют компаниям вас профилировать."
    },
    "trackerNetworksSummaryNoneBlocked": {
      "title": "Мы не заблокировали ни одного запроса отслеживания на этой странице. В случае загрузки их запросов, компании имеют возможность создавать ваш профиль."
    },
    "trackerNetworksSummaryNoneFound": {
      "title": "Мы не обнаружили на этой странице запросов на отслеживание."
    },
    "trackerNetworksSummaryNone": {
      "title": "На этом сайте не обнаружено компаний, пытавшихся загрузить запросы на отслеживание."
    },
    "trackerNetworksSummaryAllowedOnly": {
      "title": "Чтобы не нарушить работу сайта, мы не стали блокировать на этой странице запросы компаний на отслеживание."
    },
    "trackerNetworksSummaryProtectionsOff": {
      "title": "Мы не заблокировали ни одного запроса на отслеживание, так как защита на этом сайте отключена. Загруженные запросы позволяют компаниям вас профилировать."
    },
    "createNewDuckAddress": {
      "title": "Создать адрес на Duck"
    },
    "createNewDuckAddressCopied": {
      "title": "Скопировано в буфер!"
    },
    "websiteNotWorkingQ": {
      "title": "Сайт плохо работает?"
    },
    "websiteNotWorkingPrompt": {
      "title": "Сайт не работает?"
    },
    "websiteNotWorkingCta": {
      "title": "Сообщить о неработающем сайте"
    },
    "websiteNotWorkingAdvice": {
      "title": "Попробуйте отключить защиту."
    },
    "websiteNotWorkingPromptFollowUp": {
      "title": "Всё равно не работает?"
    },
    "takePrecautions": {
      "title": "Будьте осторожны!"
    },
    "majorTrackingNetworkDesc": {
      "title": "{blocked, select, true {<b>Этот сайт принадлежит компании {companyDisplayName}</b>, которая управляет сетью трекеров на {companyPrevalence}% ведущих сайтов. Нам не удалось заблокировать на этой странице несколько запросов на отслеживание.} other {<b>Этот сайт принадлежит компании {companyDisplayName}</b>, которая управляет сетью трекеров на {companyPrevalence}% ведущих сайтов. }}"
    },
    "trackersBlockedDesc": {
      "title": "{companyCount, plural, =0 {Мы остановили загрузку трекинг-запросов некоторых компаний на этой странице.} =2 {Мы заблокировали на этой странице запросы на отслеживание <b>{firstCompany}</b> и <b>{secondCompany}</b>.} =3 {Мы заблокировали на этой странице запросы на отслеживание <b>{firstCompany}</b>, <b>{secondCompany}</b> и <b>{thirdCompany}</b>.} =4 {Мы заблокировали на этой странице запросы на отслеживание <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b> и <b>{fourthCompany}</b>.} =5 {Мы заблокировали на этой странице запросы на отслеживание <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b>, <b>{fourthCompany}</b> и <b>еще одной</b> компании.} one {Мы предотвратили загрузку запроса на отслеживание от <b>{firstCompany}</b> на этом сайте.} few {Мы предотвратили загрузку запросов на отслеживание, поступивших от <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b> и <b>{fourthCompany}</b> и еще <b>{othersCount}</b> других на этом сайте.} many {Мы предотвратили загрузку запросов на отслеживание, поступивших от <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b>, <b>{fourthCompany}</b> и <b>еще {othersCount}</b> других на этом сайте.} other {Мы предотвратили загрузку запросов на отслеживание, поступивших от <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b>, <b>{fourthCompany}</b> и <b>еще {othersCount}</b> других на этом сайте.}}"
    },
    "cookiesMinimized": {
      "title": "Kуки-файлы выбраны"
    },
    "cookiesHidden": {
      "title": "Всплывающее окно куки-файлов скрыто"
    },
    "cookiesHiddenSummary": {
      "title": "На этом сайте не предусмотрена настройка куки-файлов, поэтому мы просто скрыли их всплывающее окно. Остальные средства защиты от отслеживания онлайн по-прежнему активны."
    },
    "cookiesMinimizedSummary": {
      "title": "Мы выбрали настройки куки-файлов, обеспечивающие максимальную защиту ваших данных, и закрыли всплывающее окно."
    },
    "cookiesMinimizedSettings": {
      "title": "Отключить в настройках"
    },
    "connectionSecure": {
      "title": "Соединение зашифровано"
    },
    "connectionNotSecure": {
      "title": "Соединение не зашифровано"
    },
    "connectionNotSecureInvalidCertificate": {
      "title": "Риск небезопасного соединения"
    },
    "trackerNetworksDesc": {
      "title": "Заблокирована загрузка запросов"
    },
    "trackerNetworksNotBlocked": {
      "title": "Запросы на отслеживание не блокировались"
    },
    "trackerNetworksNotFound": {
      "title": "Запросы на отслеживание не обнаружены"
    },
    "thirdPartiesLoaded": {
      "title": "Загружены сторонние запросы"
    },
    "thirdPartiesNoneFound": {
      "title": "Не найдено сторонних запросов"
    },
    "firstPartyDesc": {
      "title": "Этот сайт и найденные на нем трекеры принадлежат компании {companyName}, поэтому мы их не блокировали."
    },
    "noTrackersFound": {
      "title": "На этом сайте не обнаружено компаний, пытавшихся загрузить запросы на отслеживание."
    },
    "trackersFoundForAllowlisted": {
      "title": "Трекеры применяются для профилирования посетителей сайтов. Перечисленные здесь компании отслеживают ваши действия на этой странице."
    },
    "trackersFoundAndBlocked": {
      "title": "Трекеры применяются для профилирования посетителей сайтов. Мы заблокировали компании, которые пытались отслеживать ваши действия на этой странице."
    },
    "trackerNetworkUnknown": {
      "title": "Сеть трекеров неизвестна"
    },
    "trackerDomainsForCompany": {
      "title": "Трекерные домены {companyName}"
    },
    "zeroTrackersFound": {
      "title": "Трекеры не обнаружены"
    },
    "trackerOwnedByThisSite": {
      "title": "Трекеры, принадлежащие этому сайту (разрешены)"
    },
    "trackerCountForDomain": {
      "title": "{blocked, select, true {{trackerCount, plural, one {На {domain} заблокирован {trackerCount} трекер} few {На {domain} заблокировано {trackerCount} трекера} many {На {domain} заблокировано {trackerCount} трекеров} other {Заблокировано трекеров на сайте {domain}: {trackerCount}}}} other {{trackerCount, plural, one {На сайте {domain} обнаружен {trackerCount} трекер} few {На сайте {domain} обнаружено {trackerCount} трекера} many {На сайте {domain} обнаружено {trackerCount} трекеров} other {Обнаружено трекеров на сайте {domain}: {trackerCount}}}}}"
    },
    "trackerLimitationsNote": {
      "title": "Внимание: наши возможности обнаружения запросов зависят от ограничений платформы."
    },
    "trackerAboutLink": {
      "title": "О нашей защите от отслеживания онлайн"
    },
    "trackerAdLink": {
      "title": "Как реклама при поиске влияет на защиту"
    },
    "sectionHeadingAdAttribution": {
      "title": "Запросы этого домена были загружены в результате недавнего нажатия на рекламу {domain} в DuckDuckGo. Такие запросы помогают оценить эффективность рекламы. Рекламные объявления в DuckDuckGo не профилируют пользователей."
    },
    "sectionHeadingIgnore": {
      "title": "Мы загрузили запросы этих доменов, чтобы не нарушить работу сайта."
    },
    "sectionHeadingFirstParty": {
      "title": "Мы загрузили запросы этих доменов, так как они связаны с {domain}."
    },
    "sectionHeadingThirdParty": {
      "title": "Также загружены запросы следующих доменов."
    },
    "sectionHeadingProtectionsDisabled": {
      "title": "Мы загрузили запросы этих доменов, так как у вас отключена защита."
    },
    "thirdPartiesSummaryLoaded": {
      "title": "Ниже приведен список загруженных запросов сторонних доменов. Благодаря этим запросам компании могут вас профилировать, однако мы по-прежнему защищаем вас от отслеживания в сети другими способами."
    },
    "thirdPartiesSummaryProtectionsOff": {
      "title": "Ниже приведен список загруженных запросов сторонних доменов. Благодаря этим запросам компании могут вас профилировать, однако мы по-прежнему защищаем вас от отслеживания в сети другими способами."
    },
    "thirdPartiesSummaryNone": {
      "title": "Запросов от сторонних доменов не обнаружено."
    },
    "analyticsCategory": {
      "title": "Аналитика"
    },
    "advertisingCategory": {
      "title": "Реклама"
    },
    "socialCategory": {
      "title": "Социальная сеть"
    },
    "contentDeliveryCategory": {
      "title": "Доставка контента"
    },
    "embeddedContentCategory": {
      "title": "Встроенный контент"
    },
    "searchPlaceholder": {
      "title": "Поиск в DuckDuckGo"
    },
    "searchGoButton": {
      "title": "Поиск"
    },
    "optionsButton": {
      "title": "Другие параметры"
    },
    "navigationComplete": {
      "title": "Готово"
    },
    "navigationClose": {
      "title": "Закрыть"
    },
    "navigationCancel": {
      "title": "Отмена"
    },
    "navigationBack": {
      "title": "Назад"
    },
    "enableProtectionsSwitch": {
      "title": "Включить защиту"
    },
    "disableProtectionsSwitch": {
      "title": "Отключить защиту"
    },
    "errorMessage": {
      "title": "Что-то сработало не так: не удалось загрузить этот контент. Перезагрузите страницу."
    }
  }
}