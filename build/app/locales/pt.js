export default {
  "connection": {
    "encrypt": {
      "title": "Encriptar"
    },
    "sign": {
      "title": "Assinar"
    },
    "verify": {
      "title": "Verificar"
    },
    "derive": {
      "title": "Derivar"
    },
    "wrap": {
      "title": "Embrulhar"
    },
    "unwrap": {
      "title": "Desembrulhar"
    },
    "unknown": {
      "title": "Desconhecido"
    },
    "certificateDetail": {
      "title": "Detalhe do Certificado de Segurança"
    },
    "commonName": {
      "title": "Nome comum"
    },
    "publicKey": {
      "title": "Chave pública"
    },
    "algorithm": {
      "title": "Algoritmo"
    },
    "keySize": {
      "title": "Tamanho da chave"
    },
    "usage": {
      "title": "Uso"
    },
    "summary": {
      "title": "Resumo"
    },
    "permanent": {
      "title": "Permanente"
    },
    "effectiveSize": {
      "title": "Tamanho efetivo"
    },
    "certificateNotFound": {
      "title": "Certificado não encontrado"
    },
    "certificateForDomain": {
      "title": "Certificado para {domain}"
    },
    "insecureConnectionDesc": {
      "title": "Esta página está a utilizar uma ligação não encriptada. É possível que terceiros consigam ver a tua atividade ou intercetar informações sensíveis que envies nesta página."
    },
    "invalidConnectionDesc": {
      "title": "O certificado deste site é inválido. Podes estar a ligar a um servidor que finge ser <b>{domain}</b>, o que pode colocar as tuas informações confidenciais em risco."
    },
    "upgradedConnectionDesc": {
      "title": "Melhorámos a ligação nesta página para proteger a informação que envias enquanto estás em trânsito."
    },
    "secureConnectionDesc": {
      "title": "Esta página utiliza uma ligação encriptada, o que impede que terceiros vejam a tua atividade ou intercetem informações sensíveis que envies nesta página."
    }
  },
  "ctascreens": {
    "protectionsUnavailableNote": {
      "title": "As Proteções de Privacidade não estão disponível para páginas especiais nem páginas locais."
    },
    "spreadTitle": {
      "title": "Adoras o DuckDuckGo?"
    },
    "spreadText": {
      "title": "Recomenda-o à tua família e amigos"
    },
    "spreadButton": {
      "title": "Espalhar o DuckDuckGo"
    },
    "emailTitle": {
      "title": "Não queres que os teus e-mails sejam rastreados?"
    },
    "emailText": {
      "title": "Subscreve já a Email Protection do DuckDuckGo para a tua extensão!"
    }
  },
  "firebutton": {
    "fireDialogHeader": {
      "title": "Fechar separadores e limpar os dados"
    },
    "fireDialogHeaderNoTabs": {
      "title": "Limpar dados"
    },
    "optionCurrentSite": {
      "title": "Só no site atual"
    },
    "optionLastHour": {
      "title": "Última hora"
    },
    "optionLast24Hour": {
      "title": "Últimas 24 horas"
    },
    "optionLast7days": {
      "title": "Últimos 7 dias"
    },
    "optionLast4Weeks": {
      "title": "Últimas 4 semanas"
    },
    "optionAllTime": {
      "title": "Sempre"
    },
    "historyDuration": {
      "title": "{duration, select, hour {uma hora} day {24 horas} week {uma semana} month {4 semanas} other {Todos}}"
    },
    "summaryClearTabsHistoryDuration": {
      "title": "{openTabs, plural, one {Fechar <b>{openTabs}</b> separador e limpar <b>{durationDesc}</b> de histórico de navegação e cookies?} other {Fechar <b>{openTabs}</b> separadores e limpar <b>{durationDesc}</b> de histórico de navegação e cookies?}}"
    },
    "summaryClearTabsDuration": {
      "title": "{openTabs, plural, one {Fechar <b>{openTabs}</b> separador e limpar <b>{durationDesc}</b> de cookies?} other {Close <b>{openTabs}</b> separadores e limpar <b>{durationDesc}</b> de cookies?}}"
    },
    "summaryClearHistoryDuration": {
      "title": "Limpar <b>{durationDesc}</b> de histórico de navegação e cookies?"
    },
    "summaryClearCookiesDuration": {
      "title": "Limpar <b>{durationDesc}</b> de cookies?"
    },
    "summaryClearTabsHistoryAll": {
      "title": "Fechar <b>{openTabs}</b> {openTabs, plural, =1 {separador} other {separadores}} e limpar <b>todo</b> o histórico de navegação e cookies ({cookies} {cookies, plural, =1 {site} other {sites}})?"
    },
    "summaryClearTabsAll": {
      "title": "Fechar <b>{openTabs}</b> {openTabs, plural, =1 {separador} other {separadores}} e limpar <b>todos</b> os cookies ({cookies} {cookies, plural, =1 {site} other {sites}})?"
    },
    "summaryClearHistoryAll": {
      "title": "{cookies, plural, one {Limpar <b>todo</b> o histórico de navegação e cookies ({cookies} site)?} other {Limpar <b>todo</b> o histórico de navegação e cookies ({cookies} sites)?}}"
    },
    "summaryClearCookiesAll": {
      "title": "{cookies, plural, one {Limpar <b>todos</b> os cookies ({cookies} site)?} other {Limpar <b>todos</b> os cookies ({cookies} sites)?}}"
    },
    "summaryClearTabsHistorySite": {
      "title": "{openTabs, plural, one {Fechar <b>{openTabs} separador em {site}</b> e limpar <b>todos os cookies de {site}</b>?} other {Fechar <b>{openTabs} separadores em {site}</b> e limpar <b>todos os cookies de {site}</b>?}}"
    },
    "summaryClearTabsSite": {
      "title": "{openTabs, plural, one {Fechar <b>{openTabs} separador em {site}</b> e limpar <b>todos os cookies de {site}</b>?} other {Fechar <b>{openTabs} separadores em {site}</b> e limpar <b>todos os cookies de {site}</b>?}}"
    },
    "summaryClearHistorySite": {
      "title": "Limpar todo o histórico de navegação e cookies de <b>{site}</b>?"
    },
    "summaryClearCookiesSite": {
      "title": "Limpar todos os cookies de <b>{site}</b>?"
    },
    "summaryPinnedIgnored": {
      "title": "{tabs, plural, one {<b>{tabs} separador afixado</b> será ignorado.} other {<b>{tabs} separadores afixados</b> serão ignorados.}}"
    },
    "clearData": {
      "title": "Céu limpo"
    },
    "cancel": {
      "title": "Cancelar"
    },
    "historyAndDownloadsNotAffected": {
      "title": "Para limpar o histórico, selecione um intervalo de tempo."
    }
  },
  "permissions": {
    "camera": {
      "title": "Câmara"
    },
    "microphone": {
      "title": "Microfone"
    },
    "geolocation": {
      "title": "Geolocalização"
    },
    "popups": {
      "title": "Pop-ups"
    },
    "ask": {
      "title": "Pergunte todas as vezes"
    },
    "notify": {
      "title": "Notificar"
    },
    "grant": {
      "title": "Permitir sempre"
    },
    "deny": {
      "title": "Negar sempre"
    }
  },
  "report": {
    "selectTheOptionDesc": {
      "title": "O envio de um relatório anónimo de site com problemas ajuda-nos a melhorar a aplicação."
    },
    "selectTheOptionDescV2": {
      "title": "Seleciona a opção que melhor descreve o problema que enfrentaste."
    },
    "selectTheCategoryType": {
      "title": "Qual é o problema?"
    },
    "categoryType1": {
      "title": "O site não funciona como esperado"
    },
    "categoryType2": {
      "title": "Não gosto do conteúdo deste site"
    },
    "categoryType3": {
      "title": "Feedback geral sobre o navegador DuckDuckGo"
    },
    "selectTheCategory": {
      "title": "O que não funciona neste site?"
    },
    "tryTurningProtectionsOff": {
      "title": "Experimenta desativar as Proteções de Privacidade para ver se resolve o problema."
    },
    "skipThisStep": {
      "title": "Ignorar este passo"
    },
    "pickYourIssueFromTheList": {
      "title": "Descrever o que aconteceu"
    },
    "blocked": {
      "title": "Site bloqueado ou que não foi carregado"
    },
    "layout": {
      "title": "Disposição do site com problemas"
    },
    "emptySpaces": {
      "title": "O site contém espaços vazios grandes"
    },
    "videos": {
      "title": "O vídeo não foi reproduzido ou carregado"
    },
    "images": {
      "title": "As imagens não foram carregadas"
    },
    "comments": {
      "title": "Comentários, avaliações ou chats não foram carregados"
    },
    "content": {
      "title": "Falta conteúdo"
    },
    "links": {
      "title": "Os links ou botões não funcionam"
    },
    "login": {
      "title": "Não consigo iniciar sessão/registar-me"
    },
    "loginV2": {
      "title": "Não consigo iniciar sessão/registar-me"
    },
    "shopping": {
      "title": "Não é possível pagar, finalizar a compra ou fazer compras"
    },
    "browser": {
      "title": "Problema com o navegador ou outra extensão do navegador"
    },
    "paywall": {
      "title": "O site solicitou que desativasse o bloqueador de anúncios"
    },
    "other": {
      "title": "Outra coisa"
    },
    "tellUsMoreDesc": {
      "title": "Partilhar mais detalhes (opcional):{bullet} O que aconteceu?{bullet} O que deveria ter acontecido?{bullet} A desativação das proteções ajudou?"
    },
    "sendReport": {
      "title": "Enviar relatório"
    },
    "reportsAreAnonymousDesc": {
      "title": "As denúncias enviadas para o DuckDuckGo incluem apenas as informações necessárias para processarmos o teu feedback."
    },
    "thankYou": {
      "title": "Obrigado!"
    },
    "yourReportWillHelpDesc": {
      "title": "A tua denúncia ajuda-nos a melhorarmos os nossos produtos e a experiência de todos os utilizadores."
    },
    "dislike": {
      "title": "Não gosto do conteúdo"
    },
    "otherRequired": {
      "title": "Descreve o problema que encontraste (obrigatório)"
    },
    "otherOptional": {
      "title": "Descreve o problema que encontraste (opcional)"
    },
    "suggestionWhatHappened": {
      "title": "O que aconteceu?"
    },
    "suggestionWhatHappened2": {
      "title": "O que deveria ter acontecido?"
    },
    "suggestionWhatHappened3": {
      "title": "A desativação das Proteções de Privacidade ajudou?"
    },
    "reportTitle": {
      "title": "Informar o DuckDuckGo"
    }
  },
  "shared": {
    "trackers": {
      "title": "Rastreadores"
    }
  },
  "site": {
    "updatingProtectionList": {
      "title": "A atualizar a lista de proteção"
    },
    "protectionsEnabled": {
      "title": "As proteções estão <b>ATIVADAS</b> para este site"
    },
    "protectionsDisabled": {
      "title": "As proteções estão <b>DESATIVADAS</b> para este site"
    },
    "protectionsDisabledRemote": {
      "title": "Desativámos temporariamente as Proteções de Privacidade, pois parecem estar a impedir que o site funcione corretamente."
    },
    "protectionsDisabledRemoteOverride": {
      "title": "Recomendamos a desativação das Proteções de Privacidade para que este site funcione corretamente."
    },
    "connectionDescriptionUnencrypted": {
      "title": "<b>Este site não é seguro</b> e pode comprometer quaisquer informações que envies nesta página."
    },
    "connectionDescriptionInvalidCertificate": {
      "title": "<b>O certificado deste site é inválido.</b> Podes estar a ligar a um servidor que finge ser <b>{domain}</b>, o que pode colocar as tuas informações confidenciais em risco."
    },
    "trackerNetworksSummaryBlocked": {
      "title": "Foi bloqueado o carregamento dos pedidos dos seguintes domínios de terceiros porque foram identificados como pedidos com rastreamento. Se os pedidos de uma empresa forem carregados, a empresa pode conseguir criar um perfil sobre ti."
    },
    "trackerNetworksSummaryNoneBlocked": {
      "title": "Não foi bloqueado o carregamento de nenhum pedido com rastreamento nesta página. Se os pedidos de uma empresa forem carregados, a empresa pode conseguir criar um perfil sobre ti."
    },
    "trackerNetworksSummaryNoneFound": {
      "title": "Não identificámos pedidos de rastreamento nesta página."
    },
    "trackerNetworksSummaryNone": {
      "title": "Não encontrámos nenhuma empresa a tentar carregar pedidos com rastreamento nesta página."
    },
    "trackerNetworksSummaryAllowedOnly": {
      "title": "Para que o site funcione corretamente, não bloqueámos o carregamento de pedidos com rastreamento de nenhuma empresa."
    },
    "trackerNetworksSummaryProtectionsOff": {
      "title": "Não foram bloqueados pedidos de terceiros porque as Proteções estão desativadas para este site. Se os pedidos de uma empresa forem carregados, a empresa pode conseguir criar um perfil sobre ti."
    },
    "createNewDuckAddress": {
      "title": "Gerar um Duck Address Privado"
    },
    "createNewDuckAddressCopied": {
      "title": "Copiado para a área de transferência!"
    },
    "websiteNotWorkingQ": {
      "title": "O website não funciona como esperado?"
    },
    "websiteNotWorkingPrompt": {
      "title": "O site não está a funcionar?"
    },
    "websiteNotWorkingCta": {
      "title": "Denunciar site danificado"
    },
    "websiteNotWorkingAdvice": {
      "title": "A desativação das proteções pode ajudar."
    },
    "websiteNotWorkingPromptFollowUp": {
      "title": "O site continua sem funcionar?"
    },
    "takePrecautions": {
      "title": "Toma precauções"
    },
    "majorTrackingNetworkDesc": {
      "title": "{blocked, select, true {<b>Este site pertence à {companyDisplayName}</b>, que opera uma rede de rastreamento em {companyPrevalence}% dos principais sites. Conseguimos bloquear alguns dos pedidos desta empresa nesta página.} other {<b>Este site pertence à {companyDisplayName}</b>, que opera uma rede de rastreamento em {companyPrevalence}% dos principais sites. }}"
    },
    "trackersBlockedDesc": {
      "title": "{companyCount, plural, =0 {Bloqueámos algumas empresas de carregarem solicitações de rastreamento nesta página.} =2 {Bloqueámos o carregamento de pedidos com rastreamento da <b>{firstCompany}</b> e da <b>{secondCompany}</b> nesta página.} =3 {Bloqueámos o carregamento de pedidos com rastreamento da <b>{firstCompany}</b>, da <b>{secondCompany}</b> e da <b>{thirdCompany}</b> nesta página.} =4 {Bloqueámos o carregamento de pedidos com rastreamento da <b>{firstCompany}</b>, da <b>{secondCompany}</b>, da <b>{thirdCompany}</b> e da <b>{fourthCompany}</b> nesta página.} =5 {Bloqueámos o carregamento de pedidos com rastreamento da <b>{firstCompany}</b>, da <b>{secondCompany}</b>, da <b>{thirdCompany}</b>, da <b>{fourthCompany}</b> e de <b>mais 1 empresa</b> nesta página.} one {Bloqueámos o carregamento de pedidos com rastreamento da <b>{firstCompany}</b> nesta página.} other {Bloqueámos o carregamento de pedidos com rastreamento da <b>{firstCompany}</b>, da <b>{secondCompany}</b>, da <b>{thirdCompany}</b>, da <b>{fourthCompany}</b> e de <b>mais {othersCount} empresas</b> nesta página.}}"
    },
    "cookiesMinimized": {
      "title": "Cookies geridos"
    },
    "cookiesHidden": {
      "title": "Pop-up de cookies ocultado"
    },
    "cookiesHiddenSummary": {
      "title": "Só conseguimos ocultar o pop-up de cookies neste site porque não foram disponibilizadas opções para gerir as preferências de cookies. As nossas restantes proteções contra o rastreamento na Internet ainda se aplicam."
    },
    "cookiesMinimizedSummary": {
      "title": "Definimos as tuas preferências de cookies para maximizar a privacidade e fechámos o pop-up."
    },
    "cookiesMinimizedSettings": {
      "title": "Desativar nas definições"
    },
    "connectionSecure": {
      "title": "A ligação está encriptada"
    },
    "connectionNotSecure": {
      "title": "A ligação não está encriptada"
    },
    "connectionNotSecureInvalidCertificate": {
      "title": "A ligação pode ser insegura"
    },
    "trackerNetworksDesc": {
      "title": "Pedidos com carregamento bloqueado"
    },
    "trackerNetworksNotBlocked": {
      "title": "Nenhum pedido com rastreamento bloqueado"
    },
    "trackerNetworksNotFound": {
      "title": "Nenhum pedido com rastreamento encontrado"
    },
    "thirdPartiesLoaded": {
      "title": "Pedidos de terceiros carregados"
    },
    "thirdPartiesNoneFound": {
      "title": "Nenhum pedido de terceiros encontrado"
    },
    "firstPartyDesc": {
      "title": "{companyName} é proprietário deste site e dos monitorizadores detetados nesta página, portanto não os bloqueámos."
    },
    "noTrackersFound": {
      "title": "Não encontrámos nenhuma empresa a tentar carregar pedidos com rastreamento nesta página."
    },
    "trackersFoundForAllowlisted": {
      "title": "Os rastreadores ajudam as empresas a segmentar-te. Encontrámos estas empresas a rastrear a tua atividade nesta página."
    },
    "trackersFoundAndBlocked": {
      "title": "Os rastreadores ajudam as empresas a segmentar-te. Bloqueámos o rastreio da tua atividade nesta página por parte destas empresas."
    },
    "trackerNetworkUnknown": {
      "title": "Rede de rastreadores desconhecida"
    },
    "trackerDomainsForCompany": {
      "title": "Domínios de rastreador de {companyName}"
    },
    "zeroTrackersFound": {
      "title": "0 rastreadores encontrados"
    },
    "trackerOwnedByThisSite": {
      "title": "Rastreadores propriedade deste Site (Permitido)"
    },
    "trackerCountForDomain": {
      "title": "{blocked, select, true {{trackerCount, plural, one {{trackerCount} rastreador bloqueado em {domain}} other {{trackerCount} rastreadores bloqueados em {domain}}}} other {{trackerCount, plural, one {{trackerCount} rastreador encontrado em {domain}} other {{trackerCount} rastreadores encontrados em {domain}}}}}"
    },
    "trackerLimitationsNote": {
      "title": "Nota: as limitações da plataforma podem limitar a nossa capacidade de detetar todos os pedidos."
    },
    "trackerAboutLink": {
      "title": "Sobre as nossas proteções contra o rastreamento na internet"
    },
    "trackerAdLink": {
      "title": "Como os nossos anúncios de pesquisa afetam as nossas proteções"
    },
    "sectionHeadingAdAttribution": {
      "title": "Os pedidos do seguinte domínio foram carregados devido a um clique recente num anúncio de {domain} no DuckDuckGo. Estes pedidos ajudam a avaliar a eficácia dos anúncios. Nenhum anúncio no DuckDuckGo é utilizado para criação de perfis."
    },
    "sectionHeadingIgnore": {
      "title": "Os pedidos dos seguintes domínios foram carregados para que o site funcione corretamente."
    },
    "sectionHeadingFirstParty": {
      "title": "Os pedidos dos seguintes domínios foram carregados porque estão associados a {domain}."
    },
    "sectionHeadingThirdParty": {
      "title": "Os pedidos dos seguintes domínios também foram carregados."
    },
    "sectionHeadingProtectionsDisabled": {
      "title": "Os pedidos dos seguintes domínios foram carregados porque as proteções estão desativadas."
    },
    "thirdPartiesSummaryLoaded": {
      "title": "Os pedidos dos seguintes domínios de terceiros foram carregados. Se os pedidos de uma empresa forem carregados, a empresa pode conseguir criar um perfil sobre ti, embora sejam aplicadas as nossas outras proteções contra rastreamento na internet."
    },
    "thirdPartiesSummaryProtectionsOff": {
      "title": "Os pedidos dos seguintes domínios de terceiros foram carregados. Se os pedidos de uma empresa forem carregados, a empresa pode conseguir criar um perfil sobre ti, embora sejam aplicadas as nossas outras proteções contra rastreamento na internet."
    },
    "thirdPartiesSummaryNone": {
      "title": "Não identificámos pedidos de domínios de terceiros."
    },
    "analyticsCategory": {
      "title": "Dados analíticos"
    },
    "advertisingCategory": {
      "title": "Publicidade"
    },
    "socialCategory": {
      "title": "Redes sociais"
    },
    "contentDeliveryCategory": {
      "title": "Fornecimento de conteúdos"
    },
    "embeddedContentCategory": {
      "title": "Conteúdo incorporado"
    },
    "searchPlaceholder": {
      "title": "Pesquisar no DuckDuckGo"
    },
    "searchGoButton": {
      "title": "Pesquisar"
    },
    "optionsButton": {
      "title": "Mais opções"
    },
    "navigationComplete": {
      "title": "Feito"
    },
    "navigationClose": {
      "title": "Fechar"
    },
    "navigationCancel": {
      "title": "Cancelar"
    },
    "navigationBack": {
      "title": "Retroceder"
    },
    "enableProtectionsSwitch": {
      "title": "Ativar proteções"
    },
    "disableProtectionsSwitch": {
      "title": "Desativar proteções"
    },
    "errorMessage": {
      "title": "Algo correu mal e não conseguimos carregar este conteúdo. Tenta recarregar a página."
    }
  }
}