export default {
  "connection": {
    "encrypt": {
      "title": "Šifrovať"
    },
    "sign": {
      "title": "Podpísať"
    },
    "verify": {
      "title": "Overiť"
    },
    "derive": {
      "title": "Odvodiť"
    },
    "wrap": {
      "title": "Zabaliť"
    },
    "unwrap": {
      "title": "Rozbaliť"
    },
    "unknown": {
      "title": "Neznáme"
    },
    "certificateDetail": {
      "title": "Podrobnosti bezpečnostného certifikátu"
    },
    "commonName": {
      "title": "Všeobecný názov"
    },
    "publicKey": {
      "title": "Verejný kľúč"
    },
    "algorithm": {
      "title": "Algoritmus"
    },
    "keySize": {
      "title": "Veľkosť kľúča"
    },
    "usage": {
      "title": "Používanie"
    },
    "summary": {
      "title": "Zhrnutie"
    },
    "permanent": {
      "title": "Trvalé"
    },
    "effectiveSize": {
      "title": "Efektívna veľkosť"
    },
    "certificateNotFound": {
      "title": "Certifikát nebol nájdený"
    },
    "certificateForDomain": {
      "title": "Certifikát pre {domain}"
    },
    "insecureConnectionDesc": {
      "title": "Táto stránka používa nešifrované pripojenie. Tretie strany môžu byť schopné zobraziť vašu aktivitu alebo môžu zachytiť citlivé informácie, ktoré odošlete na tejto stránke."
    },
    "invalidConnectionDesc": {
      "title": "Certifikát pre túto stránku je neplatný. Možno sa pripájate k serveru, ktorý predstiera, že je <b>{domain}</b>, čo by mohlo ohroziť vaše dôverné údaje."
    },
    "upgradedConnectionDesc": {
      "title": "Inovovali sme pripojenie na tejto stránke, aby sme chránili vami odosielané údaje počas prenosu."
    },
    "secureConnectionDesc": {
      "title": "Táto stránka používa šifrované pripojenie, ktoré zabraňuje tretím stranám prezerať si vašu činnosť alebo zachytávať citlivé informácie, ktoré na tejto stránke odosielate."
    }
  },
  "ctascreens": {
    "protectionsUnavailableNote": {
      "title": "Ochrana súkromia nie je k dispozícii pre špeciálne stránky alebo miestne stránky."
    },
    "spreadTitle": {
      "title": "Radi používate DuckDuckGo?"
    },
    "spreadText": {
      "title": "Povedzte o DuckDuckGo rodine a priateľom"
    },
    "spreadButton": {
      "title": "Rozšíriť DuckDuckGo"
    },
    "emailTitle": {
      "title": "Máte už dosť sledovania vašich e-mailov?"
    },
    "emailText": {
      "title": "Zaregistrujte si svoje rozšírenie DuckDuckGo na ochranu e-mailov teraz!"
    }
  },
  "firebutton": {
    "fireDialogHeader": {
      "title": "Zatvoriť karty a vymazať údaje"
    },
    "fireDialogHeaderNoTabs": {
      "title": "Vymazať údaje"
    },
    "optionCurrentSite": {
      "title": "Len aktuálna stránka"
    },
    "optionLastHour": {
      "title": "Posledná hodina"
    },
    "optionLast24Hour": {
      "title": "Posledných 24 hodín"
    },
    "optionLast7days": {
      "title": "Posledných 7 dní"
    },
    "optionLast4Weeks": {
      "title": "Posledné 4 týždne"
    },
    "optionAllTime": {
      "title": "Celé obdobie"
    },
    "historyDuration": {
      "title": "{duration, select, hour {jednu hodinu} day {24 hodín} week {jeden týždeň} month {4 týždne} other {Všetky}}"
    },
    "summaryClearTabsHistoryDuration": {
      "title": "{openTabs, plural, one {Zatvoriť <b>{openTabs}</b> kartu a vymazať <b>{durationDesc}</b> histórie prehliadania a súbory cookie?} few {Zatvoriť <b>{openTabs}</b> karty a vymazať <b>{durationDesc}</b> histórie prehliadania a súbory cookie?} many {Zatvoriť <b>{openTabs}</b> karty a vymazať <b>{durationDesc}</b> histórie prehliadania a súbory cookie?} other {Zatvoriť <b>{openTabs}</b> kariet a vymazať <b>{durationDesc}</b> histórie prehliadania a súbory cookie?}}"
    },
    "summaryClearTabsDuration": {
      "title": "{openTabs, plural, one {Zavrieť <b>{openTabs}</b> kartu a vymazať <b>{durationDesc}</b> súborov cookie?} few {Zavrieť <b>{openTabs}</b> karty a vymazať <b>{durationDesc}</b> súborov cookie?} many {Zavrieť <b>{openTabs}</b> karty a vymazať <b>{durationDesc}</b> súborov cookie?} other {Zavrieť <b>{openTabs}</b> kariet a vymazať <b>{durationDesc}</b> súborov cookie?}}"
    },
    "summaryClearHistoryDuration": {
      "title": "Vymazať <b>{durationDesc}</b> histórie prehliadania a súborov cookie?"
    },
    "summaryClearCookiesDuration": {
      "title": "Vymazať <b>{durationDesc}</b> súborov cookie?"
    },
    "summaryClearTabsHistoryAll": {
      "title": "Zatvoriť <b>{openTabs}</b> {openTabs, plural, =1 {kartu} other {kariet}} a vymazať <b>celú</b> históriu prehliadania a súbory cookie ({cookies} {cookies, plural, =1 {stránka} other {stránok}})?"
    },
    "summaryClearTabsAll": {
      "title": "Zatvoriť <b>{openTabs}</b> {openTabs, plural, =1 {kartu} other {kariet}} a vymazať <b>všetky</b> súbory cookie ({cookies} {cookies, plural, =1 {stránku} other {stránok}})?"
    },
    "summaryClearHistoryAll": {
      "title": "{cookies, plural, one {Vymazať <b>celú</b> históriu prehliadania a súbory cookie ({cookies} stránka)?} few {Vymazať <b>celú</b> históriu prehliadania a súbory cookie ({cookies} stránky)?} many {Vymazať <b>celú</b> históriu prehliadania a súbory cookie ({cookies} stránky)?} other {Vymazať <b>celú</b> históriu prehliadania a súbory cookie ({cookies} stránok)?}}"
    },
    "summaryClearCookiesAll": {
      "title": "{cookies, plural, one {Vymazať <b>všetky</b> súbory cookie ({cookies} stránka)?} few {Vymazať <b>všetky</b> súbory cookie ({cookies} stránky)?} many {Vymazať <b>všetky</b> súbory cookie ({cookies} stránky)?} other {Vymazať <b>všetky</b> súbory cookie ({cookies} stránok)?}}"
    },
    "summaryClearTabsHistorySite": {
      "title": "{openTabs, plural, one {Zatvoriť <b>{openTabs} kariet na stránke {site}</b> a vymazať <b>všetky súbory cookie zo stránky {site}</b> ?} few {Zatvoriť <b>{openTabs} karty na stránke {site}</b> a vymazať <b>všetky súbory cookie zo stránky {site}</b>?} many {Zatvoriť <b>{openTabs} karty na stránke {site}</b> a vymazať <b>všetky súbory cookie zo stránky {site}</b> ?} other {Zatvoriť <b>{openTabs} kariet {site}</b> a vymazať <b>všetky súbory cookie zo stránky {site}</b> ?}}"
    },
    "summaryClearTabsSite": {
      "title": "{openTabs, plural, one {Zatvoriť <b>{openTabs} kartu na stránke {site}</b> a vymazať <b>všetky súbory cookie zo stránky {site}</b> ?} few {Zatvoriť <b>{openTabs} karty na stránke {site}</b> a vymazať <b>všetky súbory cookie zo stránky {site}</b>?} many {Zatvoriť <b>{openTabs} karty na stránke {site}</b> a vymazať <b>všetky súbory cookie zo stránky {site}</b> ?} other {Zatvoriť <b>{openTabs} kariet na stránke {site}</b> a vymazať <b>všetky súbory cookie zo stránky {site}</b> ?}}"
    },
    "summaryClearHistorySite": {
      "title": "Vymazať <b>celú históriu prehliadania a súbory cookie na stránke {site}</b>?"
    },
    "summaryClearCookiesSite": {
      "title": "Vymazať <b>všetky súbory cookie na stránke {site}</b>?"
    },
    "summaryPinnedIgnored": {
      "title": "{tabs, plural, one {<b>{tabs} pripnutá</b> karta bude ignorovaná.} few {<b>{tabs} pripnuté</b> karty budú ignorované.} many {<b>{tabs} pripnutej</b> karty bude ignorovaných.} other {<b>{tabs} pripnutých</b> kariet bude ignorovaných.}}"
    },
    "clearData": {
      "title": "Jasno"
    },
    "cancel": {
      "title": "Zrušiť"
    },
    "historyAndDownloadsNotAffected": {
      "title": "Ak chcete vymazať aj históriu, vyberte obdobie."
    }
  },
  "permissions": {
    "camera": {
      "title": "Kamera"
    },
    "microphone": {
      "title": "Mikrofón"
    },
    "geolocation": {
      "title": "Geolokácia"
    },
    "popups": {
      "title": "Automaticky zobrazené okná"
    },
    "ask": {
      "title": "Vždy sa opýtať"
    },
    "notify": {
      "title": "Oznámiť"
    },
    "grant": {
      "title": "Vždy povoliť"
    },
    "deny": {
      "title": "Vždy odoprieť"
    }
  },
  "report": {
    "selectTheOptionDesc": {
      "title": "Odoslanie anonymného hlásenia o nefunkčnej stránke nám pomáha zlepšovať aplikáciu."
    },
    "selectTheOptionDescV2": {
      "title": "Vyberte možnosť, ktorá najlepšie opisuje problém, ktorý ste zaznamenali."
    },
    "selectTheCategoryType": {
      "title": "V čom je problém?"
    },
    "categoryType1": {
      "title": "Webová lokalita nefunguje podľa očakávania?"
    },
    "categoryType2": {
      "title": "Nepáči sa mi obsah na tejto stránke"
    },
    "categoryType3": {
      "title": "Všeobecná spätná väzba k prehliadaču DuckDuckGo"
    },
    "selectTheCategory": {
      "title": "Čo na tejto stránke nefunguje?"
    },
    "tryTurningProtectionsOff": {
      "title": "Skúste vypnúť ochranu súkromia a zistite, či to problém vyrieši."
    },
    "skipThisStep": {
      "title": "Tento krok preskočte"
    },
    "pickYourIssueFromTheList": {
      "title": "Popíšte, čo sa stalo"
    },
    "blocked": {
      "title": "Stránka je zablokovaná alebo sa nenačítala"
    },
    "layout": {
      "title": "Rozloženie stránky je nefunkčné"
    },
    "emptySpaces": {
      "title": "Stránka obsahuje veľké prázdne miesta"
    },
    "videos": {
      "title": "Video sa neprehralo alebo nenačítalo"
    },
    "images": {
      "title": "Obrázky sa nepodarilo načítať"
    },
    "comments": {
      "title": "Nenačítali sa komentáre, recenzie alebo čety"
    },
    "content": {
      "title": "Chýba obsah"
    },
    "links": {
      "title": "Odkazy alebo tlačidlá nefungujú"
    },
    "login": {
      "title": "Nedá sa prihlásiť/registrovať"
    },
    "loginV2": {
      "title": "Nedá sa prihlásiť ani zaregistrovať"
    },
    "shopping": {
      "title": "Nedá sa zaplatiť, prejsť do pokladne ani nakupovať"
    },
    "browser": {
      "title": "Vyskytol sa problém s iným rozšírením prehliadača"
    },
    "paywall": {
      "title": "Stránka ma požiadala, aby som vypol/vypla blokovanie reklám"
    },
    "other": {
      "title": "Niečo iné"
    },
    "tellUsMoreDesc": {
      "title": "Zdieľajte ďalšie podrobnosti (voliteľné):{bullet}Čo sa stalo?{bullet}Kčomu malo dôjsť?{bullet} Pomáhavypnutie ochranných prvkov?"
    },
    "sendReport": {
      "title": "Odoslať správu"
    },
    "reportsAreAnonymousDesc": {
      "title": "Správy odoslané spoločnosti DuckDuckGo obsahujú len informácie, ktoré sú potrebné na to, aby sme mohli riešiť vašu spätnú väzbu."
    },
    "thankYou": {
      "title": "Ďakujeme!"
    },
    "yourReportWillHelpDesc": {
      "title": "Vaše oznámenie pomôže zlepšiť prehliadač a zlepšiť skúsenosti ostatných používateľov."
    },
    "dislike": {
      "title": "Nevhodný obsah"
    },
    "otherRequired": {
      "title": "Opíšte problém, s ktorým ste sa stretli (povinné)."
    },
    "otherOptional": {
      "title": "Opíšte problém, s ktorým ste sa stretli, a ktorých rozšírení sa týkal."
    },
    "suggestionWhatHappened": {
      "title": "Čo sa stalo?"
    },
    "suggestionWhatHappened2": {
      "title": "Čo sa malo stať?"
    },
    "suggestionWhatHappened3": {
      "title": "Pomohlo vypnutie ochrany súkromia?"
    },
    "reportTitle": {
      "title": "Nahlásiť pracovníkom DuckDuckGo"
    }
  },
  "shared": {
    "trackers": {
      "title": "Sledovacie zariadenia"
    }
  },
  "site": {
    "updatingProtectionList": {
      "title": "Aktualizácia zoznamu ochrany"
    },
    "protectionsEnabled": {
      "title": "Ochrany sú pre tento web <b>ZAPNUTÉ</b>"
    },
    "protectionsDisabled": {
      "title": "Ochrany sú pre tento web <b>VYPNUTÉ</b>"
    },
    "protectionsDisabledRemote": {
      "title": "Dočasne sme vypli ochranu súkromia, pretože sa zdá, že spôsobuje poruchy na tejto lokalite."
    },
    "protectionsDisabledRemoteOverride": {
      "title": "Odporúčame vypnúť ochranu súkromia pre túto lokalitu s cieľom zabrániť chybám."
    },
    "connectionDescriptionUnencrypted": {
      "title": "<b>Táto lokalita nie je zabezpečená</b> a môže ohroziť zabezpečenie všetkých informácií, ktoré na nej odošlete."
    },
    "connectionDescriptionInvalidCertificate": {
      "title": "<b>Certifikát pre túto lokalitu je neplatný</b>. Možno sa pripájate k serveru, ktorý predstiera, že je <b>{domain}</b>, čo by mohlo ohroziť vaše dôverné údaje."
    },
    "trackerNetworksSummaryBlocked": {
      "title": "Načítanie požiadaviek nasledujúcich domén tretích strán bolo zablokované, pretože boli identifikované ako požiadavky so sledovaním. Ak sú načítané požiadavky spoločnosti, môže im umožniť profilovať vás."
    },
    "trackerNetworksSummaryNoneBlocked": {
      "title": "Na tejto stránke nebolo zablokované načítanie žiadnych požiadaviek so sledovaním. Ak sú požiadavky spoločnosti načítané, môže im to umožniť profilovať vás."
    },
    "trackerNetworksSummaryNoneFound": {
      "title": "Na tejto stránke sme nezistili žiadne požiadavky so sledovaním."
    },
    "trackerNetworksSummaryNone": {
      "title": "Na tejto stránke sme nenašli žiadne spoločnosti, ktoré by sa pokúšali načítať požiadavky so sledovaním."
    },
    "trackerNetworksSummaryAllowedOnly": {
      "title": "Aby sme zabránili narušeniu lokality, neblokovali sme žiadnym spoločnostiam načítanie žiadostí so sledovaním na tejto stránke."
    },
    "trackerNetworksSummaryProtectionsOff": {
      "title": "Nebolo zablokované žiadne načítavanie požiadaviek so sledovaním, pretože ochrana je pre túto stránku vypnutá. Ak sú požiadavky spoločnosti načítané, môže im to umožniť profilovať vás."
    },
    "createNewDuckAddress": {
      "title": "Generovať súkromnú adresu Duck"
    },
    "createNewDuckAddressCopied": {
      "title": "Skopírované do schránky!"
    },
    "websiteNotWorkingQ": {
      "title": "Webová lokalita nefunguje podľa očakávania?"
    },
    "websiteNotWorkingPrompt": {
      "title": "Webová stránka nefunguje?"
    },
    "websiteNotWorkingCta": {
      "title": "Nahlásiť nefunkčnú lokalitu"
    },
    "websiteNotWorkingAdvice": {
      "title": "Pomôcť by mohlo VYPNUTIE ochranných prvkov."
    },
    "websiteNotWorkingPromptFollowUp": {
      "title": "Stále nefunguje webová stránka?"
    },
    "takePrecautions": {
      "title": "Vykonajte preventívne opatrenia"
    },
    "majorTrackingNetworkDesc": {
      "title": "{blocked, select, true {<b>Túto stránku vlastní spoločnosť {companyDisplayName}</b>, ktorá prevádzkuje sieť sledovačov na {companyPrevalence} % najlepších webových lokalít. Na tejto stránke sa nám podarilo zablokovať niektoré z ich požiadaviek.} other {<b>Túto stránku vlastní spoločnosť {companyDisplayName}</b>, ktorá prevádzkuje sieť sledovačov na {companyPrevalence} % najlepších webových lokalít. }}"
    },
    "trackersBlockedDesc": {
      "title": "{companyCount, plural, =0 {Niektorým spoločnostiam sme na tejto stránke zablokovali načítanie požiadaviek so sledovaním.} =2 {Zablokovali sme <b>{firstCompany}</b> a <b>{secondCompany}</b>, z načítania požiadaviek so sledovaním na tejto stránke.} =3 {Zablokovali sme <b>{firstCompany}</b>, <b>{secondCompany}</b> a <b>{thirdCompany}</b> z načítania požiadaviek so sledovaním.} =4 {Zablokovali sme <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b> a <b>{fourthCompany}</b> z načítania požiadaviek so sledovaním na tejto stránke.} =5 {Zablokovali sme <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b>, <b>{fourthCompany}</b> a <b>1 ďalšie</b> z načítania požiadaviek so sledovaním na tejto stránke.} one {Zablokovali sme <b>{firstCompany}</b> pred načítaním požiadaviek so sledovaním na tejto stránke.} few {Zablokovali sme <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b>, <b>{fourthCompany}</b>a <b>{othersCount} ďalšie</b>pred načítaním požiadaviek so sledovaním na tejto stránke.} many {Zablokovali sme <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b>, <b>{fourthCompany}</b>a <b>{othersCount} ďalšie</b>pred načítaním požiadaviek so sledovaním na tejto stránke.} other {Zablokovali sme <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b>, <b>{fourthCompany}</b>a <b>{othersCount} ďalšie</b>pred načítaním požiadaviek so sledovaním na tejto stránke.}}"
    },
    "cookiesMinimized": {
      "title": "Spravované súbory cookie"
    },
    "cookiesHidden": {
      "title": "Kontextové okno pre súbory cookie je skryté"
    },
    "cookiesHiddenSummary": {
      "title": "Na tejto lokalite sme mohli vyskakovacie okno so súhlasom pre súbory cookie iba skryť, pretože k dispozícii neboli žiadne možnosti na správu preferencií súborov cookie. Naša ostatná ochrana proti sledovaniu z webu stále funguje."
    },
    "cookiesMinimizedSummary": {
      "title": "Nastavili sme vaše preferencie pre súbory cookie na maximalizáciu ochrany súkromia a zatvorili sme automaticky otvárané okno."
    },
    "cookiesMinimizedSettings": {
      "title": "Zakázať v nastaveniach"
    },
    "connectionSecure": {
      "title": "Pripojenie je šifrované"
    },
    "connectionNotSecure": {
      "title": "Pripojenie nie je šifrované"
    },
    "connectionNotSecureInvalidCertificate": {
      "title": "Pripojenie môže byť nezabezpečené"
    },
    "trackerNetworksDesc": {
      "title": "Načítavanie žiadostí je zablokované"
    },
    "trackerNetworksNotBlocked": {
      "title": "Nie sú blokované žiadne požiadavky so sledovaním"
    },
    "trackerNetworksNotFound": {
      "title": "Nenašli sa žiadne požiadavky so sledovaním"
    },
    "thirdPartiesLoaded": {
      "title": "Požiadavky tretích strán boli načítané"
    },
    "thirdPartiesNoneFound": {
      "title": "Nenašli sa žiadne požiadavky tretích strán"
    },
    "firstPartyDesc": {
      "title": "{companyName} vlastní túto stránku a známe sledovacie zariadenia nájdené na tejto stránke, takže sme ich nezablokovali."
    },
    "noTrackersFound": {
      "title": "Na tejto stránke sme nenašli žiadne spoločnosti, ktoré by sa pokúšali načítať požiadavky so sledovaním."
    },
    "trackersFoundForAllowlisted": {
      "title": "Sledovacie zariadenia pomáhajú spoločnostiam profilovať vás. Na tejto stránke sme našli tieto spoločnosti, ktoré monitorujú vašu aktivitu."
    },
    "trackersFoundAndBlocked": {
      "title": "Sledovacie zariadenia pomáhajú spoločnostiam profilovať vás. Týmto spoločnostiam sme zablokovali monitorovanie vašej aktivity na tejto stránke."
    },
    "trackerNetworkUnknown": {
      "title": "Neznáma sieť sledovacích zariadení"
    },
    "trackerDomainsForCompany": {
      "title": "Domény sledovacích zariadení pre spoločnosť {companyName}"
    },
    "zeroTrackersFound": {
      "title": "Našlo sa '{'0'}' sledovacích zariadení"
    },
    "trackerOwnedByThisSite": {
      "title": "Sledovacie zariadenia vlastnené touto lokalitou (povolené)"
    },
    "trackerCountForDomain": {
      "title": "{blocked, select, true {{trackerCount, plural, one {{trackerCount} sledovacie zariadenie zablokované na {domain}} few {{trackerCount} sledovacie zariadenia zablokované na {domain}} many {{trackerCount} sledovacieho zariadenia zablokovaného na {domain}} other {{trackerCount} sledovacích zariadení zablokovaných na {domain}}}} other {{trackerCount, plural, one {{trackerCount} sledovacie zariadenie nájdené na {domain}} few {{trackerCount} sledovacie zariadenia nájdené na {domain}} many {{trackerCount} sledovacieho zariadenia nájdeného na {domain}} other {{trackerCount} sledovacích zariadení nájdených na {domain}}}}}"
    },
    "trackerLimitationsNote": {
      "title": "Upozornenie: obmedzenia platformy môžu obmedziť našu schopnosť zistiť všetky požiadavky."
    },
    "trackerAboutLink": {
      "title": "O našej ochrane pred sledovaním činnosti na webe"
    },
    "trackerAdLink": {
      "title": "Ako naše reklamy vo vyhľadávaní ovplyvňujú našu ochranu"
    },
    "sectionHeadingAdAttribution": {
      "title": "Požiadavky nasledujúcej domény boli načítané, pretože nedávno ste klikli na reklamu {domain} na DuckDuckGo. Tieto žiadosti pomáhajú vyhodnotiť účinnosť reklám. Všetky reklamy na DuckDuckGo sú neprofilujúce."
    },
    "sectionHeadingIgnore": {
      "title": "Požiadavky nasledujúcej domény boli načítané, aby sa zabránilo rozbitiu stránky."
    },
    "sectionHeadingFirstParty": {
      "title": "Požiadavky na nasledujúce domény boli načítané, pretože sú spojené s doménou {domain}."
    },
    "sectionHeadingThirdParty": {
      "title": "Boli načítané aj nasledujúce požiadavky domény."
    },
    "sectionHeadingProtectionsDisabled": {
      "title": "Požiadavky na nasledujúce domény boli načítané, pretože ochrany sú vypnuté."
    },
    "thirdPartiesSummaryLoaded": {
      "title": "Boli načítané nasledujúce požiadavky na domény tretích strán. Ak sú požiadavky spoločnosti načítané, môže im to umožniť profilovať vás, hoci naše ostatné ochrany sledovania webu stále platia."
    },
    "thirdPartiesSummaryProtectionsOff": {
      "title": "Boli načítané nasledujúce požiadavky na domény tretích strán. Ak sú požiadavky spoločnosti načítané, môže im to umožniť profilovať vás, hoci naše ostatné ochrany sledovania webu stále platia."
    },
    "thirdPartiesSummaryNone": {
      "title": "Neidentifikovali sme žiadne žiadosti z domén tretích strán."
    },
    "analyticsCategory": {
      "title": "Analytické údaje"
    },
    "advertisingCategory": {
      "title": "Reklama"
    },
    "socialCategory": {
      "title": "Sociálna sieť"
    },
    "contentDeliveryCategory": {
      "title": "Dodanie obsahu"
    },
    "embeddedContentCategory": {
      "title": "Vložený obsah"
    },
    "searchPlaceholder": {
      "title": "Vyhľadávajte cez DuckDuckGo"
    },
    "searchGoButton": {
      "title": "Vyhľadávať"
    },
    "optionsButton": {
      "title": "Ďalšie možnosti"
    },
    "navigationComplete": {
      "title": "Hotovo"
    },
    "navigationClose": {
      "title": "Zatvoriť"
    },
    "navigationCancel": {
      "title": "Zrušiť"
    },
    "navigationBack": {
      "title": "Späť"
    },
    "enableProtectionsSwitch": {
      "title": "Povolenie ochrany"
    },
    "disableProtectionsSwitch": {
      "title": "Zakázanie ochrany"
    },
    "errorMessage": {
      "title": "Došlo k chybe a my sme nemohli načítať tento obsah. Skúste znovu načítať stránku."
    }
  }
}