export default {
  "connection": {
    "encrypt": {
      "title": "Šifruoti"
    },
    "sign": {
      "title": "Pasirašyti"
    },
    "verify": {
      "title": "Tikrinti"
    },
    "derive": {
      "title": "Išvesti"
    },
    "wrap": {
      "title": "Suskleisti"
    },
    "unwrap": {
      "title": "Išskleisti"
    },
    "unknown": {
      "title": "Nežinomas"
    },
    "certificateDetail": {
      "title": "Išsami informacija apie saugos sertifikatą"
    },
    "commonName": {
      "title": "Bendras pavadinimas"
    },
    "publicKey": {
      "title": "Viešas raktas"
    },
    "algorithm": {
      "title": "Algoritmas"
    },
    "keySize": {
      "title": "Rakto dydis"
    },
    "usage": {
      "title": "Naudojimas"
    },
    "summary": {
      "title": "Santrauka"
    },
    "permanent": {
      "title": "Nuolatinis"
    },
    "effectiveSize": {
      "title": "Efektyvus dydis"
    },
    "certificateNotFound": {
      "title": "Sertifikatas nerastas"
    },
    "certificateForDomain": {
      "title": "Sertifikatas, skirtas {domain}"
    },
    "insecureConnectionDesc": {
      "title": "Šiame puslapyje naudojamas neužšifruotas ryšys. Trečiosios šalys gali matyti jūsų veiklą arba perimti slaptą informaciją, kurią siunčiate šiame puslapyje."
    },
    "invalidConnectionDesc": {
      "title": "Šios svetainės sertifikatas negalioja. Galbūt jungiatės prie serverio, kuris apsimeta <b>{domain}</b>, o tai gali kelti pavojų jūsų konfidencialiai informacijai."
    },
    "upgradedConnectionDesc": {
      "title": "Atnaujinome šiame puslapyje esantį ryšį, kad apsaugotume siunčiamą informaciją jos perdavimo metu."
    },
    "secureConnectionDesc": {
      "title": "Šiame puslapyje naudojamas užšifruotas ryšys, todėl trečiosios šalys negali peržiūrėti jūsų veiklos ar perimti slaptos informacijos, kurią siunčiate šiame puslapyje."
    }
  },
  "ctascreens": {
    "protectionsUnavailableNote": {
      "title": "Privatumo apsauga netaikoma specialiesiems ar vietiniams puslapiams."
    },
    "spreadTitle": {
      "title": "Mėgstate naudoti „DuckDuckGo“?"
    },
    "spreadText": {
      "title": "Padėkite mums skleisti žinią jūsų šeimai ir draugams"
    },
    "spreadButton": {
      "title": "Skleiskite žinią apie „DuckDuckGo“"
    },
    "emailTitle": {
      "title": "Pavargote nuo sekamų el. laiškų?"
    },
    "emailText": {
      "title": "Užsisakykite „DuckDuckGo“ el. pašto apsaugą savo plėtiniui dabar!"
    }
  },
  "firebutton": {
    "fireDialogHeader": {
      "title": "Uždaryti skirtukus ir valyti duomenis"
    },
    "fireDialogHeaderNoTabs": {
      "title": "Valyti duomenis"
    },
    "optionCurrentSite": {
      "title": "Tik dabartinė svetainė"
    },
    "optionLastHour": {
      "title": "Paskutinė valanda"
    },
    "optionLast24Hour": {
      "title": "Paskutinės 24 valandos"
    },
    "optionLast7days": {
      "title": "Paskutinės 7 dienos"
    },
    "optionLast4Weeks": {
      "title": "Paskutinės 4 savaitės"
    },
    "optionAllTime": {
      "title": "Viso laiko"
    },
    "historyDuration": {
      "title": "{duration, select, hour {viena valanda} day {24 valandos} week {viena savaitė} month {4 savaitės} other {Visi}}"
    },
    "summaryClearTabsHistoryDuration": {
      "title": "{openTabs, plural, one {Uždaryti <b>{openTabs}</b> skirtuką ir išvalyti <b>{durationDesc}</b> naršymo istoriją ir slapukus?} few {Uždaryti <b>{openTabs}</b> skirtukus ir išvalyti <b>{durationDesc}</b> naršymo istoriją ir slapukus?} many {Uždaryti <b>{openTabs}</b> skirtuko ir išvalyti <b>{durationDesc}</b> naršymo istoriją ir slapukus?} other {Uždaryti <b>{openTabs}</b> skirtukų ir išvalyti <b>{durationDesc}</b> naršymo istoriją ir slapukus?}}"
    },
    "summaryClearTabsDuration": {
      "title": "{openTabs, plural, one {Uždaryti <b>{openTabs}</b> skirtuką ir išvalyti <b>{durationDesc}</b> slapukus?} few {Uždaryti <b>{openTabs}</b> skirtukus ir išvalyti <b>{durationDesc}</b> slapukus?} many {Uždaryti <b>{openTabs}</b> skirtuko ir išvalyti <b>{durationDesc}</b> slapukus?} other {Uždaryti <b>{openTabs}</b> skirtukų ir išvalyti <b>{durationDesc}</b> slapukus?}}"
    },
    "summaryClearHistoryDuration": {
      "title": "Išvalyti <b>{durationDesc}</b> naršymo istoriją ir slapukus?"
    },
    "summaryClearCookiesDuration": {
      "title": "Išvalyti <b>{durationDesc}</b> slapukus?"
    },
    "summaryClearTabsHistoryAll": {
      "title": "Uždaryti <b>{openTabs}</b> {openTabs, plural, =1 {skirtuką} few {skirtukus} other {skirtukų}}, ir išvalyti <b>visą</b> naršymo istoriją ir slapukus ({cookies} {cookies, plural, =1 {svetainėje} few {svetainėse} other {svetainių}})?"
    },
    "summaryClearTabsAll": {
      "title": "Uždaryti <b>{openTabs}</b> {openTabs, plural, =1 {skirtuką} few {skirtukus} other {skirtukų}}, ir išvalyti <b>visus</b> slapukus ({cookies} {cookies, plural, =1 {svetainėje} few {svetainėse} other {svetainių}})?"
    },
    "summaryClearHistoryAll": {
      "title": "{cookies, plural, one {Išvalyti <b>visą</b> naršymo istoriją ir slapukus ({cookies} svetainėje)?} few {Išvalyti <b>visą</b> naršymo istoriją ir slapukus ({cookies} svetainėse)?} many {Išvalyti <b>visą</b> naršymo istoriją ir slapukus ({cookies} svetainės)?} other {Išvalyti <b>visą</b> naršymo istoriją ir slapukus ({cookies} svetainių)?}}"
    },
    "summaryClearCookiesAll": {
      "title": "{cookies, plural, one {Išvalyti <b>visus</b> slapukus ({cookies} svetainėje)?} few {Išvalyti <b>visus</b> slapukus ({cookies} svetainėse)?} many {Išvalyti <b>visus</b> slapukus ({cookies} svetainės)?} other {Išvalyti <b>visus</b> slapukus ({cookies} svetainių)?}}"
    },
    "summaryClearTabsHistorySite": {
      "title": "{openTabs, plural, one {Uždaryti <b>{openTabs} {site}</b> skirtuką ir išvalyti <b>visus {site}</b> slapukus?} few {Uždaryti <b>{openTabs} {site}</b> skirtukus ir išvalyti <b>visus {site}</b> slapukus?} many {Uždaryti <b>{openTabs} {site}</b> skirtuko ir išvalyti <b>visus {site}</b> slapukus?} other {Uždaryti <b>{openTabs} {site}</b> skirtukų ir išvalyti <b>visus {site}</b> slapukus?}}"
    },
    "summaryClearTabsSite": {
      "title": "{openTabs, plural, one {Uždarykite <b>{openTabs} {site}</b> skirtukus ir išvalykite <b>visus {site}</b> slapukus?} few {Uždaryti <b>{openTabs} {site}</b> skirtukus ir išvalyti <b>visus {site}</b> slapukus?} many {Uždaryti <b>{openTabs} {site}</b> skirtuko ir išvalyti <b>visus {site}</b> slapukus?} other {Uždaryti <b>{openTabs} {site}</b> skirtukų ir išvalyti <b>visus {site}</b> slapukus?}}"
    },
    "summaryClearHistorySite": {
      "title": "Išvalyti <b>visą {site}</b> naršymo istoriją ir slapukus?"
    },
    "summaryClearCookiesSite": {
      "title": "Išvalyti <b>visus {site}</b> slapukus?"
    },
    "summaryPinnedIgnored": {
      "title": "{tabs, plural, one {<b>{tabs} prisegtas</b> skirtukas bus ignoruojamas.} few {<b>{tabs} prisegti</b> skirtukai bus ignoruojami.} many {<b>{tabs} prisegto</b> skirtuko bus ignoruojama.} other {<b>{tabs} prisegtų</b> skirtukų bus ignoruojama.}}"
    },
    "clearData": {
      "title": "Valyti"
    },
    "cancel": {
      "title": "Atšaukti"
    },
    "historyAndDownloadsNotAffected": {
      "title": "Norėdami išvalyti istoriją, pasirinkite laikotarpį."
    }
  },
  "permissions": {
    "camera": {
      "title": "Fotoaparatas"
    },
    "microphone": {
      "title": "Mikrofonas"
    },
    "geolocation": {
      "title": "Geografinė padėtis"
    },
    "popups": {
      "title": "Iššokantieji langai"
    },
    "ask": {
      "title": "Klausti kiekvieną kartą"
    },
    "notify": {
      "title": "Pranešti"
    },
    "grant": {
      "title": "Visada leisti"
    },
    "deny": {
      "title": "Visada atmesti"
    }
  },
  "report": {
    "selectTheOptionDesc": {
      "title": "Anoniminio pranešimo apie neveikiančią svetainę pateikimas padeda mums tobulinti programėlę."
    },
    "selectTheOptionDescV2": {
      "title": "Pasirinkite parinktį, kuri geriausiai apibūdina iškilusią problemą."
    },
    "selectTheCategoryType": {
      "title": "Kokia problema?"
    },
    "categoryType1": {
      "title": "Svetainė neveikia taip, kaip tikėtasi"
    },
    "categoryType2": {
      "title": "Man nepatinka šios svetainės turinys"
    },
    "categoryType3": {
      "title": "Bendro pobūdžio atsiliepimas apie „DuckDuckGo“ naršyklę"
    },
    "selectTheCategory": {
      "title": "Kas neveikia šioje svetainėje?"
    },
    "tryTurningProtectionsOff": {
      "title": "Pabandykite išjungti privatumo apsaugą ir pažiūrėkite, ar tai išspręs problemą."
    },
    "skipThisStep": {
      "title": "Praleisti šį žingsnį"
    },
    "pickYourIssueFromTheList": {
      "title": "Aprašykite, kas atsitiko"
    },
    "blocked": {
      "title": "Svetainė užblokuota arba neįkelta"
    },
    "layout": {
      "title": "Svetainės išdėstymas sugadintas"
    },
    "emptySpaces": {
      "title": "Svetainėje yra didelių tuščių vietų"
    },
    "videos": {
      "title": "Vaizdo įrašas nebuvo paleistas arba įkeltas"
    },
    "images": {
      "title": "Vaizdai neįkelti"
    },
    "comments": {
      "title": "Nepavyko įkelti komentarų, apžvalgų ar pokalbių"
    },
    "content": {
      "title": "Trūksta turinio"
    },
    "links": {
      "title": "Nuorodos ar mygtukai neveikia"
    },
    "login": {
      "title": "Nepavyksta prisijungti / užsiregistruoti"
    },
    "loginV2": {
      "title": "Nepavyksta prisijungti arba užsiregistruoti"
    },
    "shopping": {
      "title": "Negalite sumokėti, atsiskaityti ar apsipirkti"
    },
    "browser": {
      "title": "Naršyklės ar kito naršyklės plėtinio problema"
    },
    "paywall": {
      "title": "Svetainė paprašė manęs išjungti reklamos blokavimo programą"
    },
    "other": {
      "title": "Kažkas kito"
    },
    "tellUsMoreDesc": {
      "title": "Pasidalinkite daugiau informacijos (neprivaloma):{bullet}Kas atsitiko?{bullet}Kas turėjo atsitikti?{bullet}Ar padėjo išjungus apsaugos priemones?"
    },
    "sendReport": {
      "title": "Siųsti ataskaitą"
    },
    "reportsAreAnonymousDesc": {
      "title": "„DuckDuckGo“ siunčiamose ataskaitose pateikiama tik ta informacija, kurios reikia, kad galėtume reaguoti į jūsų atsiliepimus."
    },
    "thankYou": {
      "title": "Ačiū!"
    },
    "yourReportWillHelpDesc": {
      "title": "Jūsų ataskaita padės patobulinti mūsų produktus ir pagerinti kitų naudotojų patirtį."
    },
    "dislike": {
      "title": "Man nepatinka turinys"
    },
    "otherRequired": {
      "title": "Aprašykite problemą, su kuria susidūrėte (privaloma)"
    },
    "otherOptional": {
      "title": "Aprašykite problemą, su kuria susidūrėte (pasirenkama)"
    },
    "suggestionWhatHappened": {
      "title": "Kas nutiko?"
    },
    "suggestionWhatHappened2": {
      "title": "Kas turėjo nutikti?"
    },
    "suggestionWhatHappened3": {
      "title": "Ar išjungus privatumo apsaugą problemos nebeliko?"
    },
    "reportTitle": {
      "title": "Praneškite „DuckDuckGo“"
    }
  },
  "shared": {
    "trackers": {
      "title": "Stebėjimo priemonės"
    }
  },
  "site": {
    "updatingProtectionList": {
      "title": "Apsaugos sąrašo atnaujinimas"
    },
    "protectionsEnabled": {
      "title": "Šios svetainės apsauga <b>ĮJUNGTA</b>"
    },
    "protectionsDisabled": {
      "title": "Šios svetainės apsauga <b>IŠJUNGTA</b>"
    },
    "protectionsDisabledRemote": {
      "title": "Laikinai išjungėme privatumo apsaugą, nes atrodo, kad ji trikdo šios svetainės veikimą."
    },
    "protectionsDisabledRemoteOverride": {
      "title": "Rekomenduojame išjungti šios svetainės privatumo apsaugą, kad svetainė veiktų tinkamai."
    },
    "connectionDescriptionUnencrypted": {
      "title": "<b>Ši svetainė nėra saugi</b> ir gali pakenkti bet kokiai informacijai, kurią siunčiate šiame puslapyje."
    },
    "connectionDescriptionInvalidCertificate": {
      "title": "<b>Šios svetainės sertifikatas negalioja.</b> Galbūt jungiatės prie serverio, kuris apsimeta <b>{domain}</b>, o tai gali kelti pavojų jūsų konfidencialiai informacijai."
    },
    "trackerNetworksSummaryBlocked": {
      "title": "Toliau nurodytų trečiųjų šalių domenų užklausų įkėlimas buvo užblokuotas, nes jos buvo identifikuotos kaip sekimo užklausos. Jei įmonės užklausos įkeliamos, ji gali leisti joms rinkti informaciją apie jus."
    },
    "trackerNetworksSummaryNoneBlocked": {
      "title": "Sekimo užklausos nebuvo užblokuotos šiame puslapyje. Jei įmonės užklausos įkeliamos, jos gali būti naudojamos informacijai apie jus rinkti."
    },
    "trackerNetworksSummaryNoneFound": {
      "title": "Šiame puslapyje nenustatėme jokių sekimo užklausų."
    },
    "trackerNetworksSummaryNone": {
      "title": "Neradome nė vienos įmonės, kuri šiame puslapyje bandytų įkelti sekimo užklausas."
    },
    "trackerNetworksSummaryAllowedOnly": {
      "title": "Kad svetainės veikimas nebūtų sutrikdytas, neužblokavome jokių įmonių, bandančių šiame puslapyje įkelti sekimo užklausas."
    },
    "trackerNetworksSummaryProtectionsOff": {
      "title": "Nebuvo užblokuotas jokių sekimo užklausų įkėlimas, nes šioje svetainėje apsauga yra išjungta. Jei įmonės užklausos įkeliamos, ji gali leisti joms rinkti informaciją apie jus."
    },
    "createNewDuckAddress": {
      "title": "Generuoti privatų „Duck“ adresą"
    },
    "createNewDuckAddressCopied": {
      "title": "Nukopijuota į iškarpinę!"
    },
    "websiteNotWorkingQ": {
      "title": "Svetainė neveikia taip, kaip tikėtasi?"
    },
    "websiteNotWorkingPrompt": {
      "title": "Svetainė neveikia?"
    },
    "websiteNotWorkingCta": {
      "title": "Pranešti apie sugadintą svetainę"
    },
    "websiteNotWorkingAdvice": {
      "title": "Gali padėti apsaugos priemonių išjungimas."
    },
    "websiteNotWorkingPromptFollowUp": {
      "title": "Svetainė vis dar neveikia?"
    },
    "takePrecautions": {
      "title": "Imkitės atsargumo priemonių"
    },
    "majorTrackingNetworkDesc": {
      "title": "{blocked, select, true {<b>Ši svetainė priklauso {companyDisplayName}</b>, kuri valdo sekimo tinklą {companyPrevalence} % geriausių svetainių. Mums pavyko užblokuoti kai kurias jų užklausas šiame puslapyje.} other {<b>Ši svetainė priklauso {companyDisplayName}</b>, kuri valdo sekimo tinklą {companyPrevalence} % geriausių svetainių. }}"
    },
    "trackersBlockedDesc": {
      "title": "{companyCount, plural, =0 {Užblokavome kai kurių bendrovių galimybę šiame puslapyje įkelti sekimo užklausas.} =2 {Užblokavome <b>{firstCompany}</b> ir <b>{secondCompany}</b> sekimo užklausų įkėlimą šiame puslapyje.} =3 {Užblokavome <b>{firstCompany}</b>, <b>{secondCompany}</b> ir <b>{thirdCompany}</b> sekimo užklausų įkėlimą šiame puslapyje.} =4 {Užblokavome <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b> ir <b>{fourthCompany}</b> sekimo užklausų įkėlimą šiame puslapyje.} =5 {Užblokavome <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b>, <b>{fourthCompany}</b> ir <b>dar 1</b> sekimo užklausų įkėlimą šiame puslapyje.} one {Užblokavome <b>{firstCompany}</b> sekimo užklausų įkėlimą šiame puslapyje.} few {Užblokavome <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b>, <b>{fourthCompany}</b> ir <b>dar {othersCount}</b> sekimo užklausų įkėlimą šiame puslapyje..} many {Užblokavome <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b>, <b>{fourthCompany}</b> ir <b>dar {othersCount}</b> sekimo užklausų įkėlimą šiame puslapyje..} other {Užblokavome <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b>, <b>{fourthCompany}</b> ir <b>dar {othersCount}</b> sekimo užklausų įkėlimą šiame puslapyje..}}"
    },
    "cookiesMinimized": {
      "title": "Slapukai valdomi"
    },
    "cookiesHidden": {
      "title": "Iškylantysis slapukų langas paslėptas"
    },
    "cookiesHiddenSummary": {
      "title": "Slapukų iškylantįjį langą šioje svetainėje galėjome paslėpti tik todėl, kad nepateikta jokių parinkčių slapukų nuostatoms tvarkyti. Kitos mūsų žiniatinklio stebėjimo apsaugos priemonės vis dar taikomos."
    },
    "cookiesMinimizedSummary": {
      "title": "Nustatėme slapukų nuostatas, kad būtų užtikrintas kuo didesnis privatumas, ir uždarėme slapukų iškylantįjį langą."
    },
    "cookiesMinimizedSettings": {
      "title": "Išjungti nustatymuose"
    },
    "connectionSecure": {
      "title": "Ryšys užšifruotas"
    },
    "connectionNotSecure": {
      "title": "Ryšys neužšifruotas"
    },
    "connectionNotSecureInvalidCertificate": {
      "title": "Ryšys gali būti nesaugus"
    },
    "trackerNetworksDesc": {
      "title": "Užklausos užblokuotos įkelti"
    },
    "trackerNetworksNotBlocked": {
      "title": "Neužblokuota jokių sekimo užklausų"
    },
    "trackerNetworksNotFound": {
      "title": "Sekimo užklausų nerasta"
    },
    "thirdPartiesLoaded": {
      "title": "Įkeltos trečiųjų šalių užklausos"
    },
    "thirdPartiesNoneFound": {
      "title": "Nėra trečiųjų šalių užklausų"
    },
    "firstPartyDesc": {
      "title": "{companyName} priklauso ši svetainė ir žinomos stebėjimo priemonės, rastos šiame puslapyje, todėl mes jų neužblokavome."
    },
    "noTrackersFound": {
      "title": "Neradome nė vienos įmonės, kuri šiame puslapyje bandytų įkelti sekimo užklausas."
    },
    "trackersFoundForAllowlisted": {
      "title": "Stebėjimo priemonės padeda įmonėms sudaryti jūsų profilį. Nustatėme, kad šios įmonės stebi jūsų veiklą šiame puslapyje."
    },
    "trackersFoundAndBlocked": {
      "title": "Stebėjimo priemonės padeda įmonėms sudaryti jūsų profilį. Užblokavome šių įmonių galimybę stebėti jūsų veiklą šiame puslapyje."
    },
    "trackerNetworkUnknown": {
      "title": "Stebėjimo priemonių tinklas nežinomas"
    },
    "trackerDomainsForCompany": {
      "title": "{companyName} stebėjimo priemonių domenai"
    },
    "zeroTrackersFound": {
      "title": "Rasta 0 stebėjimo priemonių"
    },
    "trackerOwnedByThisSite": {
      "title": "Šiai svetainei priklausančios stebėjimo priemonės (leidžiama)"
    },
    "trackerCountForDomain": {
      "title": "{blocked, select, true {{trackerCount, plural, one {{trackerCount} stebėjimo priemonė užblokuota {domain}} few {{trackerCount} stebėjimo priemonės užblokuotos {domain}} many {{trackerCount} stebėjimo priemonės užblokuota {domain}} other {{trackerCount} stebėjimo priemonių užblokuota {domain}}}} other {{trackerCount, plural, one {{trackerCount} stebėjimo priemonė rasta {domain}} few {{trackerCount} stebėjimo priemonės rastos {domain}} many {{trackerCount} stebėjimo priemonės rasta {domain}} other {{trackerCount} stebėjimo priemonių rasta {domain}}}}}"
    },
    "trackerLimitationsNote": {
      "title": "Atkreipkite dėmesį: platformos apribojimai gali riboti mūsų galimybes aptikti visas užklausas."
    },
    "trackerAboutLink": {
      "title": "Apie mūsų žiniatinklio stebėjimo apsaugą"
    },
    "trackerAdLink": {
      "title": "Kaip paieškos skelbimai veikia mūsų apsaugos priemones"
    },
    "sectionHeadingAdAttribution": {
      "title": "Šio domeno užklausos buvo įkeltos, nes neseniai „DuckDuckGo“ buvo spustelėta {domain} reklama. Šios užklausos padeda įvertinti reklamų efektyvumą. Visos „DuckDuckGo“ reklamos nerenka informacijos apie naudotoją."
    },
    "sectionHeadingIgnore": {
      "title": "Siekiant išvengti svetainės veikimo sutrikimų, buvo įkeltos šios domeno užklausos."
    },
    "sectionHeadingFirstParty": {
      "title": "Šios domeno užklausos buvo įkeltos, nes jos susijusios su {domain}."
    },
    "sectionHeadingThirdParty": {
      "title": "Taip pat buvo įkeltos šios domeno užklausos."
    },
    "sectionHeadingProtectionsDisabled": {
      "title": "Šios domeno užklausos buvo įkeltos, nes apsaugos yra išjungtos."
    },
    "thirdPartiesSummaryLoaded": {
      "title": "Buvo įkeltos šios trečiųjų šalių domenų užklausos. Jei įmonės užklausos įkeliamos, ji gali leisti joms rinkti informaciją apie jus, nors kitos mūsų žiniatinklio sekimo apsaugos priemonės tebegalioja."
    },
    "thirdPartiesSummaryProtectionsOff": {
      "title": "Buvo įkeltos šios trečiųjų šalių domenų užklausos. Jei įmonės užklausos įkeliamos, ji gali leisti joms rinkti informaciją apie jus, nors kitos mūsų žiniatinklio sekimo apsaugos priemonės tebegalioja."
    },
    "thirdPartiesSummaryNone": {
      "title": "Nenustatėme jokių užklausų iš trečiųjų šalių domenų."
    },
    "analyticsCategory": {
      "title": "Analizė"
    },
    "advertisingCategory": {
      "title": "Reklaminis"
    },
    "socialCategory": {
      "title": "Socialinis tinklas"
    },
    "contentDeliveryCategory": {
      "title": "Turinio pateikimas"
    },
    "embeddedContentCategory": {
      "title": "Įdėtasis turinys"
    },
    "searchPlaceholder": {
      "title": "Ieškoti DuckDuckGo"
    },
    "searchGoButton": {
      "title": "Paieška"
    },
    "optionsButton": {
      "title": "Daugiau parinkčių"
    },
    "navigationComplete": {
      "title": "Atlikta"
    },
    "navigationClose": {
      "title": "Uždaryti"
    },
    "navigationCancel": {
      "title": "Atšaukti"
    },
    "navigationBack": {
      "title": "Atgal"
    },
    "enableProtectionsSwitch": {
      "title": "Įjungti apsaugos priemones"
    },
    "disableProtectionsSwitch": {
      "title": "Išjungti apsaugos priemones"
    },
    "errorMessage": {
      "title": "Kažkas nepavyko ir negalėjome įkelti šio turinio. Pabandykite iš naujo įkelti puslapį."
    }
  }
}