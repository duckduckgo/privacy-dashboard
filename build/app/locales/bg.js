export default {
  "connection": {
    "encrypt": {
      "title": "Криптиране"
    },
    "sign": {
      "title": "Подписване"
    },
    "verify": {
      "title": "Проверка"
    },
    "derive": {
      "title": "Извличане"
    },
    "wrap": {
      "title": "Архивиране"
    },
    "unwrap": {
      "title": "Разархивиране"
    },
    "unknown": {
      "title": "Неизвестен"
    },
    "certificateDetail": {
      "title": "Подробности за сертификата за сигурност"
    },
    "commonName": {
      "title": "Общоприето име"
    },
    "publicKey": {
      "title": "Публичен ключ"
    },
    "algorithm": {
      "title": "Алгоритъм"
    },
    "keySize": {
      "title": "Размер на ключа"
    },
    "usage": {
      "title": "Употреба"
    },
    "summary": {
      "title": "Резюме"
    },
    "permanent": {
      "title": "Постоянен"
    },
    "effectiveSize": {
      "title": "Ефективен размер"
    },
    "certificateNotFound": {
      "title": "Не е намерен сертификат"
    },
    "certificateForDomain": {
      "title": "Сертификат за {domain}"
    },
    "insecureConnectionDesc": {
      "title": "Тази страница използва некриптирана връзка. Възможно е трети страни да могат да виждат дейността Ви или да прихващат поверителна информация, която изпращате на тази страница."
    },
    "invalidConnectionDesc": {
      "title": "Сертификатът за този сайт е невалиден. Възможно е да се свързвате със сървър, който се представя за <b>{domain}</b>, което може да изложи поверителната Ви информация на риск."
    },
    "upgradedConnectionDesc": {
      "title": "Надградихме връзката на тази страница, за да защитим изпращаната от Вас информация по време на преноса."
    },
    "secureConnectionDesc": {
      "title": "Тази страница използва криптирана връзка, която не позволява на трети страни да виждат дейността Ви или да прихващат поверителна информация, която изпращате на тази страница."
    }
  },
  "ctascreens": {
    "protectionsUnavailableNote": {
      "title": "Защитите на поверителността не са налични за специални или локални страници."
    },
    "spreadTitle": {
      "title": "Харесва ли Ви да използвате DuckDuckGo?"
    },
    "spreadText": {
      "title": "Споделете това с Вашето семейство и приятели"
    },
    "spreadButton": {
      "title": "Разпространение на DuckDuckGo"
    },
    "emailTitle": {
      "title": "Омръзна ли Ви имейлите да бъдат проследявани?"
    },
    "emailText": {
      "title": "Регистрирайте се за защита на имейли на DuckDuckGo за Вашето разширение сега!"
    }
  },
  "firebutton": {
    "fireDialogHeader": {
      "title": "Затваряне на разделите и изчистване на данните"
    },
    "fireDialogHeaderNoTabs": {
      "title": "Изчистване на данни"
    },
    "optionCurrentSite": {
      "title": "Само за текущия сайт"
    },
    "optionLastHour": {
      "title": "Последния час"
    },
    "optionLast24Hour": {
      "title": "Последните 24 часа"
    },
    "optionLast7days": {
      "title": "Последните 7 дни"
    },
    "optionLast4Weeks": {
      "title": "Последните 4 седмици"
    },
    "optionAllTime": {
      "title": "През цялото време"
    },
    "historyDuration": {
      "title": "{duration, select, hour {един час} day {24 часа} week {една седмица} month {4 седмици} other {Всички}}"
    },
    "summaryClearTabsHistoryDuration": {
      "title": "{openTabs, plural, one {Затваряне на <b>{openTabs}</b> раздел и изчистване на <b>{durationDesc}</b> от историята на сърфиране и бисквитките?} other {Затваряне на <b>{openTabs}</b> раздела и изчистване на <b>{durationDesc}</b> от историята на сърфиране и бисквитките?}}"
    },
    "summaryClearTabsDuration": {
      "title": "{openTabs, plural, one {Затваряне на <b>{openTabs}</b> раздел и изчистване на <b>{durationDesc}</b> от бисквитките?} other {Затваряне на <b>{openTabs}</b> раздела и изчистване на <b>{durationDesc}</b> от бисквитките?}}"
    },
    "summaryClearHistoryDuration": {
      "title": "Изчистване на <b>{durationDesc}</b> от историята на сърфиране и бисквитките?"
    },
    "summaryClearCookiesDuration": {
      "title": "Изчистване на <b>{durationDesc}</b> от бисквитките?"
    },
    "summaryClearTabsHistoryAll": {
      "title": "Затваряне на <b>{openTabs}</b> {openTabs, plural, =1 {раздел} other {раздела}} и изчистване на <b>цялата</b> история на сърфиране и всички бисквитки ({cookies} {cookies, plural, =1 {сайт} other {сайта}})?"
    },
    "summaryClearTabsAll": {
      "title": "Затваряне на <b>{openTabs}</b> {openTabs, plural, =1 {раздел} other {раздела}} и изчистване на <b>всички</b> бисквитки ({cookies} {cookies, plural, =1 {сайта} other {сайта}})?"
    },
    "summaryClearHistoryAll": {
      "title": "{cookies, plural, one {Изчистване на <b>цялата</b> история на сърфиране и бисквитките ({cookies} сайт)?} other {Изчистване на <b>цялата</b> история на сърфиране и бисквитките ({cookies} сайта)?}}"
    },
    "summaryClearCookiesAll": {
      "title": "{cookies, plural, one {Изчистване на <b>всички</b> бисквитки ({cookies} сайт)?} other {Изчистване на <b>всички</b> бисквитки ({cookies} сайта)?}}"
    },
    "summaryClearTabsHistorySite": {
      "title": "{openTabs, plural, one {Затваряне на <b>{openTabs} раздел {site}</b> и изчистване на <b>всички бисквитки за {site}</b>?} other {Затваряне на <b>{openTabs} раздела {site}</b> и изчистване на <b>всички бисквитки за {site}</b>?}}"
    },
    "summaryClearTabsSite": {
      "title": "{openTabs, plural, one {Затваряне на <b>{openTabs} раздел {site}</b> и изчистване на <b>всички бисквитки за {site}</b>?} other {Затваряне на <b>{openTabs} раздела {site}</b> и изчистване на <b>всички бисквитки за {site}</b>?}}"
    },
    "summaryClearHistorySite": {
      "title": "Изчистване на <b>цялата история на сърфиране и всички бисквитки за {site}</b>?"
    },
    "summaryClearCookiesSite": {
      "title": "Изчистване на <b>всички бисквитки за {site}</b>?"
    },
    "summaryPinnedIgnored": {
      "title": "{tabs, plural, one {<b>{tabs} закачен</b> раздел ще бъде игнориран.} other {<b>{tabs} закачени</b> раздела ще бъдат игнорирани.}}"
    },
    "clearData": {
      "title": "Ясно"
    },
    "cancel": {
      "title": "Отмени"
    },
    "historyAndDownloadsNotAffected": {
      "title": "За да изчистите и историята, изберете период от време."
    }
  },
  "permissions": {
    "camera": {
      "title": "Камера"
    },
    "microphone": {
      "title": "Микрофон"
    },
    "geolocation": {
      "title": "Местоположение"
    },
    "popups": {
      "title": "Изскачащи прозорци"
    },
    "ask": {
      "title": "Питай всеки път"
    },
    "notify": {
      "title": "Уведоми ме"
    },
    "grant": {
      "title": "Винаги разрешено"
    },
    "deny": {
      "title": "Винаги отказано"
    }
  },
  "report": {
    "selectTheOptionDesc": {
      "title": "Изпращането на анонимен доклад за неработещ сайт ни помага да подобрим приложението."
    },
    "selectTheOptionDescV2": {
      "title": "Изберете опцията, която най-добре описва възникналия проблем."
    },
    "selectTheCategoryType": {
      "title": "Какъв е проблемът?"
    },
    "categoryType1": {
      "title": "Сайтът не работи според очакванията"
    },
    "categoryType2": {
      "title": "Не харесвам съдържанието на този сайт"
    },
    "categoryType3": {
      "title": "Цялостна обратна връзка за браузъра DuckDuckGo"
    },
    "selectTheCategory": {
      "title": "Какво не работи в този сайт?"
    },
    "tryTurningProtectionsOff": {
      "title": "Опитайте да изключите защитата на поверителността и проверете дали това ще реши проблема."
    },
    "skipThisStep": {
      "title": "Пропускане на тази стъпка"
    },
    "pickYourIssueFromTheList": {
      "title": "Опишете проблема"
    },
    "blocked": {
      "title": "Сайтът блокира или не се зарежда"
    },
    "layout": {
      "title": "Оформлението на сайта не е както трябва"
    },
    "emptySpaces": {
      "title": "Сайтът съдържа големи празни пространства"
    },
    "videos": {
      "title": "Видеото не се възпроизвежда или зарежда"
    },
    "images": {
      "title": "Изображенията не се зареждат"
    },
    "comments": {
      "title": "Коментарите, отзивите или чатовете не се зареждат"
    },
    "content": {
      "title": "Липсва съдържание"
    },
    "links": {
      "title": "Връзките или бутоните не работят"
    },
    "login": {
      "title": "Не мога да вляза/да се регистрирам"
    },
    "loginV2": {
      "title": "Не мога да вляза или да се регистрирам"
    },
    "shopping": {
      "title": "Не мога да платя, да завърша покупката или да пазарувам"
    },
    "browser": {
      "title": "Проблем с браузъра или друго разширение на браузъра"
    },
    "paywall": {
      "title": "Сайтът иска да деактивирам инструмента за блокиране на реклами"
    },
    "other": {
      "title": "Нещо друго"
    },
    "tellUsMoreDesc": {
      "title": "Споделете повече подробности (по избор):{bullet}Какво се случи?{bullet}Какво трябваше да се случи?{bullet}Изключването на защитите помогна ли?"
    },
    "sendReport": {
      "title": "Изпращане на доклад"
    },
    "reportsAreAnonymousDesc": {
      "title": "Докладите, изпращани до DuckDuckGo, включват само информацията, необходима за обработка с Вашите отзиви."
    },
    "thankYou": {
      "title": "Благодарим Ви!"
    },
    "yourReportWillHelpDesc": {
      "title": "Вашият доклад ще помогне за подобряването на нашите продукти и изживяването на останалите потребители."
    },
    "dislike": {
      "title": "Не харесвам съдържанието"
    },
    "otherRequired": {
      "title": "Моля, опишете проблема, който сте срещнали (задължително)"
    },
    "otherOptional": {
      "title": "Моля, опишете проблема, който сте срещнали (по избор)"
    },
    "suggestionWhatHappened": {
      "title": "Какво се случи?"
    },
    "suggestionWhatHappened2": {
      "title": "Какво е трябваше да се случи?"
    },
    "suggestionWhatHappened3": {
      "title": "Изключването на защитата на поверителността помогна ли?"
    },
    "reportTitle": {
      "title": "Докладване на DuckDuckGo"
    }
  },
  "shared": {
    "trackers": {
      "title": "Тракери"
    }
  },
  "site": {
    "updatingProtectionList": {
      "title": "Актуализиране на списъка за защита"
    },
    "protectionsEnabled": {
      "title": "Защитите за този сайт са <b>ВКЛЮЧЕНИ</b>"
    },
    "protectionsDisabled": {
      "title": "Защитите за този сайт са <b>ИЗКЛЮЧЕНИ</b>"
    },
    "protectionsDisabledRemote": {
      "title": "Изключихме временно защитите на поверителността, защото изглежда, че пречат на зареждането на този сайт."
    },
    "protectionsDisabledRemoteOverride": {
      "title": "Препоръчваме да деактивирате защитите на поверителността за този сайт, за да предотвратите срив на сайта."
    },
    "connectionDescriptionUnencrypted": {
      "title": "<b>Този сайт не е защитен</b> и може да компрометира информацията, която изпращате на тази страница."
    },
    "connectionDescriptionInvalidCertificate": {
      "title": "<b>Сертификатът за този сайт е невалиден</b>. Възможно е да се свързвате със сървър, който се представя за <b>{domain}</b>, което може да изложи поверителната Ви информация на риск."
    },
    "trackerNetworksSummaryBlocked": {
      "title": "Блокирано е зареждането на заявки от следните домейни на трети страни, тъй като са идентифицирани като заявки с проследяване. Ако бъдат заредени заявки от дадена компания, те могат да ѝ позволят да Ви профилира."
    },
    "trackerNetworksSummaryNoneBlocked": {
      "title": "Няма блокирано зареждане на заявки с проследяване на тази страница. Ако бъдат заредени заявки от дадена компания, те могат да ѝ позволят да Ви профилира."
    },
    "trackerNetworksSummaryNoneFound": {
      "title": "Не идентифицирахме заявки с проследяване на тази страница."
    },
    "trackerNetworksSummaryNone": {
      "title": "Не открихме компании, които да се опитват да зареждат заявки с проследяване на тази страница."
    },
    "trackerNetworksSummaryAllowedOnly": {
      "title": "За да предотвратим срив на сайта, не сме блокирали зареждането на заявки с проследяване на тази страница от други компании."
    },
    "trackerNetworksSummaryProtectionsOff": {
      "title": "Няма блокирано зареждане на заявки с проследяване, тъй като защитите за този сайт са изключени. Ако бъдат заредени заявки от дадена компания, те могат да ѝ позволят да Ви профилира."
    },
    "createNewDuckAddress": {
      "title": "Генериране на личен Duck Address"
    },
    "createNewDuckAddressCopied": {
      "title": "Копиран в клипборда!"
    },
    "websiteNotWorkingQ": {
      "title": "Уебсайтът не работи според очакванията?"
    },
    "websiteNotWorkingPrompt": {
      "title": "Уебсайтът не работи?"
    },
    "websiteNotWorkingCta": {
      "title": "Подаване на сигнал за повреден сайт"
    },
    "websiteNotWorkingAdvice": {
      "title": "Изключването на защитите може да помогне."
    },
    "websiteNotWorkingPromptFollowUp": {
      "title": "Сайтът продължава да не работи?"
    },
    "takePrecautions": {
      "title": "Вземете предпазни мерки"
    },
    "majorTrackingNetworkDesc": {
      "title": "{blocked, select, true {<b>Този сайт е собственост на {companyDisplayName}</b> – компания, която управлява мрежа от тракери в {companyPrevalence}% от водещите уебсайтове. Успяхме да блокираме някои от техните заявки на тази страница.} other {<b>Този сайт е собственост на {companyDisplayName}</b> – компания, която управлява мрежа от тракери в {companyPrevalence}% от водещите уебсайтове. }}"
    },
    "trackersBlockedDesc": {
      "title": "{companyCount, plural, =0 {Блокирахме зареждането на заявки за проследяване на тази страница от няколко компании.} =2 {Блокирахме зареждането на заявки с проследяване на тази страница от <b>{firstCompany}</b> и <b>{secondCompany}</b>.} =3 {Блокирахме зареждането на заявки с проследяване на тази страница от <b>{firstCompany}</b>, <b>{secondCompany}</b> и <b>{thirdCompany}</b>.} =4 {Блокирахме зареждането на заявки с проследяване на тази страница от <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b> и <b>{fourthCompany}</b> .} =5 {Блокирахме зареждането на заявки с проследяване на тази страница от <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b>, <b>{fourthCompany}</b> и <b>още 1 друга компания</b>.} one {Блокирахме зареждането на заявки с проследяване на тази страница от <b>{firstCompany}</b>.} other {Блокирахме зареждането на заявки с проследяване на тази страница от <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b>, <b>{fourthCompany}</b> и още <b>{othersCount} други компании</b>.}}"
    },
    "cookiesMinimized": {
      "title": "Бисквитките са настроени"
    },
    "cookiesHidden": {
      "title": "Изскачащият прозорец за бисквитки е скрит"
    },
    "cookiesHiddenSummary": {
      "title": "Успяхме само да скрием изскачащия прозорец за бисквитките на този сайт, тъй като не бяха предоставени опции за управление на предпочитанията за бисквитките. Останалите ни защити срещу проследяване в мрежата все още са в сила."
    },
    "cookiesMinimizedSummary": {
      "title": "Настроихме предпочитанията Ви за бисквитките, за да увеличим поверителността, и затворихме изскачащия прозорец."
    },
    "cookiesMinimizedSettings": {
      "title": "Деактивиране в Настройки"
    },
    "connectionSecure": {
      "title": "Връзката е криптирана"
    },
    "connectionNotSecure": {
      "title": "Връзката не е криптирана"
    },
    "connectionNotSecureInvalidCertificate": {
      "title": "Връзката може да не е сигурна"
    },
    "trackerNetworksDesc": {
      "title": "Блокирано зареждане на заявки"
    },
    "trackerNetworksNotBlocked": {
      "title": "Няма блокирани заявки с проследяване"
    },
    "trackerNetworksNotFound": {
      "title": "Няма открити заявки с проследяване"
    },
    "thirdPartiesLoaded": {
      "title": "Заредени заявки от трети страни"
    },
    "thirdPartiesNoneFound": {
      "title": "Не са намерени заявки от трети страни"
    },
    "firstPartyDesc": {
      "title": "{companyName} е собственик на този сайт и на установените тракери, които се намират на тази страница, затова не ги блокирахме."
    },
    "noTrackersFound": {
      "title": "Не открихме компании, които да се опитват да зареждат заявки с проследяване на тази страница."
    },
    "trackersFoundForAllowlisted": {
      "title": "Тракерите се използват от компаниите за профилиране. Открихме, че тези компании следят Вашата дейност на тази страница."
    },
    "trackersFoundAndBlocked": {
      "title": "Тракерите се използват от компаниите за профилиране. Блокирахме тези компании, които следят Вашата дейност на тази страница."
    },
    "trackerNetworkUnknown": {
      "title": "Проследяващата мрежа е неизвестна"
    },
    "trackerDomainsForCompany": {
      "title": "Домейни на тракери за {companyName}"
    },
    "zeroTrackersFound": {
      "title": "0 открити тракери"
    },
    "trackerOwnedByThisSite": {
      "title": "Тракери, притежавани от този сайт (разрешени)"
    },
    "trackerCountForDomain": {
      "title": "{blocked, select, true {{trackerCount, plural, one {Блокиран е {trackerCount} тракер на {domain}} other {Блокирани са {trackerCount} тракера на {domain}}}} other {{trackerCount, plural, one {Открит е {trackerCount} тракер на {domain}} other {Открити са {trackerCount} тракера на {domain}}}}}"
    },
    "trackerLimitationsNote": {
      "title": "Моля, имайте предвид, че ограниченията на платформата могат да намалят способността ни да откриваме всички заявки."
    },
    "trackerAboutLink": {
      "title": "Относно нашите защити срещу проследяване в мрежата"
    },
    "trackerAdLink": {
      "title": "Какво влияние оказват нашите реклами при търсене върху нашите защити"
    },
    "sectionHeadingAdAttribution": {
      "title": "Заредени са заявки от следния домейн, защото наскоро сте щракнали върху реклама на {domain} в DuckDuckGo. Тези заявки помагат за оценка на ефективността на рекламата. Рекламите в DuckDuckGo не са профилиращи."
    },
    "sectionHeadingIgnore": {
      "title": "Заредени са заявки от следните домейни, за да се предотврати срив на сайта."
    },
    "sectionHeadingFirstParty": {
      "title": "Заредени са заявки от следните домейни, защото са свързани с [{domain}]."
    },
    "sectionHeadingThirdParty": {
      "title": "Заредени са също и заявки от следните домейни."
    },
    "sectionHeadingProtectionsDisabled": {
      "title": "Заредени са заявки от следните домейни, защото защитите са изключени."
    },
    "thirdPartiesSummaryLoaded": {
      "title": "Заредени са заявки от следните домейни на трети страни. Ако бъдат заредени заявки от дадена компания, те могат да ѝ позволят да Ви профилира, въпреки че останалите ни защити срещу проследяване в мрежата все още се прилагат."
    },
    "thirdPartiesSummaryProtectionsOff": {
      "title": "Заредени са заявки от следните домейни на трети страни. Ако бъдат заредени заявки от дадена компания, те могат да ѝ позволят да Ви профилира, въпреки че останалите ни защити срещу проследяване в мрежата все още се прилагат."
    },
    "thirdPartiesSummaryNone": {
      "title": "Не идентифицирахме заявки от домейни на трети страни."
    },
    "analyticsCategory": {
      "title": "Анализ"
    },
    "advertisingCategory": {
      "title": "Реклама"
    },
    "socialCategory": {
      "title": "Социална мрежа"
    },
    "contentDeliveryCategory": {
      "title": "Доставка на съдържание"
    },
    "embeddedContentCategory": {
      "title": "Вградено съдържание"
    },
    "searchPlaceholder": {
      "title": "Търси с DuckDuckGo"
    },
    "searchGoButton": {
      "title": "Търсене"
    },
    "optionsButton": {
      "title": "Още опции"
    },
    "navigationComplete": {
      "title": "Готово"
    },
    "navigationClose": {
      "title": "Затваряне"
    },
    "navigationCancel": {
      "title": "Отмени"
    },
    "navigationBack": {
      "title": "Обратно"
    },
    "enableProtectionsSwitch": {
      "title": "Активиране на защити"
    },
    "disableProtectionsSwitch": {
      "title": "Деактивиране на защити"
    },
    "errorMessage": {
      "title": "Възникна проблем и не успяхме да заредим това съдържание. Опитайте да презаредите страницата."
    }
  }
}