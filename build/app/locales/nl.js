export default {
  "connection": {
    "encrypt": {
      "title": "Versleutelen"
    },
    "sign": {
      "title": "Tekenen"
    },
    "verify": {
      "title": "Verifiëren"
    },
    "derive": {
      "title": "Afleiden"
    },
    "wrap": {
      "title": "Inpakken"
    },
    "unwrap": {
      "title": "Uitpakken"
    },
    "unknown": {
      "title": "Onbekend"
    },
    "certificateDetail": {
      "title": "Detail van veiligheidscertificaat"
    },
    "commonName": {
      "title": "Algemene naam"
    },
    "publicKey": {
      "title": "Openbare sleutel"
    },
    "algorithm": {
      "title": "Algoritme"
    },
    "keySize": {
      "title": "Sleutelgrootte"
    },
    "usage": {
      "title": "Gebruik"
    },
    "summary": {
      "title": "Overzicht"
    },
    "permanent": {
      "title": "Permanent"
    },
    "effectiveSize": {
      "title": "Effectieve grootte"
    },
    "certificateNotFound": {
      "title": "Certificaat niet gevonden"
    },
    "certificateForDomain": {
      "title": "Certificaat voor {domain}"
    },
    "insecureConnectionDesc": {
      "title": "Deze pagina gebruikt een niet-versleutelde verbinding. Mogelijk kunnen externe partijen je activiteit bekijken of gevoelige informatie onderscheppen die je op deze pagina verzendt."
    },
    "invalidConnectionDesc": {
      "title": "Het certificaat voor deze website is ongeldig. Misschien maak je verbinding met een server die zich voordoet als <b>{domain}</b>, waardoor je vertrouwelijke informatie in gevaar kan komen."
    },
    "upgradedConnectionDesc": {
      "title": "Wij hebben de verbinding op deze pagina geüpgraded om de informatie die je verzendt te beschermen tijdens de overdracht."
    },
    "secureConnectionDesc": {
      "title": "Deze pagina maakt gebruik van een versleutelde verbinding, waardoor externe partijen je activiteit niet kunnen bekijken en gevoelige informatie die je op deze pagina verzendt niet kunnen onderscheppen."
    }
  },
  "ctascreens": {
    "protectionsUnavailableNote": {
      "title": "Privacybescherming is niet beschikbaar voor speciale pagina's of lokale pagina's."
    },
    "spreadTitle": {
      "title": "Gebruik je DuckDuckGo graag?"
    },
    "spreadText": {
      "title": "Laat het weten aan je familie en vrienden"
    },
    "spreadButton": {
      "title": "DuckDuckGo verspreiden"
    },
    "emailTitle": {
      "title": "Ben je het beu om te worden gevolgd in e-mails?"
    },
    "emailText": {
      "title": "Meld u nu aan voor DuckDuckGo Email Protection om gebruik te maken van onze extensie!"
    }
  },
  "firebutton": {
    "fireDialogHeader": {
      "title": "Tabbladen sluiten en gegevens wissen"
    },
    "fireDialogHeaderNoTabs": {
      "title": "Gegevens wissen"
    },
    "optionCurrentSite": {
      "title": "Alleen huidige site"
    },
    "optionLastHour": {
      "title": "Afgelopen uur"
    },
    "optionLast24Hour": {
      "title": "Afgelopen 24 uur"
    },
    "optionLast7days": {
      "title": "Afgelopen 7 dagen"
    },
    "optionLast4Weeks": {
      "title": "Afgelopen 4 weken"
    },
    "optionAllTime": {
      "title": "Altijd"
    },
    "historyDuration": {
      "title": "{duration, select, hour {een uur} day {24 uur} week {een week} month {4 weken} other {Alle}}"
    },
    "summaryClearTabsHistoryDuration": {
      "title": "{openTabs, plural, one {<b>{openTabs}</b> tabblad sluiten en je browsegeschiedenis en cookies van <b>{durationDesc}</b> wissen?} other {<b>{openTabs}</b> tabbladen sluiten en je browsegeschiedenis en cookies van <b>{durationDesc}</b> wissen?}}"
    },
    "summaryClearTabsDuration": {
      "title": "{openTabs, plural, one {<b>{openTabs}</b> tabblad sluiten en cookies van <b>{durationDesc}</b> wissen?} other {<b>{openTabs}</b> tabbladen sluiten en cookies van <b>{durationDesc}</b> wissen?}}"
    },
    "summaryClearHistoryDuration": {
      "title": "Browsegeschiedenis en cookies van <b>{durationDesc}</b> wissen?"
    },
    "summaryClearCookiesDuration": {
      "title": "Cookies van <b>{durationDesc}</b> wissen?"
    },
    "summaryClearTabsHistoryAll": {
      "title": "<b>{openTabs}</b> {openTabs, plural, =1 {tab} other {tabs}} sluiten en <b>alle</b> browsegeschiedenis en cookies ({cookies} {cookies, plural, =1 {site} other {sites}}) wissen?"
    },
    "summaryClearTabsAll": {
      "title": "<b>{openTabs}</b> {openTabs, plural, =1 {tab} other {tabs}} sluiten en <b>alle</b> cookies ({cookies} {cookies, plural, =1 {site} other {sites}}) wissen?"
    },
    "summaryClearHistoryAll": {
      "title": "{cookies, plural, one {<b>Alle</b> browsegeschiedenis en cookies wissen ({cookies} site)?} other {Alle <b>browsegeschiedenis</b> en cookies wissen ({cookies} sites)?}}"
    },
    "summaryClearCookiesAll": {
      "title": "{cookies, plural, one {<b>Alle</b> cookies ({cookies} site) wissen?} other {<b>Alle</b> cookies ({cookies} sites) wissen?}}"
    },
    "summaryClearTabsHistorySite": {
      "title": "{openTabs, plural, one {<b>{openTabs} {site}</b> tabblad sluiten en <b>alle</b> cookies van {site} wissen?} other {<b>{openTabs} {site}</b> tabbladen sluiten en <b>alle</b> cookies van {site} wissen?}}"
    },
    "summaryClearTabsSite": {
      "title": "{openTabs, plural, one {<b>{openTabs} {site}</b> tabblad sluiten en <b>alle</b> cookies van {site} wissen?} other {<b>{openTabs} {site}</b> tabbladen sluiten en <b>alle</b> cookies van {site} wissen?}}"
    },
    "summaryClearHistorySite": {
      "title": "<b>Alle</b> browsegeschiedenis en cookies van {site} wissen?"
    },
    "summaryClearCookiesSite": {
      "title": "<b>Alle</b> cookies van {site} wissen?"
    },
    "summaryPinnedIgnored": {
      "title": "{tabs, plural, one {<b>{tabs} vastgemaakt</b> tabblad wordt genegeerd.} other {<b>{tabs} vastgemaakte</b> tabbladen worden genegeerd.}}"
    },
    "clearData": {
      "title": "Helder"
    },
    "cancel": {
      "title": "Annuleren"
    },
    "historyAndDownloadsNotAffected": {
      "title": "Selecteer een periode om de geschiedenis te wissen."
    }
  },
  "permissions": {
    "camera": {
      "title": "Camera"
    },
    "microphone": {
      "title": "Microfoon"
    },
    "geolocation": {
      "title": "Geolocatie"
    },
    "popups": {
      "title": "Pop-ups"
    },
    "ask": {
      "title": "Elke keer vragen"
    },
    "notify": {
      "title": "Informeren"
    },
    "grant": {
      "title": "Altijd toestaan"
    },
    "deny": {
      "title": "Altijd weigeren"
    }
  },
  "report": {
    "selectTheOptionDesc": {
      "title": "Als je een anoniem rapport over een kapotte site indient, helpt dat ons om de app te verbeteren."
    },
    "selectTheOptionDescV2": {
      "title": "Selecteer de optie die het beste je probleem beschrijft."
    },
    "selectTheCategoryType": {
      "title": "Wat is het probleem?"
    },
    "categoryType1": {
      "title": "De site werkt niet zoals verwacht"
    },
    "categoryType2": {
      "title": "Ik vind de inhoud op deze site niet leuk"
    },
    "categoryType3": {
      "title": "Algemene feedback over de DuckDuckGo-browser"
    },
    "selectTheCategory": {
      "title": "Wat werkt er niet op deze site?"
    },
    "tryTurningProtectionsOff": {
      "title": "Probeer de privacybescherming uit te schakelen om te zien of dat het probleem oplost."
    },
    "skipThisStep": {
      "title": "Deze stap overslaan"
    },
    "pickYourIssueFromTheList": {
      "title": "Beschrijf wat er is gebeurd"
    },
    "blocked": {
      "title": "Site geblokkeerd of niet geladen"
    },
    "layout": {
      "title": "Site wordt niet goed vormgegeven"
    },
    "emptySpaces": {
      "title": "Site bevat grote lege ruimtes"
    },
    "videos": {
      "title": "Video niet afgespeeld of geladen"
    },
    "images": {
      "title": "Afbeeldingen zijn niet geladen"
    },
    "comments": {
      "title": "Reacties, recensies of chats niet geladen"
    },
    "content": {
      "title": "Inhoud ontbreekt"
    },
    "links": {
      "title": "Links of knoppen werken niet"
    },
    "login": {
      "title": "Aanmelden/registreren niet mogelijk"
    },
    "loginV2": {
      "title": "Aanmelden/registreren niet mogelijk"
    },
    "shopping": {
      "title": "Betalen, afrekenen of winkelen niet mogelijk"
    },
    "browser": {
      "title": "Probleem met browser of andere browserextensie"
    },
    "paywall": {
      "title": "De site vroeg me om de adblocker uit te schakelen"
    },
    "other": {
      "title": "Iets anders"
    },
    "tellUsMoreDesc": {
      "title": "Meer details delen (optioneel):{bullet}Wat is er gebeurd?{bullet}Wat had er moeten gebeuren?{bullet} Hielp het uitschakelen van de beveiliging?"
    },
    "sendReport": {
      "title": "Verzend Rapport"
    },
    "reportsAreAnonymousDesc": {
      "title": "Rapporten die naar DuckDuckGo worden verzonden, bevatten alleen de info die nodig is om ons te helpen je feedback te verwerken."
    },
    "thankYou": {
      "title": "Dankjewel!"
    },
    "yourReportWillHelpDesc": {
      "title": "Je rapport helpt ons om onze producten én de ervaring voor anderen te verbeteren."
    },
    "dislike": {
      "title": "Ik vind de inhoud niet leuk"
    },
    "otherRequired": {
      "title": "Beschrijf het probleem dat je hebt ondervonden (verplicht)"
    },
    "otherOptional": {
      "title": "Beschrijf het probleem dat je hebt ondervonden (optioneel)"
    },
    "suggestionWhatHappened": {
      "title": "Wat is er gebeurd?"
    },
    "suggestionWhatHappened2": {
      "title": "Wat had er moeten gebeuren?"
    },
    "suggestionWhatHappened3": {
      "title": "Heeft het uitschakelen van privacybescherming geholpen?"
    },
    "reportTitle": {
      "title": "Rapporteren aan DuckDuckGo"
    }
  },
  "shared": {
    "trackers": {
      "title": "Trackers"
    }
  },
  "site": {
    "updatingProtectionList": {
      "title": "Beveiligingslijst updaten"
    },
    "protectionsEnabled": {
      "title": "Beveiliging is <b>INGESCHAKELD</b> voor deze website"
    },
    "protectionsDisabled": {
      "title": "Beveiliging is <b>UITGESCHAKELD</b> voor deze website"
    },
    "protectionsDisabledRemote": {
      "title": "We hebben privacybescherming tijdelijk uitgeschakeld, omdat het erop lijkt dat deze site hierdoor niet goed meer werkt."
    },
    "protectionsDisabledRemoteOverride": {
      "title": "We raden aan om Privacybescherming voor deze site uit te schakelen om te voorkomen dat de site niet meer goed werkt."
    },
    "connectionDescriptionUnencrypted": {
      "title": "<b>Deze site is niet veilig</b> en kan alle informatie die je op deze pagina verzendt, in gevaar brengen."
    },
    "connectionDescriptionInvalidCertificate": {
      "title": "<b>Het certificaat voor deze website is ongeldig.</b> Mogelijk maak je verbinding met een server die zich voordoet als <b>{domain}</b>, waardoor je vertrouwelijke informatie in gevaar kan komen."
    },
    "trackerNetworksSummaryBlocked": {
      "title": "De volgende verzoeken van domeinen van derden werden geblokkeerd omdat ze werden geïdentificeerd als verzoeken met tracking. Als de verzoeken van een bedrijf worden geladen, kunnen ze een profiel van je maken."
    },
    "trackerNetworksSummaryNoneBlocked": {
      "title": "Er zijn geen verzoeken met tracking geblokkeerd die op deze pagina worden geladen. Als de verzoeken van een bedrijf worden geladen, kunnen deze bedrijven een profiel van je maken."
    },
    "trackerNetworksSummaryNoneFound": {
      "title": "We hebben geen verzoeken met tracking op deze pagina gevonden."
    },
    "trackerNetworksSummaryNone": {
      "title": "We hebben geen bedrijven gevonden die verzoeken met tracking op deze pagina proberen te laden."
    },
    "trackerNetworksSummaryAllowedOnly": {
      "title": "Om te voorkomen dat deze site niet meer goed werkt, hebben we geen enkel verzoek met tracking van bedrijven op deze pagina geblokkeerd."
    },
    "trackerNetworksSummaryProtectionsOff": {
      "title": "Verzoeken met tracking werden niet verhinderd om te laden omdat beschermingen zijn uitgeschakeld voor deze site. Als de verzoeken van een bedrijf worden geladen, kunnen ze een profiel van je maken."
    },
    "createNewDuckAddress": {
      "title": "Privé-Duck Address genereren"
    },
    "createNewDuckAddressCopied": {
      "title": "Naar je klembord gekopieerd!"
    },
    "websiteNotWorkingQ": {
      "title": "Werkt de website niet zoals verwacht?"
    },
    "websiteNotWorkingPrompt": {
      "title": "Werkt de website niet?"
    },
    "websiteNotWorkingCta": {
      "title": "Defecte website melden"
    },
    "websiteNotWorkingAdvice": {
      "title": "Het kan helpen om de beveiliging uit te schakelen."
    },
    "websiteNotWorkingPromptFollowUp": {
      "title": "Werkt de site nog steeds niet?"
    },
    "takePrecautions": {
      "title": "Neem voorzorgsmaatregelen"
    },
    "majorTrackingNetworkDesc": {
      "title": "{blocked, select, true {<b>Deze site is eigendom van {companyDisplayName}</b>. Dit bedrijf beheert een netwerk met trackers op {companyPrevalence}% van de meest bezochte websites. We hebben een aantal verzoeken op deze pagina kunnen blokkeren.} other {<b>Deze site is eigendom van {companyDisplayName}</b>. Dit bedrijf beheert een netwerk met trackers op {companyPrevalence}% van de meest bezochte websites. }}"
    },
    "trackersBlockedDesc": {
      "title": "{companyCount, plural, =0 {We hebben het laden van trackingverzoeken van bepaalde bedrijven op deze pagina geblokkeerd.} =2 {We hebben voorkomen dat <b>{firstCompany}</b> en <b>{secondCompany}</b> verzoeken met tracking op deze pagina laden.} =3 {We hebben voorkomen dat <b>{firstCompany}</b>, <b>{secondCompany}</b> en <b>{thirdCompany}</b> verzoeken met tracking op deze pagina laden.} =4 {We hebben voorkomen dat <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b> en <b>{fourthCompany}</b> verzoeken met tracking op deze pagina laden.} =5 {We hebben voorkomen dat <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b>, <b>{fourthCompany}</b> en <b>1 ander bedrijf</b> verzoeken met tracking op deze pagina laden.} one {We hebben voorkomen dat <b>{firstCompany}</b> verzoeken met tracking op deze pagina laadt.} other {We hebben voorkomen dat <b>{firstCompany}</b>, <b>{secondCompany}</b>, <b>{thirdCompany}</b>, <b>{fourthCompany}</b> en <b>{othersCount} andere bedrijven</b> verzoeken met tracking op deze pagina laden.}}"
    },
    "cookiesMinimized": {
      "title": "Beheerde cookies"
    },
    "cookiesHidden": {
      "title": "Cookiepop-up verborgen"
    },
    "cookiesHiddenSummary": {
      "title": "We konden de cookiepop-up op deze site alleen verbergen omdat er geen opties waren om cookievoorkeuren te beheren. De rest van onze bescherming tegen webtracking is nog steeds van toepassing."
    },
    "cookiesMinimizedSummary": {
      "title": "We hebben je cookievoorkeuren ingesteld voor maximale privacy en de pop-up gesloten."
    },
    "cookiesMinimizedSettings": {
      "title": "Uitschakelen in Instellingen"
    },
    "connectionSecure": {
      "title": "Verbinding is versleuteld"
    },
    "connectionNotSecure": {
      "title": "Verbinding is niet versleuteld"
    },
    "connectionNotSecureInvalidCertificate": {
      "title": "De verbinding is misschien onveilig"
    },
    "trackerNetworksDesc": {
      "title": "Verzoeken geblokkeerd bij laden"
    },
    "trackerNetworksNotBlocked": {
      "title": "Geen verzoeken met tracking geblokkeerd"
    },
    "trackerNetworksNotFound": {
      "title": "Geen verzoeken met tracking gevonden"
    },
    "thirdPartiesLoaded": {
      "title": "Verzoeken van derden geladen"
    },
    "thirdPartiesNoneFound": {
      "title": "Geen verzoeken van derden gevonden"
    },
    "firstPartyDesc": {
      "title": "{companyName} is eigenaar van deze site en de bekende trackers op deze pagina. Die hebben we dus niet geblokkeerd."
    },
    "noTrackersFound": {
      "title": "We hebben geen bedrijven gevonden die verzoeken met tracking op deze pagina proberen te laden."
    },
    "trackersFoundForAllowlisted": {
      "title": "Trackers helpen bedrijven om een profiel van je te maken. We hebben deze bedrijven gevonden die je activiteit op deze pagina volgen."
    },
    "trackersFoundAndBlocked": {
      "title": "Trackers helpen bedrijven om een profiel van je te maken. We hebben voorkomen dat deze bedrijven je activiteit op deze pagina volgen."
    },
    "trackerNetworkUnknown": {
      "title": "Tracker-netwerk onbekend"
    },
    "trackerDomainsForCompany": {
      "title": "Trackerdomeinen voor {companyName}"
    },
    "zeroTrackersFound": {
      "title": "0 trackers gevonden"
    },
    "trackerOwnedByThisSite": {
      "title": "Trackers die eigendom zijn van deze site (Toegestaan)"
    },
    "trackerCountForDomain": {
      "title": "{blocked, select, true {{trackerCount, plural, one {{trackerCount} tracker geblokkeerd op {domain}} other {{trackerCount} trackers geblokkeerd op {domain}}}} other {{trackerCount, plural, one {{trackerCount} tracker gevonden op {domain}} other {{trackerCount} trackers gevonden op {domain}}}}}"
    },
    "trackerLimitationsNote": {
      "title": "Let op: beperkingen op het platform kunnen ons vermogen om alle verzoeken te detecteren beperken."
    },
    "trackerAboutLink": {
      "title": "Over onze bescherming tegen webtracking"
    },
    "trackerAdLink": {
      "title": "Hoe onze zoekadvertenties onze bescherming beïnvloeden"
    },
    "sectionHeadingAdAttribution": {
      "title": "De verzoeken van het volgende domein zijn geladen omdat er onlangs in DuckDuckGo op een advertentie van {domain} is geklikt. Deze verzoeken helpen de effectiviteit van advertenties te beoordelen. Geen enkele advertentie op DuckDuckGo maakt een profiel van je."
    },
    "sectionHeadingIgnore": {
      "title": "De verzoeken van het volgende domein werden geladen om te voorkomen dat de site niet meer goed zou werken."
    },
    "sectionHeadingFirstParty": {
      "title": "De verzoeken van de volgende domeinen zijn geladen omdat ze aan {domain} zijn gekoppeld."
    },
    "sectionHeadingThirdParty": {
      "title": "De verzoeken van de volgende domeinen zijn ook geladen."
    },
    "sectionHeadingProtectionsDisabled": {
      "title": "De verzoeken van de volgende domeinen zijn geladen omdat de beveiliging is uitgeschakeld."
    },
    "thirdPartiesSummaryLoaded": {
      "title": "De volgende verzoeken van domeinen van derden zijn geladen. Als de verzoeken van een bedrijf worden geladen, kunnen ze een profiel van je maken, hoewel onze andere beschermingen tegen webtracking nog steeds van toepassing zijn."
    },
    "thirdPartiesSummaryProtectionsOff": {
      "title": "De volgende verzoeken van domeinen van derden zijn geladen. Als de verzoeken van een bedrijf worden geladen, kunnen ze een profiel van je maken, hoewel onze andere beschermingen tegen webtracking nog steeds van toepassing zijn."
    },
    "thirdPartiesSummaryNone": {
      "title": "We hebben geen verzoeken van domeinen van derden gedetecteerd."
    },
    "analyticsCategory": {
      "title": "Analyses"
    },
    "advertisingCategory": {
      "title": "adverteren"
    },
    "socialCategory": {
      "title": "Sociaal netwerk"
    },
    "contentDeliveryCategory": {
      "title": "Levering van inhoud"
    },
    "embeddedContentCategory": {
      "title": "Geïntegreerde inhoud"
    },
    "searchPlaceholder": {
      "title": "Zoek in DuckDuckGo"
    },
    "searchGoButton": {
      "title": "Zoeken"
    },
    "optionsButton": {
      "title": "Meer opties"
    },
    "navigationComplete": {
      "title": "Klaar"
    },
    "navigationClose": {
      "title": "Sluiten"
    },
    "navigationCancel": {
      "title": "Annuleren"
    },
    "navigationBack": {
      "title": "Terug"
    },
    "enableProtectionsSwitch": {
      "title": "Bescherming inschakelen"
    },
    "disableProtectionsSwitch": {
      "title": "Bescherming uitschakelen"
    },
    "errorMessage": {
      "title": "Er is iets misgegaan en we kunnen deze inhoud niet laden. Probeer de pagina opnieuw te laden."
    }
  }
}